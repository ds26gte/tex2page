...
style sheets with \p{\inputcss}.   Some general-purpose
...
thence to PDF using a utility like \p{epstopdf}; or \item(ii)
...
\item(ii) To generate PNG, in \p{lambda.mp} set
...
\item(i) Setting  \p{\tracingcommands=1} tells \TeX2page to log all calls
to atomic commands.

\item(ii) Setting  \p{\tracingmacros=1} tells
...
\item(iii) The command \p{\tracingall} turns {\em on} both
\p{\tracingcommands} and \p{\tracingmacros}.

\index{errmessage@\p{\errmessage}}

\item(iv) The command \p{\errmessage} can be used to generate
...
\item(ii) \p{.tex2page.hdir} in the working directory.

\item(iii) \p{.tex2page.hdir} in the user’s \p{HOME} directory.
...
\item(i) Type \p{./configure --help} at your OS command line
