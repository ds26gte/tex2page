  ...
  (let ((s s) (r '()))
    (loop
      (let ((i (string-index s sep)))
        ; return not in subform tail call, but ok since we won't be translating this to Scheme
...
(defun edit-offending-file ()
  (let ((calling-from-text-editor-p (retrieve-env "VIMRUNTIME")) it)
          ...
          (let ((texedit-string (retrieve-env "TEXEDIT"))
            ...
            (when texedit-string
              (cond ((setq it (search "%d" texedit-string))
                     (setq texedit-string (string-append (subseq texedit-string 0 it)
                                            (write-to-string *input-line-no*)
                                            (subseq texedit-string (+ it 2)))))
            ...
            (unless texedit-string
              (when ill-formed-texedit-p
                ...
                (terpri))
              (setq texedit-string
                    (string-append (or (retrieve-env "EDITOR") "vi")
                      " +" (write-to-string *input-line-no*)
                      " " *current-source-file*)))
            (when texedit-string
...
(defun get-number-corresp-to-ctl-seq (x)
  ...
  (let (it)
    (cond ((string= x "\\the") (get-number-corresp-to-ctl-seq (get-ctl-seq)))
          ...
          ((find-chardef x))
          ((setq it (find-countdef x))
           (find-count it))
          ((setq it (find-dimendef x))
           (find-dimen it))
          ((find-count x))
          ((find-dimen x))
          ;not sure about following
          ((setq it (resolve-defs x))
           (char-code (char it 0)))
...
(defun close-html-output-stream (op)
  (let ((p (ostream*-stream op))
        (backed-up-spaces (ostream*-hbuffer op)))
    (when (consp backed-up-spaces)
      (setf (ostream*-hbuffer op) '())
      (mapc (lambda (x) (princ x p)) (nreverse backed-up-spaces)))
    (close-output-port p)))
...
(defun emit (s)
  (when *emit-enabled-p*
    (let* ((p (ostream*-stream *html*))
           (backed-up-spaces (ostream*-hbuffer *html*)))
      (when (consp backed-up-spaces)
        (setf (ostream*-hbuffer *html*) '())
        (mapc (lambda (x) (princ x p)) (nreverse backed-up-spaces)))
      (unless *in-para-started-p* (setq *in-para-started-p* t))
      (princ s p))))
...
(defun the-count (dracula)
  (let ((n (find-countdef dracula)))
    (if n (find-count n)
        (terror 'the-count "Missing count register."))))
  ...
  (let ((cs (get-ctl-seq)) it)
    ...
    (cond ((setq it (find-countdef cs))
           (let ((countval (find-count it)))
             ...
             (tex-def-count it (floor countval (get-number)) globalp)))
          ((setq it (find-dimendef cs))
           (let ((dimenval (find-dimen it)))
             (unless dimenval (terror 'do-divide "Missing dimen register."))
             (tex-def-dimen it (floor dimenval (get-number)) globalp)))
...
(defun the-toks (ctlseq)
  (let (it)
    (cond ((setq it (find-toksdef ctlseq))
           (find-toks it))
...
(defun activate-cdef (c)
  (let ((y (find-cdef-in-top-frame c)))
    (unless y
      (setq y (ensure-cdef c (top-texframe)))
      (let ((d (find-cdef c)))
...
(defun do-para (&optional (noindentp nil))
  (when (consp (ostream*-hbuffer *html*))
    (setf (ostream*-hbuffer *html*) '()))
...
(defun do-caption ()
  ...
  (let* ((i-fig (index-of *tabular-stack* :figure ))
         (i-tbl (index-of *tabular-stack* :table ))
...
(defun expand-html-index ()
  (let* ((s (get-peeled-group))
         ...
         (page-mention (1+ (gethash pageno *index-page-mention-alist* 0)))
         )
    ...
    (setf (gethash pageno *index-page-mention-alist*) page-mention)
    (emit-link-stop)))
...
(defun set-text-width ()
  (let* (it
          (hsize (cond ((setq it (find-def "\\TZPhsize"))
...
(defun dump-tex-preamble (o)
  ...
  (case *tex-format*
    ...
    (t (princ *imgpreamble* o)
     ...
     (when (member :epsfbox *imgpreamble-inferred*)
       (princ "\\ifx\\epsfbox\\UNDEFINED" o)
       (princ "\\ifx\\directlua\\UNDEFINED\\input epsf \\else" o)
       (princ "\\input miniltx \\input graphicx " o)
       (princ "\\def\\epsfbox#1{\\convertMPtoPDF{#1}{1}{1}}" o)
       (princ "\\fi\\fi" o)
       (terpri o))
     (princ "\\nopagenumbers" o)
     (terpri o))))
...
(defun do-write-aux (o)
  (declare (fixnum o))
  (let ((output (tex-write-output-string (get-peeled-group))) it)
    (cond ((and (= o 18) *enable-write-18-p*)
          ...
          ((setq it (find-ostream o))
           (when (eq it :free) (terror 'do-write-aux))
           (princ output it)
           (princ #\space it))
...
(defun initialize-globals ()
  ...
  (plain-dimen (string-append "\\z" (string *small-commercial-at*)) 0) ;TB p347
...
(defun do-arabic ()
  (let* ((counter-name (ungroup (get-group)))
         ...
         (n (counter*-value counter)))
    (if n (emit n)
        (trace-if nil "do-arabic failed"))))
...
(defun do-newcommand (renewp)
  ...
  (let* ((lhs (string-trim-blanks (ungroup (get-token))))
         ...
         (argc (cond ((setq it (get-bracketed-text-if-any))
                      (setq optarg (get-bracketed-text-if-any))
                      (read-from-string (string-trim-blanks it)))
                     (t 0)))
...
(defun dump-groupoid (p)
  (declare (stream p))
  (ignorespaces)
  (let ((write-char #'write-char) (d (get-actual-char)) c)
    (unless p
...
(defun do-verbatim-latex (env)
  ...
  (bgroup)
  (emit "<pre class=verbatim>")
  (let ((*verb-visible-space-p* (eat-star)) it)
...
(defun set-latex-counter-aux (counter-name addp new-value)
  ...
  (let (it)
    (cond ((setq it (gethash counter-name *dotted-counters*))
           (if addp
               (incf (counter*-value it) new-value)
               (setf (counter*-value it) new-value)))
          (t
            (let ((count-seq (string-append "\\" counter-name)))
              (cond ((setq it (section-ctl-seq-p count-seq))
                     (setf (gethash it *section-counters*)
                           (if addp
                               (+ new-value (gethash it *section-counters* 0))
                               new-value)))
  ...
  (cond ((= (catcode c) **comment**) (do-comment))
        ...
        ((= (catcode c) **alignment**)
         (cond ((consp *tabular-stack*)
               ...
               (t (emit-html-char c))))
        ((char= c #\|)
         (if (and (consp *tabular-stack*) (eq (car *tabular-stack*) :ruled-table))
             (do-ruledtable-colsep) (emit c)))
        ((char= c #\newline)
         ; (format t "doing do-tex-char newline~%")
         (do-newline))
        (t
         (cond (*math-mode-p*
                (case c
...
(defun do-tex-ctl-seq (z)
  ...
  (let (it)
    (cond ((setq it (resolve-defs z))
          ...
          ((setq it (find-countdef z))
           (do-count= it nil))
          ((setq it (find-dimendef z))
           (do-dimen= it nil))
          ((setq it (find-toksdef z))
           (do-toks= it nil))
          (t (do-tex-prim z))))
...
(defun do-input ()
  (ignorespaces)
  (let* ((f (get-filename-possibly-braced))
         (boilerplate-index *inputting-boilerplate-p*) it)
...
(defun do-scm-slatex-lines (env display-p result-p)
  ...
  (let ((*catcodes* *catcodes*)
    ...
    (emit "</pre></div>")
    (egroup)
    (cond (display-p (do-para :noindent))
...
(defun do-index-help (idx-entry)
  ...
  (let ((tag (string-append *html-node-prefix* "index_"
               (write-to-string *index-count*))) it)
    (emit-anchor tag)
    (unless *index-stream*
    ...
    (cond ((or (search "|see{" idx-entry)
          ...
          ((setq it (search "|(" idx-entry))
           (display-index-entry (subseq idx-entry 0 it) *index-stream*)
