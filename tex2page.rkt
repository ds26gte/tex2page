...
(define *tex2page-copyright-notice*
 (let ((%type 'string)
       (%ee
        (list "Copyright (C) 1997-" (substring *tex2page-version* 0 4)
              " Dorai Sitaram")))
   (let ((%res
          (if (eq? %type 'string)
              ""
...
(define (char-tex-alphabetic-p c)
 (or (char-alphabetic? c) (member c *tex-extra-letters*)))
...
(define (write-log x)
 ...
 (cond
  ((and *write-log-possible-break-p* (> *write-log-index* *write-log-max*))
   (newline *log-port*) (newline) (set! *write-log-possible-break-p* false)
...
(define (terror where . args) (write-log ':separation-newline) (write-log "! ")
 ...
 (display-error-context-lines) (close-all-open-ports) (output-stats)
...
(define (invisible-space-p x)
 (or (eq? x *invisible-space*) (eq? x *outer-invisible-space*)))

(define (outer-invisible-space-p x) (eq? x *outer-invisible-space*))

(define (check-outerness c)
 (cond ((eq? c *outer-invisible-space*) (set! *outer-p* true) *outer-p*)
       (else false)))
...
(define (ignorespaces . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg))
       (stop-at-first-newline-p false))
   (when (< 0 %lambda-rest-arg-len)
     (set! stop-at-first-newline-p (list-ref %lambda-rest-arg 0)))
   (cond
                   ...
                   ((char=? c #\newline)
                    (cond (newline-active-p (return))
                          ...
                          (else (get-actual-char)
                           (set! newline-already-read-p true)
                           (cond (stop-at-first-newline-p (return))
                                 (else false)))))
...
(define (esc-char-p c) (member c *esc-chars*))
             ...
             (let ((nesting 0) (s null))
               (let* ((%loop-returned false)
                 ...
                 (let %loop
                   ()
                   (let ((c (get-actual-char)))
                     (cond ((eq? c ':eof-object) (terror 'get-url "Missing }"))
                           (else false))
                     (cond
                      ((and *comment-char* (char=? c *comment-char*))
...
(define (bgroup) (set! *tex-env* (cons (make-texframe*) *tex-env*))
 (cond
    ...
    (lambda ()
      (let ((res (get-output-string *html*)))
        (set! res
         (let ((%type 'string)
               (%ee
                (list "<table><tr><td align=center>" res "</td></tr></table>")))
           (let ((%res
                  (if (eq? %type 'string)
                      ""
                      null)))
         ...
         ((or *math-delim-left* *math-delim-right*)
          ...
          (set! res
           (let ((%type 'string)
                 (%ee
                  (list "<table><tr><td>"
                        (tex-math-delim-string *math-delim-left*) "</td><td>"
                        res "</td><td>"
...
(define (set-catcode c val)
 (cond ((not (= val 13)) (deactivate-cdef c)) (else false))
 (cond
  ((not (= val 11)) (set! *tex-extra-letters* (remove c *tex-extra-letters*))
   *tex-extra-letters*)
  (else false))
 (case val
   ((0) (set! *esc-chars* (cons c *esc-chars*)) *esc-chars*)
   ((11)
    (set! *tex-extra-letters* (cons c *tex-extra-letters*))
    *tex-extra-letters*)
   ((13) (activate-cdef c))))
...
(define (do-para) (do-end-para)
 (let ((in-table-p
   ...
   (cond (in-table-p (emit "</td></tr><tr><td>") (emit-newline)) (else false))
   (emit "<p>")
   (set! *in-para-p* true)
   *in-para-p*))
...
(define (do-para-nopadding) (do-end-para) (emit-newline)
 (emit "<p class=nopadding>") (set! *in-para-p* true) *in-para-p*)
...
(define (do-maketitle) (do-end-para) (bgroup)
 ...
 (emit "<div align=center>") (emit-newline) (tex2page-string "\\TIIPauthor")
 (do-para) (tex2page-string "\\TIIPdate") (do-end-para) (emit "</div>")
...
(define (section-type-to-depth sectype)
 (cond ((string->number sectype)) ((string=? sectype "chapter") 0)
       ...
       ((string=? sectype "paragraph") 4) ((string=? sectype "subparagraph") 5)
       (else 3)))
...
(define (do-marginpar) (get-bracketed-text-if-any)
 (emit "<table align=left border=2><tr><td>") (tex2page-string (get-group))
 (emit "</td></tr></table>"))
...
(define (do-end-equation) (do-end-para) (emit "</td>")
 (cond
  ...
  (else false))
 (emit "</tr>") (emit-newline) (emit "</table></div>")
 ...
 (let ((old-html *html*) (fn-tmp-port (open-output-string)))
   ...
   (add-aftergroup-to-top-frame
    (lambda ()
      (set! *footnote-list*
       (cons
        (make-footnotev* ':mark fnmark ':text (get-output-string fn-tmp-port)
     ...
     (let ((blue false))
       (let ((hue6 (* 6 hue)))
         (let ((i (inexact->exact (floor hue6))))
           (let ((f (- hue6 i)))
             (case i
               ((0) (set! red 0) (set! green (- 1 f)) (set! blue 1) blue)
 ...
 (let ((hue
       ...
       (brightness
        (cond ((<= w 362.857) 0) ((< w 420) (+ 0.3 (* 0.7 (/ (- w 380) 40))))
              ((<= w 700) 1) ((< w 814.285) (+ 0.3 (* 0.7 (/ (- w 780) -80))))
              (else 0))))
 ...
 (let ((label-ref (label-bound-p label ext-file)))
   (let ((label-text
          (cond (link-text (tex-string-to-html-string link-text))
                (label-ref (label*-value label-ref)) (else label))))
     (cond
      (label-ref
   ...
   (let* ((%loop-returned false)
     ...
     (let %loop
       ...
       (cond ((eat-word "height") (set! height (get-pixels)) height)
             ...
             ((eat-word "depth") (get-pixels)) (else (return)))
       (if %loop-returned
           %loop-result
           (%loop))))
   (emit "<img")
...
(define (output-html-preamble)
 (cond
  ((string? *doctype*) (emit "<!DOCTYPE ") (emit *doctype*) (emit ">")
   (emit-newline))
  (else false))
 (emit "<html>") (emit-newline) (emit "<!--") (emit-newline)
 (emit "Generated from ") (emit *main-tex-file*) (emit " by tex2page, ")
 (emit "v. ") (emit *tex2page-version*) (emit-newline)
 (emit *tex2page-copyright-notice*) (emit-newline) (emit "(running on ")
 (emit *scheme-version*) (emit ")") (emit-newline) (emit *tex2page-website*)
...
(define (do-bye) (check-tex2page-lisp) (note-down-tex2page-flags)
 ...
 (write-aux `(!last-page-number ,*last-page-number*)) (do-end-page)
 (cond
...
(define (do-math-fragment s . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (display-p false))
   ...
   (let ((old-math-mode-p *math-mode-p*)
     ...
     (cond (display-p (emit "<table><tr><td>")) (else false))
     (bgroup)
     (toss-back-char #\})
     (toss-back-string s)
     (add-aftergroup-to-top-frame
 ...
 (let ((nesting 0) (escape-p false) (c false))
   (let* ((%loop-returned false)
     ...
     (let %loop
       ...
       (cond ((eq? c ':eof-object) (terror 'dump-till-char "Missing " d "."))
             (else false))
            ...
            (let ((env-nesting 0))
              (let* ((%loop-returned false)
                ...
                (let %loop
                  ...
                  (let ((c (snoop-actual-char)))
                    (cond
                     ((eq? c ':eof-object) (terror 'dump-till-end-env env))
                     (else false))
                    (cond
                     ((esc-char-p c)
                      (let ((x (get-ctl-seq)))
...
(define (do-typein)
 (let ((ctlseq (get-bracketed-text-if-any))
   ...
   (let ((l (read-tex-line p)))
     (cond ((eq? l ':eof-object) (set! l "") l) (else false))
      ...
      ((not tex-prog-name)
       (let ((d (find-def "\\TZPtexprogname")))
         (cond (d (set! tex-prog-name (tdef*-expansion d)) tex-prog-name)
               (else false)))
       (cond ((not tex-prog-name) (set! tex-prog-name "xetex") tex-prog-name)
             (else false))
                                     ...
                                     (set! x
                                      (let ((%read-line-res (read-line i)))
                                        (when (eof-object? %read-line-res)
                                          (set! %read-line-res ':eof-object))
                                        %read-line-res))
                                     (cond ((eq? x ':eof-object) (return true))
                                           (else false))
                                     (unless %loop-returned
                                       (cond
...
(define (do-mfpic-opengraphsfile)
 ...
 (display *mfpic-file-stem* *mfpic-port*) (display #\} *mfpic-port*)
 (newline *mfpic-port*)
...
(define (do-mfpic-closegraphsfile) (display "\\closegraphsfile" *mfpic-port*)
 (newline *mfpic-port*)
 (let ((%close-port-arg *mfpic-port*))
   ((if (input-port? %close-port-arg)
        close-input-port
...
(define (do-eqalign type) (ignorespaces)
 (let ((c (get-actual-char)))
   (cond ((eq? c ':eof-object) (terror 'do-eqalign "Missing {")) (else false))
...
(define (eat-till-char d)
 (let* ((%loop-returned false)
   ...
   (let %loop
     ()
     (let ((c (get-actual-char)))
       (cond ((or (eq? c ':eof-object) (char=? c d)) (return)) (else false)))
     (if %loop-returned
         %loop-result
         (%loop)))))
...
(define (do-rawhtml)
 (let* ((%loop-returned false)
   ...
   (let %loop
     ()
     (let ((c (snoop-actual-char)))
       (cond ((eq? c ':eof-object) (terror 'do-rawhtml "missing \\endrawhtml"))
             ((esc-char-p c)
              (let ((x (get-ctl-seq)))
...
(define (initialize-globals) (set! *global-texframe* (make-texframe*))
 ...
 (tex-def-dimen "\\hoffset" 0 true) (tex-def-dimen "\\voffset" 0 true)
 (tex-def-dimen "\\epsfxsize" 0 true) (tex-def-dimen "\\epsfysize" 0 true)
 ...
 (tex-def-dimen "\\leftskip" 0 true) (tex-def-dimen "\\parfillskip" 0 true)
 ...
 (tex-def-toks "\\everypar" "" true) (tex-def-toks "\\headline" "" true)
...
(define (get-dimen ctlseq)
 (cond ((find-dimen ctlseq)) (else (tex-length 6.5 ':in))))

(define (the-count dracula) (or (find-count dracula) (terror 'the-count)))

(define (do-count= z globalp) (get-equal-sign)
 (tex-def-count z (get-number) globalp))

(define (do-toks= z globalp) (get-equal-sign)
 (tex-def-toks z (get-group) globalp))

(define (do-dimen= z globalp) (get-equal-sign)
 (tex-def-dimen z (get-scaled-points) globalp) (ignorespaces))

(define (get-gcount ctlseq)
 (table-get ctlseq (texframe*-counts *global-texframe*) 0))
...
(define (expand-the)
 (let ((ctlseq (get-ctl-seq)))
   (cond
    ...
    ((find-toks ctlseq)) (else (trace-if false "expand-the failed")))))
...
(define (make-reusable-img globalp)
  ...
  ((aux
    (lambda ()
      (let ((c (snoop-actual-char)))
        (cond
         ((eq? c ':eof-object)
          (terror 'get-def-arguments "EOF found while scanning definition of "
           lhs))
         (else false))
        (cond
         ((esc-char-p c)
              ...
              (let %loop
                ...
                (let ((c (snoop-actual-char)))
                  ...
                  (cond
                   ...
                   ((char=? c c0) (return s))
                   ((esc-char-p c) (set! s (cons (get-actual-char) s))
                    (set! escape-p true) escape-p)
    ...
    (let ((tmplt (get-halign-template)))
      (let* ((%loop-returned false)
        ...
        (let %loop
          ...
          (let ((c (snoop-actual-char)))
            (cond
             ((eq? c ':eof-object) (terror 'do-halign "Eof inside \\halign"))
             (else false))
            (cond
 ...
 (let ((settabs-spec ""))
   (let* ((%loop-returned false)
     ...
     (let %loop
       ()
       (let ((x (get-token/ps)))
         (cond ((eq? x ':eof-object) (terror 'do-settabs "Eof in \\settabs"))
               (else false))
         (cond
          ((string=? x "\\columns")
           (tex2page-string
            (let ((%type 'string)
              ...
              (let ((%res
                ...
                %res)))
           (return))
          ((string=? x "\\cr") (tex-def-count "\\TIIPsettabscolumns" 0 false)
           (return))
          (else
           (set! settabs-spec
 ...
 (let ((cell-contents "") (cell-width ""))
   ...
   (let* ((%loop-returned false)
     ...
     (let %loop
       ()
       (let ((x (get-token/ps)))
         (cond ((eq? x ':eof-object) (terror 'do-tablign "Eof in \\tabalign"))
               (else false))
         (cond
    ...
    (let ((k k) (r r))
      (let* ((%loop-returned false)
        ...
        (let %loop
          ()
          (cond
           ((>= k n) (cond ((= k 0) (ignorespaces true)) (else false))
            (return r))
           (else false))
          (unless %loop-returned
                    ...
                    (cond
                     ...
                     ((begin (set! *it* (find-def x)) *it*)
                      (let ((y *it*))
                        (cond
                         ((and (null? (tdef*-argpat y)) (not (tdef*-optarg y))
                               (not (tdef*-thunk y)) (not (tdef*-prim y))
                               (not (tdef*-defer y)))
                          (toss-back-char *invisible-space*)
...
(define (expand-tex-macro optarg argpat rhs)
 (let ((k 0))
   (let ((r
          (if (not optarg)
              null
...
(define (do-verb-delimed d)
 (let* ((%loop-returned false)
   ...
   (let %loop
     ...
     (let ((c (get-actual-char)))
       (cond
        ((eq? c ':eof-object) (terror 'do-verb-delimed "Eof inside verbatim"))
        (else false))
...
(define (get-char-definitely c0) (ignorespaces)
 (let ((c (get-actual-char)))
   (cond ((eq? c ':eof-object) (terror 'get-char-defnitely "Runaway argument"))
         (else false))
                 ...
                 (if (string=? cs "\\end")
                     (cond
                      ((begin (set! *it* (get-grouped-environment-name-if-any))
                        *it*)
                       (let ((e *it*))
                         (cond ((string=? *it* env) (return)) (else false))
                         (emit-html-string cs)
                         (emit-html-char #\{)
                         (emit-html-string e)
                         (emit-html-char #\})))
   ...
   (fluid-let ((*in-alltt-p* %fluid-var-*in-alltt-p*))
    (let* ((%loop-returned false)
      ...
      (let %loop
        ()
        (let ((c (snoop-actual-char)))
          (cond ((eq? c ':eof-object) (terror 'do-alltt "Eof inside alltt"))
                (else false))
          (case c
            ((#\\) (do-tex-ctl-seq (get-ctl-seq)))
            ((#\{) (get-actual-char) (bgroup))
  ...
  (lambda ()
    (let* ((%loop-returned false)
      ...
      (let %loop
        ()
        (ignore-all-whitespace)
        (cond ((eq? (snoop-actual-char) ':eof-object) (return)) (else false))
        (unless %loop-returned
   ...
   (let %loop
     ...
     (let ((c (get-actual-char)))
       (cond ((eq? c ':eof-object) (return))
             ((char=? c #\|)
              (let ((c2 (snoop-actual-char)))
                (cond ((eq? c2 ':eof-object) (scm-emit-html-char c) (return))
   ...
   ((:eqalign :eqalignno :displaylines :pmatrix :mathbox)
    (cond
     ((not (char=? (snoop-actual-char) #\}))
      ...
      (emit-newline) (emit "<tr><td align=center>")
 ...
 (let ((lhs (get-raw-token/is)))
   ...
   (let ((n (get-number-or-false)))
     (if n
         ...
         (terror 'do-chardef "non-number found while scanning definition of "
          lhs)))))
...
(define (tex-math-cal c)
 (let ((%type 'string)
       (%ee
        (list "&#x"
              (write-to-string
               (+ 120016 (- (char->integer c) (char->integer #\A))) ':base 16)
              ";")))
   (let ((%res
          (if (eq? %type 'string)
              ""
...
(define (do-tex-ctl-seq z) (trace-if (> (find-count "\\tracingcommands") 0) z)
 (cond
  ((begin (set! *it* (resolve-defs z)) *it*)
   (let ((s *it*))
     (trace-if (> (find-count "\\tracingmacros") 0) "    --> " s)
     (toss-back-char *invisible-space*)
     (toss-back-string s)))
   ...
   (fluid-let ((*filename-delims* %fluid-var-*filename-delims*))
    (let* ((%loop-returned false)
      ...
      (let %loop
        ...
        (ignorespaces)
        (let ((c (snoop-actual-char)))
          (cond ((eq? c ':eof-object) (terror 'do-includeonly)) (else false))
