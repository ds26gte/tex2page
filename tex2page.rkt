...
(define *tex2page-version* "20170107")
     ...
     (let ((s (list c)) (nesting 0) (escape-p false))
       (let* ((%loop-returned false)
         ...
         (let %loop
           ...
           (let ((c (get-actual-char)))
             (cond
              ((eq? c ':eof-object) (terror 'get-group "Runaway argument?"))
              (else false))
             (cond
              (escape-p
               (let ((%push-new-stack (cons c s)))
               ...
               (let ((nesting 0) (s null))
                 (let* ((%loop-returned false)
                   ...
                   (let %loop
                     ...
                     (let ((c (get-actual-char)))
                       (cond
                        ((eq? c ':eof-object) (terror 'get-url "Missing }"))
                        (else false))
                       (cond
                        ((and *comment-char* (char=? c *comment-char*))
 ...
 (lambda ()
   ...
   (cond
    ((and *in-display-math-p* (not *math-script-mode-p*))
     (let ((s (get-token)))
       (cond
        ((string=? s ")") (set! *math-delim-right* ':rparen)
         *math-delim-right*)
...
(define perform-afterassignment
 (lambda ()
   (let ((z *afterassignment*))
     (cond (z (set! *afterassignment* false) (do-tex-ctl-seq z))
           (else false)))))
...
(define cleanse-tdef
 (lambda (d)
   (set!tdef*-argpat d null)
   (tdef*-argpat d)
   (set!tdef*-expansion d "")
   (tdef*-expansion d)
   (set!tdef*-optarg d false)
   (tdef*-optarg d)
   (set!tdef*-thunk d false)
   (tdef*-thunk d)
   (set!tdef*-prim d false)
   (tdef*-prim d)
   (set!tdef*-defer d false)
   (tdef*-defer d)))
                     ...
                     (let* ((%loop-returned false)
                       ...
                       (let %loop
                         ...
                         (cond ((> i+3 n-9) (return k))
                               ((string=? (subseq s i i+3) "sub") (set! i i+3)
...
(define section-type-to-depth
 (lambda (sectype)
   (cond ((string->number sectype)) ((string=? sectype "chapter") 0)
         ...
         ((string=? sectype "subparagraph") 5) (else 3))))
 ...
 (lambda (type)
   (cond
    ...
    (else (do-end-para) (bgroup)
     ...
     (cond
      ((and (eqv? type ':eqnarray) (eat-star)) (set! type ':eqnarray*) type)
      (else false))
     (let ((%push-new-stack (cons type *tabular-stack*)))
       (begin (set! *tabular-stack* %push-new-stack) *tabular-stack*))
     (set! *math-mode-p* true) (set! *in-display-math-p* true)
     (let ((eqn-tag
            (let ((%type 'string)
...
(define do-nonumber
 (lambda () (set! *equation-numbered-p* false) *equation-numbered-p*))
...
(define do-node
 (lambda () (set! *recent-node-name* (get-peeled-group)) *recent-node-name*))
           ...
           (let ((c (string-ref s i)))
             ...
             (cond
              ...
              ((char=? c #\&)
               ...
               (set! skip-entity true) skip-entity)
              (else
               (let ((%tmp (+ res 1)))
                 (begin (set! res %tmp) res))))))
         (if %loop-returned
                              ...
                              (cond
                               ((and g (string=? g env))
                                (let ((%tmp (+ env-nesting 1)))
                                  (begin (set! env-nesting %tmp) env-nesting)))
                               (else false))))
                           ((string=? x "\\end")
                            (let ((g (get-grouped-environment-name-if-any)))
                              (cond
                        ...
                        (cond
                         ((char=? c #\{)
                          (let ((%tmp (+ brace-nesting 1)))
                            (begin (set! brace-nesting %tmp) brace-nesting)))
                         ((char=? c #\})
                          (let ((%tmp (- brace-nesting 1)))
                                ...
                                (cond
                                 ((string=? x "\\or") (ignorespaces)
                                  ...
                                  (let ((%push-new-stack
                                    ...
                                    (begin (set! or-clauses %push-new-stack)
                                     or-clauses))
                                  (return))
...
(define initialize-scm-words
 (lambda ()
   (set! *scm-keywords* (make-table ':test equal?))
...
(define get-dimen
 (lambda (ctlseq) (cond ((find-dimen ctlseq)) (else (tex-length 6.5 ':in)))))
...
(define valid-img-file-p (lambda (f) false))
                        ...
                        (let ((y *it*))
                          (cond
                           ((and (null? (tdef*-argpat y))
                                 (not (tdef*-optarg y)) (not (tdef*-thunk y))
               ...
               (let ((n
                 ...
                 (cond
                  ((and f (file-exists? f)) (do-end-para) (bgroup)
                   ...
                   (table-get f *opmac-verbinput-table*) (emit "</pre>")
                   (egroup) (do-para))
                  (else (non-fatal-error "File " f0 " not found"))))))))))))
...
(define do-endverbatim-eplain
 (lambda () (set! *inside-eplain-verbatim-p* false) *inside-eplain-verbatim-p*))
 ...
 (lambda ()
   (let ((n (get-number-or-false)))
     ...
     (cond ((< n 128) (emit-html-char (integer->char n)))
           (else (emit "&#x") (emit (write-to-string n ':base 16))
            (emit ";"))))))
        ...
        (let %loop
          ...
          (let ((c (snoop-actual-char)))
            ...
            (cond
             ...
             (else
              (let ((%push-new-stack (cons (get-filename) *includeonly-list*)))
...
(define eval-for-tex-only
 (lambda ()
   (set! *eval-for-tex-only-p* true)
   (do-end-page)
   (ensure-file-deleted *html-page*)
   (set! *main-tex-file* false)
...
(define load-aux-file
 (lambda ()
   ...
   (start-css-file)
   (cond
    ((not (null? *toc-list*)) (set! *toc-list* (reverse *toc-list*))
     *toc-list*)
    (else false))
   (cond
...
(define !tex-like-layout
 (lambda () (set! *tex-like-layout-p* true) *tex-like-layout-p*))

(define !head-line (lambda (e) (tex-def-toks "\\headline" e true)))

(define !foot-line (lambda (e) (tex-def-toks "\\footline" e true)))

(define !toc-page (lambda (p) (set! *toc-page* p) *toc-page*))

(define !index-page (lambda (p) (set! *index-page* p) *index-page*))
...
(define !last-page-number
 (lambda (n) (set! *last-page-number* n) *last-page-number*))
...
(define !using-chapters
 (lambda () (set! *using-chapters-p* true) *using-chapters-p*))
...
(define !using-external-program (lambda (x) false))

(define !external-labels (lambda (f) false))

(define !doctype (lambda (d) (set! *doctype* d) *doctype*))
...
(define !preferred-title (lambda (title) (set! *title* title) *title*))
