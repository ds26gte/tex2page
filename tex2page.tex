...
\message{version 20161206} % last change
...
\def\beginchapter#1 #2 \par{%
  ...
  \noindent\bgroup\chapterfont
  \ifx\beginchapterQnr\empty\else
  \beginchapterQnr\enspace\fi
  #2%
  \egroup\medskip\nobreak\noindent}

% OPmac stuff, unless OPmac already loaded
...
{\ifx\activettcharQcurr\UnDeFiNeD\else
  \catcode\activettcharQcurr=12 \fi
  \catcode`\|=0 |catcode`|\=12
  |long|gdef|begttQii#1\endtt{#1|endgroup}}
