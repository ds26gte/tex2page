...
(define *operating-system*
 (if (getenv "COMSPEC")
  (let ((term (getenv "TERM"))) (if (and (string? term) (string=? term "cygwin")) ':cygwin ':windows))
  ':unix))

(define *scheme-version* (string-append "Racket " (version) " " (symbol->string *operating-system*)))
...
(define (decode-universal-time s)
 (let ((ht (and s (seconds->date s))))
  (cond (ht (list false (date-minute ht) (date-hour ht) (date-day ht) (date-month ht) (date-year ht)))
   (else (list false false false false false false false false false)))))

(define (strftime ignore-format d)
 (let
  ((m (date-minute d)) (h (date-hour d)) (dy (date-day d)) (mo (date-month d)) (y (date-year d))
   (dow (date-week-day d)) (dst (date-dst? d)) (tzsec (date-time-zone-offset d)))
  (let ((tz (and tzsec (/ tzsec 3600))))
   (string-append (vector-ref *week-day-names* dow) ", " (vector-ref *short-month-names* (- mo 1)) " "
    (number->string dy) ", " (number->string y)
    (if tz
     (string-append ", " (let ((h (modulo h 12))) (number->string (if (= h 0) 12 h))) ":"
      (if (< m 10) "0" "") (number->string m) " " (if (<= 0 h 11) "a" "p") "m UTC" (if (> tz 0) "+" "−")
      (number->string (abs tz)))
     "")))))

(define (seconds-to-human-time s) (strftime "%a, %b %e, %Y, %l:%M %p %Z" (seconds->date s)))

(define list-position
 (lambda (x s)
  (let loop ((s s) (i 0)) (cond ((null? s) false) ((eq? (car s) x) i) (else (loop (cdr s) (+ i 1)))))))
     ...
     (lambda (s . ff)
      (let ((s-s (symbol->string s)) (n (length ff)))
       (let* ((n+1 (+ n 1)) (vv (make-vector n+1)))
        (let loop ((i 1) (ff ff))
         (if (< i n+1)
          (let ((f (car ff))) (vector-set! vv i (if (pair? f) (cadr f) (not 't))) (loop (+ i 1) (cdr ff)))
          0))
        (let*
         ((ff-without-colons (map (lambda (f) (symbol->string (if (pair? f) (car f) f))) ff))
          (ff-with-colons (map (lambda (f) (string->symbol (string-append ":" f))) ff-without-colons)))
         `(begin
           (define ,(string->symbol (string-append "make-" s-s))
            (lambda fvfv
             (let ((st (make-vector ,n+1)) (ff ',ff-with-colons)) (vector-set! st 0 ',s)
              ,@(let loop ((i 1) (r '()))
                 (if (>= i n+1) r (loop (+ i 1) (cons `(vector-set! st ,i ,(vector-ref vv i)) r))))
              (let loop ((fvfv fvfv))
               (if (null? fvfv) 0
                (begin (vector-set! st (+ (list-position (car fvfv) ff) 1) (cadr fvfv))
                 (loop (cddr fvfv)))))
              st)))
           ,@(let loop ((i 1) (procs '()))
              (if (>= i n+1) procs
               (loop (+ i 1)
                (let* ((f-s (list-ref ff-without-colons (- i 1))))
                 (cons
                  `(define ,(string->symbol (string-append s-s "-" f-s)) (lambda (x) (vector-ref x ,i)))
                  (cons
                   `(define ,(string->symbol (string-append "set!" s-s "-" f-s))
                     (lambda (x v) (vector-set! x ,i v)))
                   procs))))))
           (define ,(string->symbol (string-append s-s "?"))
            (lambda (x) (and (vector? x) (eq? (vector-ref x 0) ',s)))))))))
     (cdr %so-d))))))
...
(define-syntax cl-with-output-to-string
 (lambda (%so)
  (datum->syntax %so
   (let ((%so-d (syntax->datum %so)))
    (apply (lambda (ignore-wots-arg . body) (list 'with-output-to-string (list* 'lambda '() body)))
     (cdr %so-d))))))

(defstruct table (test eqv?) (alist '()))

(define (table-get k tbl . d)
 (cond ((lassoc k (table-alist tbl) (table-test tbl)) => (lambda (c) (vector-ref (cdr c) 0)))
...
(define (table-put! k tbl v)
 (let ((al (table-alist tbl)))
  (let ((c (lassoc k al (table-test tbl))))
   (if c (vector-set! (cdr c) 0 v) (set!table-alist tbl (cons (cons k (vector v)) al))))))

(define (table-for-each p tbl)
 (for-each (lambda (c) (p (car c) (vector-ref (cdr c) 0))) (table-alist tbl)))

(define (substring? s1 s2)
 (let* ((s1-len (string-length s1)) (s2-len (string-length s2)) (n-give-up (+ 1 (- s2-len s1-len))))
  (let loop ((i 0))
   (if (< i n-give-up)
    (let loop2 ((j 0) (k i))
     (if (< j s1-len)
      (if (char=? (string-ref s1 j) (string-ref s2 k)) (loop2 (+ j 1) (+ k 1)) (loop (+ i 1))) i))
    false))))

(define (lassoc k al equ?)
 (let loop ((al al)) (if (null? al) false (let ((c (car al))) (if (equ? (car c) k) c (loop (cdr al)))))))

(define-syntax rassoc
 (lambda (%so)
  (datum->syntax %so
   (let ((%so-d (syntax->datum %so)))
    (apply (lambda (k al . z) `(scheme-rassoc ,k ,al ,(if (null? z) 'eqv? (cadr z)))) (cdr %so-d))))))

(define (scheme-rassoc k al equ)
 (let loop ((al al)) (if (null? al) false (let ((c (car al))) (if (equ (cdr c) k) c (loop (cdr al)))))))

(define (write-to-string n . z)
 (if (pair? z) (number->string (if (inexact? n) (inexact->exact n) n) 16) (number->string n)))

(define (number-to-roman n . upcase?) (set! upcase? (and (pair? upcase?) (car upcase?)))
 (unless (and (integer? n) (>= n 0)) (terror 'number-to-roman "Missing number"))
 (let
  ((roman-digits '((1000 #\m 100) (500 #\d 100) (100 #\c 10) (50 #\l 10) (10 #\x 1) (5 #\v 1) (1 #\i 0)))
   (approp-case (lambda (c) (if upcase? (char-upcase c) c))))
  (let loop ((n n) (dd roman-digits) (s '()))
   (if (null? dd) (if (null? s) "0" (list->string (reverse s)))
    (let* ((d (car dd)) (val (car d)) (char (approp-case (cadr d))) (nextval (caddr d)))
     (let loop2 ((q (quotient n val)) (r (remainder n val)) (s s))
      (if (= q 0)
       (if (>= r (- val nextval))
        (loop (remainder r nextval) (cdr dd) (cons char (cons (approp-case (cadr (assv nextval dd))) s)))
        (loop r (cdr dd) s))
       (loop2 (- q 1) r (cons char s)))))))))

(define (string-index s c)
 (let ((n (string-length s)))
  (let loop ((i 0)) (cond ((>= i n) false) ((char=? (string-ref s i) c) i) (else (loop (+ i 1)))))))

(define (string-reverse-index s c)
 (let loop ((i (- (string-length s) 1)))
  (cond ((< i 0) false) ((char=? (string-ref s i) c) i) (else (loop (- i 1))))))
...
(define (read-6hex i)
 (let*
  ((x (read i))
   (htmlcolor
    (string-upcase
     (cond ((symbol? x) (symbol->string x)) ((number? x) (number->string x))
      (else (terror 'atom-to-6hex "Misformed argument."))))))
  (string-append "#"
   (case (string-length htmlcolor) ((1) "00000") ((2) "0000") ((3) "000") ((4) "00") ((5) "0") (else ""))
   htmlcolor)))

;Translated from Common Lisp source tex2page.lisp by CLiiScm v. 20170126, clisp.


(define *tex2page-version* "20180629")

(define *tex2page-website* "http://ds26gte.github.io/tex2page/index.html")

(define *tex2page-copyright-notice*
 (let
  ((%type 'string) (%ee (list "Copyright (C) 1997-" (substring *tex2page-version* 0 4) " Dorai Sitaram")))
   ...
   (let*
    ...
    (let %loop ()
     (let
      ...
      (cond ((not i) (set! r (cons p r)) (return (reverse r))) (else false))
      (set! r (cons (substring p 0 i) r)) (set! p (substring p (add1 i))) p)
     (if %loop-returned %loop-result (%loop)))))))

(define *month-names*
 (vector "January" "February" "March" "April" "May" "June" "July" "August" "September" "October" "November"
  "December"))
...
(define *ghostscript-options* " -q -dBATCH -dNOPAUSE -dNO_PAUSE -sDEVICE=ppmraw")
...
(define *tex-logo*
 (let
  ((%type 'string)
   (%ee
    (list "T" "<span style=\"" "position: relative; " "top: 0.5ex; " "margin-left: -0.1667em; "
     "margin-right: -0.125em; " "text-transform: uppercase" "\">e</span>" "X")))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
    (if (null? %ee) %res
     (let ((%a (car %ee)))
...
(define *tex-files-to-ignore* '("btxmac" "eplain" "epsf" "lmfonts" "mfpic" "supp-pdf"))
...
(define *catcodes*
 (list (cons #\\ **escape**) (cons #\space **space**) (cons #\% **comment**)
  (cons (integer->char 0) **ignore**) (cons #\return 5) (cons #\newline 5) (cons #\{ **bgroup**)
  (cons #\} **egroup**) (cons #\$ **math**) (cons #\& **alignment**) (cons #\# **parameter**) (cons #\^ 7)
  (cons #\_ 8) (cons #\tab **space**) (cons #\~ **active**)))
...
(defstruct texframe* (definitions (make-table ':test equal?)) (cdefinitions (make-table))
 ...
 (tokses (make-table)) (boxes (make-table ':test equal?)) (istreams (make-table)) (ostreams (make-table))
 (postludes null) (uccodes (make-table)) (lccodes (make-table)) (catcodes null) (aftergroups null))

(defstruct tdef* (argpat null) (expansion "") (optarg false) (thunk false) (prim false) (defer false)
 (catcodes false))

(defstruct cdef* (argpat false) (expansion false) (optarg false) (active false) (catcodes false))
...
(define (gen-temp-string) (set! *temp-string-count* (+ *temp-string-count* 1))
 (let ((%type 'string) (%ee (list "Temp_" (write-to-string *temp-string-count*))))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
    (if (null? %ee) %res
     (let ((%a (car %ee)))
...
(define (file-stem-name f)
 (let
  ((slash
    (let ((%position-v #\/) (%position-s f) (%ee (list ':test char=? ':from-end true)))
     (cond ((string? %position-s) (string-reverse-index %position-s %position-v))
      (else (list-position %position-v %position-s))))))
  ...
  ((slash
   ...
   (dot
    (let ((%position-v #\.) (%position-s f) (%ee (list ':test char=? ':from-end true)))
     (cond ((string? %position-s) (string-reverse-index %position-s %position-v))
      (else (list-position %position-v %position-s))))))
  (if (and dot (not (= dot 0)) (or (not slash) (< (add1 slash) dot))) (substring f dot) false)))

(define (ensure-file-deleted f) (cond ((file-exists? f) (delete-file f)) (else false)))
 ...
 (cond
  ((not *aux-stream*)
   (let
    ...
    (set! *aux-stream*
     (let*
      ((%f f) (%ee (list ':direction ':output ':if-exists ':supersede)) (%direction (memv ':direction %ee))
       (%if-exists (memv ':if-exists %ee)) (%if-does-not-exist ':error)
    ...
    (set! *log-stream*
     (let*
      ((%f *log-file*) (%ee (list ':direction ':output ':if-exists ':supersede))
       (%direction (memv ':direction %ee)) (%if-exists (memv ':if-exists %ee)) (%if-does-not-exist ':error)
       (%if-does-not-exist-from-user (memv ':if-does-not-exist %ee)))
      (when %direction (set! %direction (cadr %direction)))
      (when %if-exists (set! %if-exists (cadr %if-exists)))
      (when %if-does-not-exist-from-user (set! %if-does-not-exist (cadr %if-does-not-exist-from-user)))
                ...
                (let
                 ((l
                   (let ((%read-line-res (read-line ip)))
                    (when (eof-object? %read-line-res) (set! %read-line-res false)) %read-line-res)))
                 (cond ((not l) (return ll)) ((< i n1) (set! i (+ i 1)) i)
                  ((<= i nf) (set! i (+ i 1)) (set! ll (cons (cons i l) ll)) ll) (else (return ll))))
                (if %loop-returned %loop-result (%loop)))))))
           (cond (ip ((if (input-port? ip) close-input-port close-output-port) ip)) (else false))
           %with-open-file-res))))
       (cond
        ((not (null? ll))
         (let ((border "__________________________..."))
          (let ((only-1-p (= (length ll) 1)))
           (let ((nf (caar ll)))
            (let ((ll (reverse ll)))
             (let ((n1 (caar ll))) (write-log "Likely error context: ") (write-log *current-source-file*)
              (write-log ", line") (cond ((not only-1-p) (write-log "s")) (else false)) (write-log " ")
              (write-log n1) (cond ((not only-1-p) (write-log "-") (write-log nf)) (else false))
              (write-log ":") (write-log #\newline) (write-log " /") (write-log border)
              (write-log #\newline)
              (for-each (lambda (l) (write-log " | ") (write-log (cdr l)) (write-log #\newline)) ll)
              (write-log " |") (write-log border) (write-log #\newline) (write-log "/")))))))
        (else false))))))
   (else false))))
...
(define (edit-offending-file)
 (let ((calling-from-text-editor-p (getenv "VIMRUNTIME")))
  (cond
   ((not calling-from-text-editor-p) (display "Type E to edit your file, X to quit.") (newline)
    (display "? ") (flush-output)
    (let
     ((c
       (let*
        ((%read-char-port false)
         (%read-char-res (if %read-char-port (read-char %read-char-port) (read-char))))
     ...
     (cond
      ((and c (char-ci=? c #\e))
       (let ((texedit-string (getenv "TEXEDIT")) (ill-formed-texedit-p false))
            ...
            (let ((i *it*))
             (set! texedit-string
              (let
               ((%type 'string)
                (%ee
                 (list (substring texedit-string 0 i) (write-to-string *input-line-no*)
               ...
               (let ((%res (if (eq? %type 'string) "" null)))
                ...
                %res)))
             texedit-string))
           (else (set! ill-formed-texedit-p true) (set! texedit-string false) texedit-string)))
         (else false))
        (cond
         (texedit-string
          (cond
            ...
            (let ((i *it*))
             (set! texedit-string
              (let
               ((%type 'string)
                (%ee
                 (list (substring texedit-string 0 i) *current-source-file*
                  (substring texedit-string (+ i 2)))))
               (let ((%res (if (eq? %type 'string) "" null)))
                (let %concatenate-loop ((%ee %ee))
                 (if (null? %ee) %res
        ...
        (cond
         ((not texedit-string)
          (cond (ill-formed-texedit-p (display "Ill-formed TEXEDIT; using EDITOR.") (newline))
           (else false))
          (cond
           ((begin (set! *it* (or (getenv "EDITOR") "vi")) *it*)
            (let ((e *it*))
             (set! texedit-string
              (let
               ((%type 'string)
                (%ee (list e " +" (write-to-string *input-line-no*) " " *current-source-file*)))
               (let ((%res (if (eq? %type 'string) "" null)))
                (let %concatenate-loop ((%ee %ee))
                 (if (null? %ee) %res
                  (let ((%a (car %ee)))
...
(define (trace-if write-p . args)
 (cond
  (write-p (write-log ':separation-newline)
   (cond ((> *input-line-no* 0) (write-log "l.") (write-log *input-line-no*) (write-log #\space))
    (else false))
   (for-each write-log args) (write-log ':separation-newline))
  (else false)))

(define (terror where . args) (write-log ':separation-newline) (write-log "! ") (for-each write-log args)
 (write-log ':separation-newline) (write-log *current-source-file*) (write-log #\:)
 (write-log *input-line-no*) (write-log ": error") (write-log ':separation-newline) (write-log "l.")
 (write-log *input-line-no*) (write-log #\space) (write-log where) (write-log " failed.")
 (write-log ':separation-newline) (display-error-context-lines) (close-all-open-streams) (output-stats)
...
(define (do-tracingall) (plain-count "\\tracingcommands" 1 false) (plain-count "\\tracingmacros" 1 false))
   ...
   ((%with-open-file-res
     (let
      ...
      (fluid-let
       ...
       (th)))))
   (cond (i ((if (input-port? i) close-input-port close-output-port) i)) (else false))
   %with-open-file-res)))
      ...
      ((c
        (let*
         ((%read-char-port p)
          (%read-char-res (if %read-char-port (read-char %read-char-port) (read-char))))
         (when (eof-object? %read-char-res) (set! %read-char-res false)) %read-char-res)))
      (cond ((not c) c) ((char=? c #\newline) (set! *input-line-no* (+ *input-line-no* 1)) c) (else c)))))
...
(define (snoop-actual-char)
 (let ((c (snoop-char)))
  (cond ((not c) c) ((= (catcode c) **ignore**) (get-char) (snoop-actual-char))
...
(define (get-actual-char)
 (let ((c (get-char)))
  (cond ((not c) c) ((= (catcode c) **ignore**) (get-actual-char))
   ((char=? c #\return)
    (let ((c (snoop-actual-char))) (if (and c (char=? c #\newline)) (get-actual-char) #\newline)))
   (else c))))
...
(define (get-line)
 (let ((r null))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop ()
...
(define (ignorespaces . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (newlines ':stop-before-par))
    ...
    (let ((newline-active-p (not (= (catcode #\newline) 5))) (num-newlines-read 0) (c false))
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (set! c (snoop-char))
       (cond ((eqv? c #\return) (set! c (snoop-actual-char)) c) (else false))
       (cond ((not c) (return))
        ((= (catcode c) **ignore**) (get-char) (cond (*reading-control-sequence-p* (return)) (else false)))
        ((char=? c #\newline)
         (if newline-active-p (return)
          (case newlines ((:stop-before-first-newline) (return))
           ((:stop-after-first-newline) (get-actual-char) (return))
           ((:stop-before-par)
            (case num-newlines-read
             ((0) (get-actual-char) (set! num-newlines-read (+ num-newlines-read 1)) num-newlines-read)
             (else (toss-back-char #\newline) (return))))
...
(define (munch-newlines)
 (let ((n 0))
  (let*
   ...
   (let %loop ()
    (let ((c (snoop-actual-char)))
     (cond ((not c) (return n)) ((char=? c #\newline) (get-actual-char) (set! n (+ n 1)) n)
      ((char-whitespace? c) (get-actual-char)) (else (return n))))
    (if %loop-returned %loop-result (%loop))))))
...
(define (munched-a-newline-p)
 (let*
  ...
  (let %loop ()
   (let ((c (snoop-actual-char)))
    (cond ((not c) (return false)) ((char=? c #\newline) (get-actual-char) (return true))
     ((char-whitespace? c) (get-actual-char)) (else (return false))))
   (if %loop-returned %loop-result (%loop)))))

(define (do-xspace)
 (let ((c (snoop-actual-char)))
  (cond ((not (member c '(#\space #\" #\. #\! #\, #\: #\; #\? #\/ #\' #\) #\-))) (emit-space #\space))
   (else false))))
...
(define (get-ctl-seq) (ignorespaces)
 (let ((bs (get-actual-char)))
  (cond ((not (= (catcode bs) **escape**)) (terror 'get-ctl-seq "Missing control sequence (" bs ")"))
   (else false))
  (let ((c (get-char)))
   (cond ((not c) "\\ ") ((= (catcode c) **ignore**) "\\ ")
    ((= (catcode c) **letter**)
     (let
      ((%type 'string)
       (%ee
         ...
         (reverse
          (let ((s (list c #\\)))
           (let*
            ...
            (let %loop ()
             (let ((c (snoop-char)))
              (cond ((not c) (return s)) ((= (catcode c) **ignore**) (return s))
               ((= (catcode c) **letter**) (get-char) (set! s (cons c s)) s)
               (else (ignorespaces ':stop-before-first-newline) (return s))))
             (if %loop-returned %loop-result (%loop)))))))))
      (let ((%res (if (eq? %type 'string) "" null)))
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
...
(define (if-aware-ctl-seq-p z)
 (or (member z *if-aware-ctl-seqs*)
  (and (>= (let ((%length-arg z)) ((if (string? %length-arg) string-length length) %length-arg)) 3)
   (char=? (string-ref z 1) #\i) (char=? (string-ref z 2) #\f))
  (let ((z-th (find-corresp-prim-thunk z)))
   (if (string? z-th) false
    (ormap (lambda (y) (eq? z-th (find-corresp-prim-thunk y))) *if-aware-ctl-seqs*)))))

(define (get-group-as-reversed-chars) (ignorespaces)
 (let ((c (get-actual-char))) (cond ((not c) (terror 'get-group "Runaway argument?")) (else false))
      ...
      (cond (escape-p (set! s (cons c s)) (set! escape-p false) escape-p)
       ((= (catcode c) **escape**)
        (if *expand-escape-p*
          ...
          ((s1
            (begin (toss-back-char c)
             (let ((x (get-ctl-seq)))
              (cond
               ((member x '("\\ " "\\{" "\\}"))
                (let ((%type 'string) (%ee (list (list (string-ref x 1)))))
                 (let ((%res (if (eq? %type 'string) "" null)))
                  (let %concatenate-loop ((%ee %ee))
                   (if (null? %ee) %res
                    (let ((%a (car %ee)))
         ...
         (begin (begin (set! s (cons c s)) s) (begin (set! escape-p true) escape-p))))
       ((char=? c #\{) (set! s (cons c s)) (set! nesting (+ nesting 1)) (set! escape-p false) escape-p)
       ((char=? c #\}) (set! s (cons c s))
        (if (= nesting 0) (return s)
         (begin (begin (set! nesting (- nesting 1)) nesting) (begin (set! escape-p false) escape-p))))
   ...
   (begin (get-actual-char)
    (let ((s null))
     (let*
      ...
      (let %loop ()
       (let ((c (snoop-actual-char)))
        (cond ((or (char-alphabetic? c) (char=? c #\*)) (get-actual-char) (set! s (cons c s)) s)
             ...
             (let %concatenate-loop ((%ee %ee))
              (if (null? %ee) %res
               (let ((%a (car %ee)))
                ...
                (%concatenate-loop (cdr %ee)))))
             %res))))
         (else (for-each toss-back-char s) (toss-back-char #\{) (return false))))
       (if %loop-returned %loop-result (%loop)))))))))
        ...
        (reverse
         (let ((s null) (nesting 0) (escape-p false))
          (let*
           ...
           (let %loop ()
            (let ((c (get-actual-char)))
             (cond ((not c) (terror 'get-bracketed-text-if-any "Runaway argument?")) (else false))
             (cond (escape-p (set! s (cons c s)) (set! escape-p false) escape-p)
              ((= (catcode c) **escape**) (set! s (cons c s)) (set! escape-p true) escape-p)
              ((char=? c #\{) (set! s (cons c s)) (set! nesting (+ nesting 1)) nesting)
              ((char=? c #\}) (set! s (cons c s)) (set! nesting (- nesting 1)) nesting)
              ((char=? c #\]) (if (= nesting 0) (return s) (begin (set! s (cons c s)) s)))
...
(define (ungroup s)
 (let ((n (let ((%length-arg s)) ((if (string? %length-arg) string-length length) %length-arg))))
  (let ((n-1 (sub1 n)))
   (if (and (>= n 2) (char=? (string-ref s 0) #\{) (char=? (string-ref s n-1) #\})) (substring s 1 n-1)
    s))))

(define (eat-alphanumeric-string) (ignorespaces)
 (let ((c (snoop-actual-char)))
  (if (char=? c #\") (begin (get-actual-char) (eat-till-char #\"))
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (set! c (snoop-actual-char))
     (cond ((not (or (char-alphabetic? c) (char-numeric? c) (member c '(#\:)))) (return)) (else false))
     (unless %loop-returned (get-actual-char)) (if %loop-returned %loop-result (%loop)))))))

(define (get-filename . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (bracedp false))
  (when (< 0 %lambda-rest-arg-len) (set! bracedp (list-ref %lambda-rest-arg 0))) (ignorespaces)
  (cond
   (bracedp
    (let ((c (snoop-actual-char)))
     (if (and (char? c) (char=? c #\{)) (get-actual-char) (begin (set! bracedp false) bracedp))))
   (else false))
  (let
   ((%type 'string)
    (%ee
        ...
        (let*
         ...
         (let %loop ()
          (let ((c (snoop-actual-char)))
           (cond ((not c) (return s))
            ((and (not bracedp)
              (or (char-whitespace? c) (= (catcode c) **comment**) (member c *filename-delims*)))
               ...
               (begin
                (set! s
                 (append
                  ...
                  s))
                s)
               (begin (toss-back-char *invisible-space*) (toss-back-string x) (return s)))))
...
(define (get-filename-possibly-braced) (ignorespaces)
 (let ((c (snoop-actual-char))) (get-filename (and (char? c) (char=? c #\{)))))
      ...
      (let ((s null))
       (let*
        ...
        (let %loop ()
         (let ((c (snoop-actual-char)))
          (cond ((not c) (return s))
           ((or (char-whitespace? c) (= (catcode c) **comment**) (= (catcode c)) **escape**) (return s))
           (else (get-actual-char) (set! s (cons c s)) s)))
         (if %loop-returned %loop-result (%loop)))))))))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
...
(define (get-integer base) (ignorespaces)
 (let ((s null) (c false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (set! c (snoop-actual-char)) (cond ((not c) (return)) (else false))
    (unless %loop-returned
     (cond ((not (or (char-numeric? c) (and (= base 16) (char-alphabetic? c)))) (ignorespaces) (return))
      (else false)))
    (unless %loop-returned (get-actual-char)) (unless %loop-returned (set! s (cons c s)) s)
    (if %loop-returned %loop-result (%loop))))
  (cond
   ((pair? s)
    (string->number
  ...
  (let ((s null))
   (let*
    ...
    (let %loop ()
     (let ((c (snoop-actual-char)))
      (cond ((not c) (return))
       ((or (char-numeric? c) (char=? c #\.)) (get-actual-char) (set! s (cons c s)) s)
...
(define (get-by) (ignorespaces)
 (cond
  ((char=? (snoop-actual-char) #\b) (get-actual-char)
   (if (char=? (snoop-actual-char) #\y) (get-actual-char) (toss-back-char #\b)))
  (else false)))

(define (get-to) (ignorespaces)
 (cond
  ((char=? (snoop-actual-char) #\t) (get-actual-char)
   (cond ((char=? (snoop-actual-char) #\o) (get-actual-char) (ignorespaces ':stop-before-first-newline))
    (else (toss-back-char #\t))))
  (else false)))

(define (get-number-corresp-to-ctl-seq x)
 (cond ((string=? x "\\the") (get-number-corresp-to-ctl-seq (get-ctl-seq))) ((string=? x "\\active") 13)
  ...
  ((string=? x "\\sectiondnumber") (table-get (string->number (ungroup (get-token))) *section-counters* 0))
...
(define (get-number-or-false) (ignorespaces)
 (let ((c (snoop-actual-char)))
  (cond ((= (catcode c) **escape**) (get-number-corresp-to-ctl-seq (get-ctl-seq)))
   ((char=? c #\') (get-actual-char) (get-integer 8)) ((char=? c #\") (get-actual-char) (get-integer 16))
   ((char=? c #\`) (get-actual-char) (ignorespaces)
    (char->integer
     (if (= (catcode (snoop-actual-char)) **escape**) (string-ref (get-ctl-seq) 1) (get-actual-char))))
   ((char=? c #\+) (get-actual-char) (get-number-or-false))
   ((char=? c #\-) (get-actual-char) (let ((n (get-number-or-false))) (and n (- n))))
   ((char-numeric? c) (get-integer 10)) (else false))))

(define (get-number) (or (get-number-or-false) (terror 'get-number "Missing number.")))

(define (get-tex-char-spec)
 (let ((n (get-number-or-false))) (if n (integer->char n) (terror 'get-tex-char-spec "not a char"))))
  ...
  (let ((c0 (string-ref x 0)))
   (cond
    ((and (= (catcode c0) **escape**)
      (> (let ((%length-arg x)) ((if (string? %length-arg) string-length length) %length-arg)) 1))
...
(define (get-url) (ignorespaces)
 (let ((c (get-actual-char)))
  (cond ((or (not c) (not (char=? c #\{))) (terror 'get-url "Missing {")) (else false))
  (string-trim
   (let
    ((%type 'string)
     (%ee
         ...
         (let*
          ((%loop-returned false) (%loop-result 0)
           (return
            (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
          (let %loop ()
           (let ((c (get-actual-char))) (cond ((not c) (terror 'get-url "Missing }")) (else false))
            (cond
             ((= (catcode c) **comment**)
              (let ((c1 (snoop-actual-char)))
               (if (and (char? c1) (char-whitespace? c1)) (ignorespaces) (begin (set! s (cons c s)) s))))
             ((char=? c #\{) (set! nesting (+ nesting 1)) (set! s (cons c s)) s)
             ((char=? c #\})
              (cond ((= nesting 0) (return s)) (else (set! nesting (- nesting 1)) (set! s (cons c s)) s)))
             (else (set! s (cons c s)) s)))
           (if %loop-returned %loop-result (%loop)))))))))
    (let ((%res (if (eq? %type 'string) "" null)))
     (let %concatenate-loop ((%ee %ee))
  ...
  ((rev-lbl
    (let ((s null))
     (let*
      ...
      (let %loop ()
       (let ((c (get-actual-char)))
        (cond ((not c) (terror 'get-csv "Runaway argument of \\cite, " "\\nocite, \\expandhtmlindex?"))
         (else false))
        (cond ((char=? c #\,) (return s)) ((char=? c closing-delim) (toss-back-char c) (return s))
         (else (set! s (cons c s)) s)))
       (if %loop-returned %loop-result (%loop)))))))
  (cond
   ((not (null? rev-lbl))
...
(define (get-raw-token)
 (let ((c (snoop-actual-char)))
  (cond ((not c) c) ((= (catcode c) **escape**) (get-ctl-seq))
   (else
...
(define (get-token) (ignorespaces)
 (let ((c (snoop-actual-char)))
  (cond ((not c) c) ((= (catcode c) **escape**) (get-ctl-seq)) ((char=? c #\{) (get-group))
...
(define (eat-word word) (ignorespaces)
 (let ((r null))
  (let
   ((%dotimes-n (let ((%length-arg word)) ((if (string? %length-arg) string-length length) %length-arg)))
    (i 0))
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (if (>= i %dotimes-n) (return true) false)
     (unless %loop-returned
      (let ((c (snoop-actual-char)))
       (cond ((char=? c (string-ref word i)) (get-actual-char) (set! r (cons c r)) r)
      ...
      (let ((firstp fullp))
       (let*
        ((%loop-returned false) (%loop-result 0)
         (return
          (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
        (let %loop () (ignorespaces)
         (let ((c (snoop-actual-char)))
          (cond ((not c) (return)) ((and (= (catcode c) **escape**) firstp) (get-ctl-seq) (return))
           ((or (char-numeric? c) (char=? c #\.)) (get-real))
           ((or (char=? c #\') (char=? c #\")) (get-number)) ((ormap eat-word '("+" "-")) true)
           ((ormap eat-word
             '("bp" "cc" "cm" "dd" "em" "ex" "filll" "fill" "fil" "in" "minus" "mm" "pc" "plus" "pt" "sp"
               "true"))
...
(define (eat-integer)
 (let ((%fluid-var-*not-processing-p* true))
  (fluid-let ((*not-processing-p* %fluid-var-*not-processing-p*)) (ignorespaces) (get-equal-sign)
   (get-number))))
      ...
      (let ((s null) (esc-p false) (c false))
       (let*
        ((%loop-returned false) (%loop-result 0)
         (return
          (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
        (let %loop () (set! c (snoop-actual-char)) (cond ((not c) (return s)) (else false))
         (unless %loop-returned
          (cond (esc-p (get-actual-char) (set! s (cons c s)) (set! esc-p false) esc-p)
           ((char=? c #\\) (get-actual-char) (set! s (cons c s)) (set! esc-p true) esc-p)
           ((or (char-whitespace? c) (member c *scm-token-delims*)) (return s))
           (else (get-actual-char) (set! s (cons c s)) s)))
         (if %loop-returned %loop-result (%loop)))))))))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
 ...
 (cond
  ((not (not c))
   (cond ((char=? c #\newline) (emit-newline)) (*outputting-to-non-html-p* (emit c))
    ...
    (else (emit (case c ((#\<) "&#x3c;") ((#\>) "&#x3e;") ((#\") "&#x22;") ((#\&) "&#x26;") (else c))))))
  (else false)))

(define (emit-html-string s)
 (let
  ((%dotimes-n (let ((%length-arg s)) ((if (string? %length-arg) string-length length) %length-arg)))
   (i 0))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (if (>= i %dotimes-n) (return) false)
    (unless %loop-returned (emit-html-char (string-ref s i))) (unless %loop-returned (set! i (+ i 1)))
    (if %loop-returned %loop-result (%loop))))))
...
(define (catcode c . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (n false) (globalp false))
  ...
  (cond
   ((not n)
    (cond ((begin (set! *it* (assoc c *catcodes*)) *it*) (cdr *it*)) ((char-alphabetic? c) 11) (else 12)))
   (else (set! *catcodes* (cons (cons c n) *catcodes*)) ((if (= n 13) activate-cdef deactivate-cdef) c)))))
 ...
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (raisep false))
  ...
  (let ((n (get-pixels)))
   (let ((box (read-box))) (emit "<span style=\"position: relative; top: ")
    ...
    (add-postlude-to-top-frame (lambda () (emit "</span>"))) (toss-back-char #\}) (toss-back-string box)
    false))))
...
(define (bgroup) (set! *tex-env* (cons (make-texframe* ':catcodes *catcodes*) *tex-env*))
 (cond ((and *in-display-math-p* (not *math-script-mode-p*)) (bgroup-math-hook)) (else false)))

(define (egroup) (cond ((null? *tex-env*) (terror 'egroup "Too many }'s")) (else false))
 (perform-postludes) (perform-aftergroups)
 (let* ((%pop-old-stack *tex-env*) (%pop-top-value (car %pop-old-stack)))
  (begin (set! *tex-env* (cdr %pop-old-stack)) *tex-env*) %pop-top-value)
 (cond ((begin (set! *it* (top-texframe)) *it*) (set! *catcodes* (texframe*-catcodes *it*)) *catcodes*)
  (else (terror 'egroup "This can't happen."))))

(define (bgroup-math-hook)
 (let
  ((old-html *html*) (old-math-delim-left *math-delim-left*) (old-math-delim-right *math-delim-right*)
   (old-math-height *math-height*))
  (set! *html* (make-html-output-stream)) (set! *math-delim-left* false) (set! *math-delim-right* false)
    ...
    (let ((res (html-output-stream-to-string *html*)))
     ...
     (cond
      ((or *math-delim-left* *math-delim-right*)
       (cond
        ((and
          (or (member *math-delim-left* '(:lbrace :rbrace)) (member *math-delim-right* '(:lbrace :rbrace)))
          (even? *math-height*))
       ...
       (set! res
        (let
         ((%type 'string)
          (%ee
           (list "<table><tr><td>" (tex-math-delim-string *math-delim-left*) "</td><td>" res "</td><td>"
            (tex-math-delim-string *math-delim-right*) "</td></tr></table>")))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
       ...
       res)
      (else false))
     (set! *html* old-html) (set! *math-delim-left* old-math-delim-left)
     (set! *math-delim-right* old-math-delim-right) (set! *math-height* (+ old-math-height *math-height*))
     (pop-tabular-stack ':mathbox) (emit "</td><td>") (emit res) (emit "</td><td>"))))))

(define (do-math-left) (ignorespaces)
 (cond
  ((and *in-display-math-p* (not *math-script-mode-p*))
   (let ((s (get-token))) (bgroup)
    (cond ((string=? s "(") (set! *math-delim-left* ':lparen) *math-delim-left*)
...
(define (do-math-right) (ignorespaces)
 (cond
  ((and *in-display-math-p* (not *math-script-mode-p*))
   (let ((s (get-token)))
    (cond ((string=? s ")") (set! *math-delim-right* ':rparen) *math-delim-right*)
     ((string=? s "]") (set! *math-delim-right* ':rbrack) *math-delim-right*)
     ((string=? s "\\}") (set! *math-delim-right* ':rbrace) *math-delim-right*)
     ((string=? s "|") (set! *math-delim-right* ':rvert) *math-delim-right*)
     ((string=? s ".") (set! *math-delim-right* ':nulldelim) *math-delim-right*)
     (else (terror 'do-math-right)))
    (egroup)))
  (else false)))

(define (perform-postludes) (for-each (lambda (p) (p)) (texframe*-postludes (top-texframe))))
...
(define (perform-afterassignment)
 (let ((z *afterassignment*)) (cond (z (set! *afterassignment* false) (do-tex-ctl-seq z)) (else false))))

(define (add-postlude-to-top-frame p)
 (let ((fr (if (null? *tex-env*) *global-texframe* (car *tex-env*))))
  (set!texframe*-postludes fr (cons p (texframe*-postludes fr))) (texframe*-postludes fr)))

(define (add-aftergroup-to-top-frame ag)
 (let ((fr (if (null? *tex-env*) *global-texframe* (car *tex-env*))))
  (set!texframe*-aftergroups fr (cons ag (texframe*-aftergroups fr))) (texframe*-aftergroups fr)))

(define (top-texframe) (if (null? *tex-env*) *global-texframe* (car *tex-env*)))

(define (kopy-tdef lft rt) (set!tdef*-argpat lft (tdef*-argpat rt)) (tdef*-argpat lft)
 (set!tdef*-expansion lft (tdef*-expansion rt)) (tdef*-expansion lft)
 ...
 (set!tdef*-defer lft (tdef*-defer rt)) (tdef*-defer lft) (set!tdef*-catcodes lft (tdef*-catcodes rt))
 (tdef*-catcodes lft) false)
...
(define (cleanse-tdef d) (set!tdef*-argpat d null) (tdef*-argpat d) (set!tdef*-expansion d "")
 (tdef*-expansion d) (set!tdef*-optarg d false) (tdef*-optarg d) (set!tdef*-thunk d false) (tdef*-thunk d)
 (set!tdef*-prim d false) (tdef*-prim d) (set!tdef*-defer d false) (tdef*-defer d)
 (set!tdef*-catcodes d false) (tdef*-catcodes d))

(define (ensure-def name frame)
 (let ((frame-defs (texframe*-definitions frame)))
  (or (table-get name frame-defs)
   (let ((d (make-tdef*))) (table-put! name frame-defs d) (table-get name frame-defs) d))))

(define (tex-def name argpat expansion optarg thunk prim defer frame)
 (cond ((not frame) (set! frame (top-texframe)) frame) (else false))
 (let ((d (ensure-def name frame))) (set!tdef*-argpat d argpat) (set!tdef*-expansion d expansion)
  (set!tdef*-optarg d optarg) (set!tdef*-thunk d thunk) (set!tdef*-prim d prim) (set!tdef*-defer d defer)
  (set!tdef*-catcodes d *catcodes*) (tdef*-catcodes d))
 (perform-afterassignment))

(define (tex-def-prim prim thunk) (tex-def prim null false false thunk prim false *primitive-texframe*))
...
(define (tex-def-0arg cs expn) (tex-def cs null expn false false false false false))

(define (ctl-seq-no-arg-expand-once cs) (let ((d (find-def cs))) (and d (tdef*-expansion d))))

(define (tex-gdef-0arg cs expn) (tex-def cs null expn false false cs false *global-texframe*))

(define (tex-def-prim-0arg cs expn) (tex-def cs null expn false false cs false *primitive-texframe*))

(define (get-0arg-expn cs) (let ((d (find-def cs))) (if d (tdef*-expansion d) "0")))

(define (tex2page-flag-value cs) (string-ref (get-0arg-expn cs) 0))

(define (tex2page-flag-boolean cs) (not (member (string-ref (get-0arg-expn cs) 0) '(#\0 #\f #\F #\n #\N))))

(define (tex-let lft rt frame) (cond ((not frame) (set! frame (top-texframe)) frame) (else false))
 (let ((frame-defs (texframe*-definitions frame)))
  (let
   ((lft-def
     (or (table-get lft frame-defs)
      (let ((lft-def (make-tdef*))) (table-put! lft frame-defs lft-def) (table-get lft frame-defs)
       lft-def))))
   (cond
    ((begin (set! *it* (or (find-def rt) (find-math-def rt))) *it*)
     (let ((rt-def *it*)) (kopy-tdef lft-def rt-def)))
    (else (cleanse-tdef lft-def))))))

(define (tex-let-general lhs rhs frame)
 (if (ctl-seq-p rhs) (tex-let lhs rhs frame) (tex-def lhs null rhs false false false false frame)))

(define (tex-let-prim lft rt) (tex-let lft rt *primitive-texframe*))

(define (tex-def-thunk name thunk frame)
 (cond ((not (inside-false-world-p)) (tex-def name null false false thunk name false frame)) (else false)))
...
(define (gen-regno c . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (avoid null))
  ...
  (let ((n (table-get c (texframe*-counts *global-texframe*)))) (set! n (+ n 1))
   ...
   (table-put! c (texframe*-counts *global-texframe*) n) (table-get c (texframe*-counts *global-texframe*))
   n)))
...
(define (do-count= regno globalp) (cond ((not globalp) (set! globalp (globally-p)) globalp) (else false))
 (get-equal-sign) (tex-def-count regno (get-number) globalp) (perform-afterassignment))
...
(define (do-countdef)
 (let ((cltseq (get-raw-token/is))) (get-equal-sign)
  (let ((n (get-number-or-false))) (cond ((not n) (terror 'do-countdef "Missing number.")) (else false))
...
(define (get-gcount ctlseq) (table-get (find-countdef ctlseq) (texframe*-counts *global-texframe*) 0))
...
(define (do-dimen= regno globalp) (cond ((not globalp) (set! globalp (globally-p)) globalp) (else false))
 (get-equal-sign) (tex-def-dimen regno (get-scaled-points) globalp) (perform-afterassignment))
...
(define (do-dimendef)
 (let ((cltseq (get-raw-token/is))) (get-equal-sign)
  (let ((n (get-scaled-points))) (cond ((not n) (terror 'do-dimendef "Missing number.")) (else false))
...
(define (tex-def-box regno bcontents . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (globalp false))
  ...
  (let ((frame (top-texframe))) (table-put! regno (texframe*-boxes frame) bcontents)
   ...
   (cond
    ((and globalp (pair? *tex-env*)) (table-put! regno (texframe*-ostreams *global-texframe*) bcontents)
     (table-get regno (texframe*-ostreams *global-texframe*)))
    (else false)))))
...
(define (do-toks= regno globalp) (cond ((not globalp) (set! globalp (globally-p)) globalp) (else false))
 (get-equal-sign) (tex-def-toks regno (get-group) globalp) (perform-afterassignment))

(define (do-toksdef)
 (let ((ctlseq (get-ctl-seq))) (get-equal-sign) (tex-def-toksdef ctlseq false (get-group) (globally-p))))
...
(define (ensure-cdef c f)
 (let ((f-cdefs (texframe*-cdefinitions f)))
  (or (table-get c f-cdefs) (let ((d (make-cdef*))) (table-put! c f-cdefs d) (table-get c f-cdefs) d))))

(define (tex-def-char c argpat expansion frame)
 (cond ((not frame) (set! frame (top-texframe)) frame) (else false))
 (let ((d (ensure-cdef c frame))) (set!cdef*-argpat d argpat) (set!cdef*-expansion d expansion)
...
(define (do-undefcsactive) (ignorespaces) (deactivate-cdef (string-ref (get-ctl-seq) 1)))

(define (do-catcode)
 (let ((c (get-tex-char-spec))) (let ((val (begin (get-equal-sign) (get-number)))) (catcode c val))))

(define (do-opmac-activettchar) (ignorespaces)
 (let ((c (get-token-as-tex-char-spec))) (set! *opmac-active-tt-char* c) (activate-cdef c)
...
(define (do-opmac-intext-verb) (bgroup)
 (let ((%fluid-var-*ligatures-p* false))
  (fluid-let ((*ligatures-p* %fluid-var-*ligatures-p*))
   (cond (*outputting-external-title-p* false) (else (emit "<code class=verbatim>")))
...
(define (make-external-title title)
 (let ((%fluid-var-*outputting-external-title-p* true))
  (fluid-let ((*outputting-external-title-p* %fluid-var-*outputting-external-title-p*)) (bgroup)
   (let
    ((s
      (tex-string-to-html-string
       (let
        ((%type 'string)
         (%ee
          (list "\\let\\\\\\ignorespaces" "\\def\\resizebox#1#2#3{}" "\\let\\thanks\\TIIPgobblegroup"
...
(define (output-external-title)
 (let ((%fluid-var-*outputting-external-title-p* true))
  (fluid-let ((*outputting-external-title-p* %fluid-var-*outputting-external-title-p*)) (emit "<title>")
   (emit-newline) (emit (or *title* *jobname*)) (emit-newline) (emit "</title>") (emit-newline))))
...
(define (do-subject) (tex-gdef-0arg "\\TIIPtitleused" "1") (do-end-para) (ignorespaces)
 (let
  ((title
    (let ((c (snoop-actual-char))) (if (or (not c) (not (char=? c #\{))) (get-till-par) (get-group)))))
  (cond ((not *title*) (flag-missing-piece ':document-title)) (else false))
  (write-aux `(!default-title ,(make-external-title title))) (output-title title)))

(define (do-latex-title)
 (let ((title (get-group))) (cond ((not *title*) (flag-missing-piece ':document-title)) (else false))
  (write-aux `(!default-title ,(make-external-title title))) (toss-back-string title)
  (toss-back-string "\\def\\TIIPtitle")))

(define (do-title) (if (eq? *tex-format* ':latex) (do-latex-title) (do-subject)))
...
(define (do-today)
 (let ((m (get-gcount "\\month")))
  (if (= m 0) (emit "[today]")
   (begin (emit (vector-ref *month-names* (sub1 m))) (emit " ") (emit (get-gcount "\\day")) (emit ", ")
...
(define (do-end-para)
 (cond
  (*in-para-p* (cond (*use-closing-p-tag-p* (emit "</p>")) (else false))
   (cond
    ((not (null? *afterpar*)) (for-each (lambda (ap) (ap)) (reverse *afterpar*)) (set! *afterpar* null)
     *afterpar*)
    (else false))
 ...
 (cond
  ...
  (else (do-end-para)
   (let ((in-table-p (and (pair? *tabular-stack*) (eq? (car *tabular-stack*) ':block))))
    (cond (in-table-p (emit "</td></tr><tr><td>") (emit-newline)) (else false)) (emit "<p>") (emit-newline)
    (set! *in-para-p* true) *in-para-p*))))

(define (do-noindent) (do-end-para) (emit-newline) (emit "<p class=noindent>") (set! *in-para-p* true)
 *in-para-p*)

(define (do-indent)
 (let ((parindent (sp-to-pixels (the-dimen "\\parindent")))) (emit "<span style=\"margin-left: ")
  (emit parindent) (emit "pt\"></span>")))

(define (do-para-nopadding) (do-end-para) (emit-newline) (emit "<p class=nopadding>") (emit-newline)
 (set! *in-para-p* true) *in-para-p*)

(define (do-maketitle) (do-end-para) (bgroup)
 (tex2page-string
  (let ((%type 'string) (%ee (list "\\let\\\\\\break" "\\let\\and\\break" "\\let\\thanks\\symfootnote")))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
       ...
       (%concatenate-loop (cdr %ee)))))
    %res)))
 (output-title "\\TIIPtitle") (do-para) (do-end-para) (emit "<div class=centerline>") (emit-newline)
 (tex2page-string "\\TIIPauthor") (do-para) (tex2page-string "\\TIIPdate") (do-end-para) (emit "</div>")
 (emit-newline) (egroup) (do-para))

(define (do-inputcss) (ignorespaces)
 (let ((f (get-filename-possibly-braced)))
  (cond ((null? *stylesheets*) (flag-missing-piece ':stylesheets)) (else false))
    ...
    (let ((c (snoop-actual-char)))
     (cond
      ((= (catcode c) **escape**)
       (let ((x (get-ctl-seq)))
        (cond
         ((string=? x "\\endcsname") (toss-back-char #\}) (for-each toss-back-string r) (toss-back-char c)
          (toss-back-char #\{) (toss-back-string "TIIPcsname") (toss-back-char c) (return))
...
(define (do-saved-csname) (let ((x (get-peeled-group))) (do-tex-ctl-seq x)))
...
(define (link-stylesheets)
 (let
  ((link-it
    (lambda (css) (emit "<link rel=\"stylesheet\" href=\"") (emit css) (emit "\" />") (emit-newline))))
  (for-each link-it *stylesheets*)
  (cond
   ((or (tex2page-flag-boolean "\\TIIPsinglepage") (tex2page-flag-boolean "\\TZPsinglepage"))
    (emit "<style>") (emit-newline) (emit *basic-style*) (emit "</style>") (emit-newline))
   (else false))
  (link-it
...
(define (link-scripts)
 (let ((link-it (lambda (jsf) (emit "<script src=\"") (emit jsf) (emit "\"></script>") (emit-newline))))
...
(define (increment-section-counter seclvl nonum-p)
 (cond
  ((not nonum-p)
   (cond
    ((not (table-get seclvl *section-counters*)) (table-put! seclvl *section-counters* 0)
     (table-get seclvl *section-counters*))
    (else false))
   (table-put! seclvl *section-counters* (+ (table-get seclvl *section-counters*) 1))
   (table-get seclvl *section-counters*))
  (else false))
 (table-for-each
  (lambda (k v)
   (cond ((and (> k seclvl) (> k 0)) (table-put! k *section-counters* 0) (table-get k *section-counters*))
    (else false)))
  *section-counters*)
 (cond
  ((and (not (or (tex2page-flag-boolean "\\TIIPsinglepage") (tex2page-flag-boolean "\\TZPsinglepage")))
    (= seclvl 0))
   (tex-gdef-count "\\footnotenumber" 0))
  (else false))
 (for-each
  (lambda (counter-name) (set!counter*-value (table-get counter-name *dotted-counters*) 0)
...
(define (section-counter-value seclvl)
 (if (= seclvl -1) (number-to-roman (table-get -1 *section-counters*) true)
  (let ((i (if *using-chapters-p* 0 1)))
   (let
    ((outermost-secnum
      (let ((n (table-get i *section-counters* 0)))
       (if *inside-appendix-p*
        (let ((%type 'string) (%ee (list (list (integer->char (+ (char->integer #\A) -1 n))))))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
    ...
    (let ((i (add1 i)) (r outermost-secnum))
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (cond ((> i seclvl) (return r)) (else false))
       (unless %loop-returned
        (set! r
         (let ((%type 'string) (%ee (list r "." (write-to-string (table-get i *section-counters* 0)))))
          (let ((%res (if (eq? %type 'string) "" null)))
           (let %concatenate-loop ((%ee %ee))
            (if (null? %ee) %res
             (let ((%a (car %ee)))
...
(define (section-ctl-seq-p s)
 (cond ((string=? s "\\sectiond") (string->number (ungroup (get-token)))) ((string=? s "\\part") -1)
  ((string=? s "\\chapter") 0)
  (else
   (let ((n (let ((%length-arg s)) ((if (string? %length-arg) string-length length) %length-arg))))
    (cond ((< n 8) false)
       ...
       (let*
        ...
        (let %loop ()
         (cond ((> i+3 n-9) (return k))
          ((string=? (substring s i i+3) "sub") (set! i i+3) (set! i+3 (+ i+3 3)) (set! k (add1 k)) k)
          (else (return false)))
         (if %loop-returned %loop-result (%loop))))))
...
(define (do-heading seclvl)
 (let ((starred-p (cond ((char=? (snoop-actual-char) #\*) (get-actual-char) true) (else false))))
  (let
   ((too-deep-p
     (let ((secnumdepth (get-gcount "\\secnumdepth")))
      (cond ((< secnumdepth -1) false) ((> seclvl secnumdepth) true) (else false)))))
   (let ((nonum-p (or starred-p too-deep-p)))
    (let
     ((header
       (let ((%fluid-var-*tabular-stack* (list ':header)))
        (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*)) (get-bracketed-text-if-any)
 ...
 (cond
  ((= seclvl 0) (!using-chapters)
   (cond
    ...
    (else false))
   (cond ((or (> *html-page-count* 0) (tex2page-flag-boolean "\\TIIPtitleused")) (do-eject))
    (else (tex-gdef-0arg "\\TIIPtitleused" "1") (do-para))))
  (else false))
 (cond
  ((and (= seclvl 1) (or (tex2page-flag-boolean "\\TIIPslides") (tex2page-flag-boolean "\\TZPslides")))
   (do-eject))
  (else false))
 (increment-section-counter seclvl nonum-p) (cond (lbl-val (set! nonum-p false) nonum-p) (else false))
 (cond ((not lbl-val) (set! lbl-val (if nonum-p "IGNORE" (section-counter-value seclvl))) lbl-val)
  (else false))
  ...
  (let
   ((lbl
     (let
      ((%type 'string)
       (%ee
        (list *html-node-prefix* (case seclvl ((-1) "part") ((0) "chap") (else "sec")) "_"
       ...
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
         (let ((%a (car %ee)))
          ...
          (%concatenate-loop (cdr %ee)))))
       %res))))
   (cond ((not false) (tex-def-0arg "\\TIIPcurrentnodename" lbl) (tex-def-0arg "\\@currentlabel" lbl-val))
    (else false))
   (do-end-para) (emit-anchor lbl) (emit-newline) (ignorespaces ':all) (emit "<h") (emit htmlnum)
   (case seclvl ((-1) (emit " class=part class=centerline")) ((0) (emit " class=chapter"))
    (else (emit " class=section")))
   (emit ">")
   (let
    ((write-to-toc-p
      (and (not notoc-p) *toc-page* (not (and (eqv? *tex-format* ':latex) (string=? header "Contents"))))))
    (cond
     ((eqv? *tex-format* ':latex)
      (case seclvl
       ((-1) (emit "<div class=partheading>")
        ...
        (emit "</div><br>") (emit-newline))
       ((0) (emit-newline) (emit "<div class=chapterheading>")
        (if nonum-p (emit-nbsp 1)
         (begin
          (cond
           (write-to-toc-p
            (emit-page-node-link-start *toc-page*
             (let ((%type 'string) (%ee (list *html-node-prefix* "toc_" lbl)))
              (let ((%res (if (eq? %type 'string) "" null)))
               ...
               %res))))
           (else false))
          (tex2page-string (if *inside-appendix-p* "\\appendixname" "\\chaptername")) (emit " ")
          (emit lbl-val) (cond (write-to-toc-p (emit-link-stop)) (else false))))
        (emit "</div><br>") (emit-newline))))
     (else false))
    (cond
     (write-to-toc-p
      (emit-page-node-link-start *toc-page*
       (let ((%type 'string) (%ee (list *html-node-prefix* "toc_" lbl)))
        (let ((%res (if (eq? %type 'string) "" null)))
         ...
         %res))))
     (else false))
    (cond ((not (or (and (eqv? *tex-format* ':latex) (<= seclvl 0)) nonum-p)) (emit lbl-val) (emit-nbsp 2))
     (else false))
    (emit header) (cond (write-to-toc-p (emit-link-stop)) (else false)) (emit "</h") (emit htmlnum)
    (emit ">") (do-noindent)
    (let ((tocdepth (get-gcount "\\tocdepth")))
     (cond
      ((and write-to-toc-p (not (and (eq? *tex-format* ':latex) starred-p))
       ...
       (write-aux
        `(!toc-entry ,(if (= seclvl -1) -1 (if *using-chapters-p* seclvl (sub1 seclvl))) ,lbl-val
          ,*html-page-count* ,lbl ,header)))
      (else false))))
   (cond
    (*recent-node-name* (do-label-aux *recent-node-name*) (set! *recent-node-name* false)
     *recent-node-name*)
    (else false)))))

(define (section-type-to-depth sectype)
 (cond ((string->number sectype)) ((string=? sectype "chapter") 0) ((string=? sectype "section") 1)
  ((string=? sectype "subsection") 2) ((string=? sectype "subsubsection") 3)
  ((string=? sectype "paragraph") 4) ((string=? sectype "subparagraph") 5) (else 3)))
...
(define (do-write-to-toc-aux seclvl secnum sectitle)
 (let
  ((node-name
    (let
     ((%type 'string)
      (%ee (list *html-node-prefix* "sec_" (if (string=? secnum "") (gen-temp-string) secnum))))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (tex-def-0arg "\\TIIPcurrentnodename" node-name) (tex-def-0arg "\\@currentlabel" secnum)
  (emit-anchor node-name) (emit-newline)
  (write-aux `(!toc-entry ,seclvl ,secnum ,*html-page-count* ,node-name ,sectitle))))

(define (do-addcontentsline)
 (let ((toc (get-peeled-group)))
  (cond ((not (string=? toc "toc")) (terror 'do-addcontentsline "only #1=toc supported")) (else false))
  (let ((seclvl (section-type-to-depth (get-peeled-group))))
   (let ((sectitle (tex-string-to-html-string (get-group))))
    (write-aux
     `(!toc-entry ,(if (= seclvl -1) -1 (if *using-chapters-p* seclvl (sub1 seclvl)))
...
(define (do-documentclass) (probably-latex) (get-bracketed-text-if-any)
 (let ((x (get-peeled-group))) (cond ((member x '("report" "book")) (!using-chapters)) (else false))))
   ...
   (let %loop ()
    (let ((c (get-actual-char)))
     (cond
      ...
      (newline-p
       (cond
        ((not (char-whitespace? c)) (set! r (cons #\space r)) (set! r (cons c r)) (set! newline-p false)
         newline-p)
        (else false)))
...
(define (do-beginchapter) (ignorespaces)
 (let ((chapno (tex-string-to-html-string (get-till-char #\space))))
  (let
   ((header
     (begin (ignorespaces)
      (let ((%fluid-var-*tabular-stack* (list ':header)))
       (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*))
        (tex-string-to-html-string (get-till-par)))))))
   (tex-gdef-0arg "\\chapno" chapno) (tex-gdef-count "\\subsecno" 0) (tex-gdef-count "\\footnotenumber" 0)
...
(define (pop-tabular-stack type)
 (let
  ((type-in-stack
    (let* ((%pop-old-stack *tabular-stack*) (%pop-top-value (car %pop-old-stack)))
     (begin (set! *tabular-stack* (cdr %pop-old-stack)) *tabular-stack*) %pop-top-value)))
  (cond
...
(define (do-end-table/figure type)
 (cond ((and (eqv? type ':figure) (char=? (snoop-actual-char) #\*)) (get-actual-char)) (else false))
 (do-end-para) (emit "</td></tr>") (emit "</table>") (emit "</div>") (pop-tabular-stack type) (egroup)
 (do-para))

(define (bump-dotted-counter name)
 (let ((counter (table-get name *dotted-counters*)))
  (let ((new-value (add1 (counter*-value counter)))) (set!counter*-value counter new-value)
        ...
        (%ee
         (list
          (let ((sec-num (counter*-within counter)))
           (if sec-num
            (let ((%type 'string) (%ee (list (section-counter-value sec-num) ".")))
             (let ((%res (if (eq? %type 'string) "" null)))
              (let %concatenate-loop ((%ee %ee))
               (if (null? %ee) %res
                (let ((%a (car %ee)))
        ...
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
          (let ((%a (car %ee)))
           ...
           (%concatenate-loop (cdr %ee)))))
        %res))))
    (tex-def-0arg "\\@currentlabel" num) num))))
...
(define (do-caption) (do-end-para)
 (let
  ...
  (let
   ...
   (let
    ((type
      (cond ((and (not i-fig) (not i-tbl)) (terror 'do-caption "Mislaid \\caption")) ((not i-fig) ':table)
       ((not i-tbl) ':figure) ((< i-fig i-tbl) ':figure) ((< i-tbl i-fig) ':table)
       (else (terror 'do-caption "cant happen")))))
    (let ((counter-name (if (eq? type ':table) "table" "figure")))
     (let ((caption-title (if (eq? type ':table) "\\tablename" "\\figurename")))
      (let ((num (bump-dotted-counter counter-name))) (get-bracketed-text-if-any) (emit "</td></tr>")
       (emit-newline) (emit "<tr><td class=") (emit *display-justification*) (emit "><b>")
       (tex2page-string caption-title) (emit " ") (emit num) (emit ":</b>") (emit-nbsp 2)
       (tex2page-string (get-group)) (emit "</td></tr>") (emit-newline) (emit "<tr><td>"))))))))

(define (do-marginnote) (emit "<span class=marginnote>") (tex2page-string (get-group)) (emit "</span>"))

(define (do-marginpar) (get-bracketed-text-if-any) (emit "<table class=leftline border=2><tr><td>")
 (tex2page-string (get-group)) (emit "</td></tr></table>"))

(define (do-minipage) (get-bracketed-text-if-any) (get-group)
 (let
  ((in-table-p
    (and (not (null? *tabular-stack*)) (member (car *tabular-stack*) '(:block :figure :table)))))
  (if in-table-p (emit "</td><td>") (begin (do-para) (do-end-para))) (emit "<div class=leftline>")
  (set! *tabular-stack* (cons ':minipage *tabular-stack*)) *tabular-stack*))

(define (do-endminipage) (pop-tabular-stack ':minipage)
 (let ((in-table-p (member (car *tabular-stack*) '(:block :figure :table)))) (emit "</div>")
  (if in-table-p (emit "</td><td>") (do-para))))

(define (do-tabbing) (set! *tabular-stack* (cons ':tabbing *tabular-stack*)) (do-para))
...
(define (do-equation type)
 (cond
  ((and (or (not (tex2page-flag-boolean "\\TZPmathtext")) (tex2page-flag-boolean "\\TZPmathimage"))
   ...
   (do-latex-env-as-image (case type ((:equation) "equation") ((:align) "align") (else "eqnarray"))
    ':display))
  (else (do-end-para) (bgroup) (cond ((eqv? type ':align) (set! type ':eqnarray) type) (else false))
   (cond ((and (eqv? type ':eqnarray) (eat-star)) (set! type ':eqnarray*) type) (else false))
   (set! *tabular-stack* (cons type *tabular-stack*)) (set! *math-mode-p* true)
   (set! *in-display-math-p* true)
   (let
    ((eqn-tag
      (let ((%type 'string) (%ee (list *html-node-prefix* "eqn_" (gen-temp-string))))
       (let ((%res (if (eq? %type 'string) "" null)))
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
          (let ((%a (car %ee)))
           ...
           (%concatenate-loop (cdr %ee)))))
        %res))))
    (tex-def-0arg "\\TIIPcurrentnodename" eqn-tag) (emit-anchor eqn-tag) (emit-newline)
    (cond
     ((not (eqv? type ':eqnarray*)) (set! *equation-number* (bump-dotted-counter "equation"))
      *equation-number*)
     (else false))
    (emit "<div class=") (emit *display-justification*) (emit "><table width=100%>") (emit-newline)
    (emit "<tr><td align=") (emit (if (eqv? type ':equation) "center" "right")) (emit ">")))))

(define (do-end-equation type)
 (cond ((and (eqv? type ':eqnarray) (eat-star)) (set! type ':eqnarray*) type) (else false)) (do-end-para)
 (emit "</td>")
 (cond
  ((not (or (eq? (car *tabular-stack*) ':eqnarray*) (not *equation-numbered-p*))) (emit "<td>(")
   (emit *equation-number*) (emit ")</td>"))
  (else false))
 (emit "</tr>") (emit-newline) (emit "</table></div>") (pop-tabular-stack type) (set! *math-mode-p* false)
 (set! *in-display-math-p* false) (egroup) (set! *equation-numbered-p* true) (set! *equation-position* 0)
 (do-para))
   ...
   (let ((%dotimes-n 2) (i 0))
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop () (if (>= i %dotimes-n) (return) false) (unless %loop-returned (ignorespaces))
      (unless %loop-returned
       (let ((c (snoop-actual-char)))
        (cond
          ...
          (let ((%fluid-var-*math-script-mode-p* true))
           (fluid-let ((*math-script-mode-p* %fluid-var-*math-script-mode-p*))
            (let ((s (get-token))) (emit "<span style=\"font-size: 85%; position: relative; ")
             (emit
              (case c ((#\_) "top: 2.5ex; ") ((#\^) "bottom: 3ex; ") (else (error 'ecase "0xdeadc0de"))))
             (emit "\">") (tex2page-string s) (emit "</span>")))))
         (else false))))
      (unless %loop-returned (set! i (+ i 1))) (if %loop-returned %loop-result (%loop))))))))
...
(define (indent-n-levels n)
 (let ((%dotimes-n (add1 n)) (i 0))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (if (>= i %dotimes-n) (return) false) (unless %loop-returned (emit-nbsp 1))
    (unless %loop-returned (emit " ")) (unless %loop-returned (emit-nbsp 1))
    (unless %loop-returned (emit " ")) (unless %loop-returned (set! i (+ i 1)))
    (if %loop-returned %loop-result (%loop))))))

(define (do-toc)
 (let
  ((%fluid-var-*subjobname*
      ...
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res)))
   (%fluid-var-*img-file-count* 0) (%fluid-var-*imgdef-file-count* 0))
  (fluid-let
   ((*imgdef-file-count* %fluid-var-*imgdef-file-count*) (*img-file-count* %fluid-var-*img-file-count*)
    (*subjobname* %fluid-var-*subjobname*))
   (cond
    ((eqv? *tex-format* ':latex)
     (tex2page-string (if *using-chapters-p* "\\chapter*{\\contentsname}" "\\section*{\\contentsname}")))
    (else false))
   (emit-anchor
          ...
          (let ((seclabel (tocentry*-label x)))
           (let
            ((subentries-p
              (or (= lvl -1)
               (and (= lvl 0)
                (or (< tocdepth -1) (and *using-chapters-p* (> tocdepth 0))
                 (and (not *using-chapters-p*) (> tocdepth 1)))))))
            (cond
             (subentries-p
              (if (or (tex2page-flag-boolean "\\TIIPtexlayout") (tex2page-flag-boolean "\\TZPtexlayout"))
               (do-bigskip ':medskip) (do-para))
              (do-noindent) (emit "<b>") (emit-newline))
             (else false))
            (indent-n-levels lvl)
            (emit-anchor
             (let ((%type 'string) (%ee (list *html-node-prefix* "toc_" seclabel)))
              (let ((%res (if (eq? %type 'string) "" null)))
               (let %concatenate-loop ((%ee %ee))
                (if (null? %ee) %res
                 (let ((%a (car %ee)))
            ...
            (cond ((not (or (string=? secnum "") (string=? secnum "IGNORE"))) (emit secnum) (emit-nbsp 2))
             (else false))
            (let ((%fluid-var-*tabular-stack* (list 'header)))
             (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*)) (emit (tocentry*-header x))))
            (emit-link-stop) (cond (subentries-p (emit "</b>")) (else false)) (emit "<br>")
            (emit-newline))))))
       *toc-list*))))
   (emit-anchor
...
(define (tex-string-to-html-string s)
 (let ((%fluid-var-*html* (make-html-output-stream)))
  (fluid-let ((*html* %fluid-var-*html*)) (tex2page-string s) (html-output-stream-to-string *html*))))
...
(define (do-plain-footnote)
 (do-footnote-aux
  (let ((%fluid-var-*temporarily-use-utf8-for-math-p* true))
   (fluid-let ((*temporarily-use-utf8-for-math-p* %fluid-var-*temporarily-use-utf8-for-math-p*))
    (tex-string-to-html-string (get-token))))))

(define (do-footnote) (if (eq? *tex-format* ':latex) (do-numbered-footnote) (do-plain-footnote)))
...
(define (do-footnote-aux fnmark)
 (let ((fnno false))
  (let ((fnlabel (gen-temp-string)))
   (let
    ((fntag
      (let ((%type 'string) (%ee (list *html-node-prefix* "footnote_" fnlabel)))
       (let ((%res (if (eq? %type 'string) "" null)))
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
          (let ((%a (car %ee)))
    ...
    (let
     ((fncalltag
       (let ((%type 'string) (%ee (list *html-node-prefix* "call_footnote_" fnlabel)))
        (let ((%res (if (eq? %type 'string) "" null)))
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
           (let ((%a (car %ee)))
     ...
     (cond
      ...
      (else false))
     (emit-anchor fncalltag) (emit "<span class=footnotemark>") (cond (fnno (emit "<sup>")) (else false))
     (emit-page-node-link-start false fntag) (emit fnmark) (emit-link-stop)
     (cond (fnno (emit "</sup>")) (else false)) (emit "</span>")
...
(define (do-vfootnote-aux fnmark fncalltag fntag) (ignorespaces)
 (cond ((not (char=? (get-actual-char) #\{)) (terror 'do-vfootnote-aux "Missing {")) (else false)) (bgroup)
 (let ((old-html *html*)) (set! *html* (make-html-output-stream))
  (cond (fncalltag (tex-def-0arg "\\TIIPcurrentnodename" fntag) (tex-def-0arg "\\@currentlabel" fnmark))
   (else false))
  (add-aftergroup-to-top-frame
   (lambda ()
    (set! *footnote-list*
     (cons
      (make-footnotev* ':mark fnmark ':text (html-output-stream-to-string *html*) ':tag fntag ':caller
       fncalltag)
      *footnote-list*))
...
(define (output-footnotes)
 (let ((n (length *footnote-list*)))
  (cond
   ((not (= n 0)) (emit "<div class=footnoterule><hr></div>") (do-para) (do-end-para)
    ...
    (let ((i (sub1 n)))
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (cond ((< i 0) (return)) (else false))
       (unless %loop-returned
          ...
          (let ((fnno (string->number fnmark)))
           (let ((fncalltag (footnotev*-caller fv))) (do-para) (emit "<span class=footnotemark>")
            (cond
             ...
             (else false))
            (emit fnmark)
            (cond (fncalltag (emit-link-stop) (cond (fnno (emit "</sup>")) (else false))) (else false))
            (emit "</span>") (emit " ") (emit (footnotev*-text fv)) (do-end-para) (set! i (- i 1)) i)))))
       (if %loop-returned %loop-result (%loop)))))
...
(define (rgb-frac-to-rrggbb r g b) (rgb-dec-to-rrggbb (* r 255) (* g 255) (* b 255)))
...
(define (hsl-in-html h s l)
 (let
  ((%type 'string)
   (%ee
    (list "hsl(" (write-to-string h) "," (write-to-string (* s 100)) "%," (write-to-string (* l 100))
     "%)")))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
    (if (null? %ee) %res
...
(define (hsb360-to-hsl h s b)
 (let ((l (* 0.5 b (- 2 s)))) (let ((s2 (/ (* b s) (- 1 (abs (sub1 (* 2 l))))))) (hsl-in-html h s2 l))))

(define (wavelength-to-hsl w)
 (let
  ((hue
    (* 1/6
     (cond ((<= w 362.857) 5) ((< w 440) (+ 4 (/ (- w 440) -60))) ((< w 490) (- 4 (/ (- w 440) 50)))
      ((< w 510) (+ 2 (/ (- w 510) -20))) ((< w 580) (- 2 (/ (- w 510) 70))) ((< w 645) (/ (- w 645) -65))
      (else 0))))
   (brightness
 ...
 (case model
  ((:cmy) (bgroup)
   (call-with-input-string
    ...
    (lambda (i) (egroup)
     (let
      ((c (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
      (let
       ((m (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
       (let
        ((y (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
        (ignorespaces) (rgb-frac-to-rrggbb (- 1 c) (- 1 m) (- 1 y))))))))
  ((:cmyk) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
     ...
     (let
      ...
      (let
       ...
       (let
        ((y (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
        (let
         ((k (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
         (ignorespaces) (cmyk-to-rrggbb c m y k))))))))
  ((:rgb) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
    ...
    (lambda (i) (egroup)
     (let
      ((r (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
      (let
       ((g (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
       (let
        ((b (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
        (ignorespaces) (rgb-frac-to-rrggbb r g b)))))))
  ((:rgb255) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
    ...
    (lambda (i) (egroup)
     (let
      ...
      (let
       ...
       (let
        ((b (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
        (ignorespaces) (rgb-dec-to-rrggbb r g b)))))))
  ((:gray)
   (call-with-input-string (tex-string-to-html-string (get-token))
    (lambda (i)
     (let
      ((g (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
      (ignorespaces) (hsb360-to-hsl 0 0 g)))))
  ((:gray15)
   (call-with-input-string (tex-string-to-html-string (get-token))
    (lambda (i)
     (let
      ((g (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
      (ignorespaces) (hsb360-to-hsl 0 0 (/ g 15))))))
  ((:html)
   (call-with-input-string (tex-string-to-html-string (get-token))
    (lambda (i) (let ((rrggbb (read-6hex i))) (ignorespaces) rrggbb))))
  ((:hsb) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
    ...
    (lambda (i) (egroup)
     (let
      ((h (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
      (let
       ((s (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
       (let
        ((b (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
  ...
  ((:wave)
   (call-with-input-string (tex-string-to-html-string (get-token))
    (lambda (i)
     (let
      ((w (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
      (ignorespaces) (wavelength-to-hsl w)))))
  ((:hsl) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
    ...
    (lambda (i) (egroup)
     (let
      ...
      (let
       ((s (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
       (let
        ((l (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
        (ignorespaces) (hsl-in-html (* 360 h) s l)))))))
  ((:hsl360) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
    ...
    (lambda (i) (egroup)
     (let
      ...
      (let
       ...
       (let
        ((l (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res)))
        (ignorespaces) (hsl-in-html h s l)))))))
  (else
   (let ((name (get-peeled-group)))
    (let ((c (assoc name *color-names*))) (ignorespaces) (if c (cdr c) name))))))

(define (color-model-to-keyword model)
 (cond ((not model) ':colornamed) ((string=? model "rgb") ':rgb) ((string=? model "RGB") ':rgb255)
  ((string=? model "cmyk") ':cmyk) ((string=? model "cmy") ':cmy) ((string=? model "gray") ':gray)
  ((string=? model "Gray") ':gray15) ((string=? model "HTML") ':html) ((string=? model "hsb") ':hsb)
  ((string=? model "Hsb") ':hsb360) ((string=? model "HSB") ':hsb240) ((string=? model "wave") ':wave)
  ((string=? model "hsl") ':hsl) ((string=? model "Hsl") ':hsl360) (else ':colornamed)))
   ...
   (add-postlude-to-top-frame
    (case sw
     ((:rm)
      (cond
       (*math-mode-p*
        (let ((old-math-font *math-font*)) (set! *math-font* ':rm)
         (lambda () (set! *math-font* old-math-font) *math-font*)))
       (else false)))
     ((:em) (emit "<em>") (lambda () (emit "</em>"))) ((:it) (emit "<i>") (lambda () (emit "</i>")))
     ((:bf) (emit "<strong>") (lambda () (emit "</strong>")))
     ((:sl) (emit "<span style=\"font-style: oblique\">") (lambda () (emit "</span>")))
     ((:sf) (emit "<span style=\"font-family: sans-serif\">") (lambda () (emit "</span>")))
     ((:tt)
      (let ((old-ligatures-p *ligatures-p*)) (set! *ligatures-p* false)
       (emit "<span style=\"font-family: monospace\">")
       (lambda () (emit "</span>") (set! *ligatures-p* old-ligatures-p) *ligatures-p*)))
     ((:sc)
      (let ((old-in-small-caps-p *in-small-caps-p*)) (set! *in-small-caps-p* true)
       (lambda () (set! *in-small-caps-p* old-in-small-caps-p) *in-small-caps-p*)))
     ((:span) (emit "<span ") (emit (get-peeled-group)) (emit ">") (lambda () (emit "</span>")))
     ((:div) (emit "<div ") (emit (get-peeled-group)) (emit ">") (lambda () (emit "</div>")))
     ((:tiny) (emit "<span class=tiny>") (lambda () (emit "</span>")))
     ((:scriptsize) (emit "<span class=scriptsize>") (lambda () (emit "</span>")))
     ((:footnotesize :fiverm) (emit "<span class=footnotesize>") (lambda () (emit "</span>")))
     ((:small :sevenrm) (emit "<span class=small>") (lambda () (emit "</span>")))
     ((:normalsize) (emit "<span class=normalsize>") (lambda () (emit "</span>")))
     ((:large) (emit "<span class=large>") (lambda () (emit "</span>")))
     ((:large-cap) (emit "<span class=largecap>") (lambda () (emit "</span>")))
     ...
     ((:cmy :cmyk :rgb :rgb255 :gray :gray15 :html :hsb :hsb360 :hsb240 :wave :hsl :hsl360 :colornamed)
      (emit "<span style=\"color: ") (emit (read-color sw)) (emit "\">") (lambda () (emit "</span>")))
     ((:bgcolor) (emit "<span style=\"background-color: ")
      (let ((model (color-model-to-keyword (get-bracketed-text-if-any)))) (emit (read-color model)))
      (emit "\">") (lambda () (emit "</span>")))
     ((:strike) (emit "<strike>") (lambda () (emit "</strike>")))
     ((:narrower) (do-end-para) (emit "<blockquote>")
      (lambda () (do-end-para) (emit "</blockquote>") (do-para)))
     ((:raggedleft) (do-end-para) (emit "<div class=rightline>")
      (lambda () (do-end-para) (emit "</div>") (do-para)))
     ((:oldstyle) (emit "<span class=oldstyle>") (lambda () (emit "</span>")))
     ((:cal)
      (cond
       (*math-mode-p*
        (let ((old-math-font *math-font*)) (set! *math-font* ':cal)
         (lambda () (set! *math-font* old-math-font) *math-font*)))
       (else false)))
     (else (emit "<span class=") (emit sw) (emit ">") (lambda () (emit "</span>"))))))
  (else false)))

(define (do-obeylines) (ignorespaces ':stop-after-first-newline) (activate-cdef #\newline)
 (tex-def-char #\newline null "\\TIIPbr" false))

(define (do-obeyspaces) (catcode #\space 13) (tex-def-char #\space null "\\TIIPnbsp" false))

(define (do-block z) (do-end-para) (emit "<div ")
 (emit
  (case z ((:flushleft) "class=leftline") ((:flushright) "class=rightline") (else "class=centerline")))
 (emit ">") (set! *tabular-stack* (cons ':block *tabular-stack*)) (emit "<table><tr><td>") (bgroup)
 (emit-newline))

(define (do-end-block) (do-end-para) (egroup) (emit "</td></tr></table></div>") (pop-tabular-stack ':block)
 (emit-newline))

(define (do-function fn)
 (let ((%fluid-var-*math-mode-p* *math-mode-p*))
  (fluid-let ((*math-mode-p* %fluid-var-*math-mode-p*))
   (cond (*outputting-external-title-p* false) ((string=? fn "\\emph") (emit "<em>"))
    ((string=? fn "\\leftline") (do-end-para) (emit "<div class=leftline>"))
    ((string=? fn "\\centerline") (do-end-para) (emit "<div class=centerline>&#xa0;"))
    ((string=? fn "\\rightline") (do-end-para) (emit "<div class=rightline>&#xa0;"))
    ((string=? fn "\\underline") (emit "<u>"))
    ((string=? fn "\\textbf") (set! *math-mode-p* false) (emit "<b>"))
    ((or (string=? fn "\\textit") (string=? fn "\\textsl")) (set! *math-mode-p* false) (emit "<i>"))
    ((string=? fn "\\textrm") (set! *math-mode-p* false) *math-mode-p*)
    ((string=? fn "\\texttt") (set! *math-mode-p* false) (emit "<span style=\"font-family: monospace\">"))
   ...
   (cond (*outputting-external-title-p* false) ((string=? fn "\\emph") (emit "</em>"))
    ...
    ((or (string=? fn "\\leftline") (string=? fn "\\centerline")) (do-end-para) (emit "&#xa0;</div>")
     (emit-newline))
    ((string=? fn "\\underline") (emit "</u>")) ((string=? fn "\\textbf") (emit "</b>"))
    ((or (string=? fn "\\textsl") (string=? fn "\\textit")) (emit "</i>"))
    ((string=? fn "\\texttt") (emit "</span>"))))))

(define (do-discretionary) (tex2page-string (get-group)) (get-group) (get-group))

(define (do-aftergroup) (ignorespaces)
 (let ((z (get-ctl-seq))) (add-aftergroup-to-top-frame (lambda () (toss-back-string z)))))
...
(define (scaled-point-equivalent-of unit)
 (case unit ((:pt) 65536) ((:pc) (* 12 (scaled-point-equivalent-of ':pt)))
  ((:in) (* 72.27 (scaled-point-equivalent-of ':pt))) ((:bp) (* (/ 72) (scaled-point-equivalent-of ':in)))
  ((:cm) (* (/ 2.54) (scaled-point-equivalent-of ':in))) ((:mm) (* 0.1 (scaled-point-equivalent-of ':cm)))
  ((:dd) (* (/ 1238 1157) (scaled-point-equivalent-of ':pt)))
  ((:cc) (* 12 (scaled-point-equivalent-of ':dd))) ((:sp) 1)
  ((:em) (* 16 (scaled-point-equivalent-of ':pt))) ((:ex) (* 0.45 (scaled-point-equivalent-of ':em)))
...
(define (get-scaled-points)
 (let ((n (or (get-real) 1))) (ignorespaces)
  (* n
   (if (= (catcode (snoop-actual-char)) **escape**) (let ((x (get-ctl-seq))) (get-dimen x))
    (begin
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (cond ((not (eat-word "true")) (return)) (else false))
       (if %loop-returned %loop-result (%loop))))
     (cond ((eat-word "bp") (tex-length 1 ':bp)) ((eat-word "cc") (tex-length 1 ':cc))
      ((eat-word "cm") (tex-length 1 ':cm)) ((eat-word "dd") (tex-length 1 ':dd))
      ((eat-word "em") (tex-length 1 ':em)) ((eat-word "ex") (tex-length 1 ':ex))
      ((eat-word "in") (tex-length 1 ':in)) ((eat-word "mm") (tex-length 1 ':mm))
      ((eat-word "pc") (tex-length 1 ':pc)) ((eat-word "pt") (tex-length 1 ':pt)) ((eat-word "sp") 1)
      (else 1)))))))
...
(define (do-vskip)
 (let ((x (get-points))) (eat-skip-fluff false) (emit "<div style=\"height: ") (emit x)
  (emit "pt\"></div>") (emit-newline) (emit "<p style=\"margin-top: 0pt; margin-bottom: 0pt\">")
  (emit-newline) (set! *in-para-p* true) *in-para-p*))
...
(define (do-sup) (emit "<sup>")
 (let ((%fluid-var-*math-script-mode-p* true))
  (fluid-let ((*math-script-mode-p* %fluid-var-*math-script-mode-p*)) (tex2page-string (get-token))))
 (emit "</sup>"))
...
(define (do-hyphen)
 (cond (*math-mode-p* (emit (if (eq? *math-font* ':rm) "-" "&#x2212;"))) ((not *ligatures-p*) (emit #\-))
  (else
   (let ((c (snoop-actual-char)))
    (if (and (char? c) (char=? c #\-)) (begin (get-actual-char) (do-ndash)) (emit #\-))))))

(define (do-excl)
 (if (or *math-mode-p* (not *ligatures-p*)) (emit #\!)
  (let ((c (snoop-actual-char)))
   (if (and (char? c) (char=? c #\`)) (begin (get-actual-char) (emit "&#xa1;")) (emit #\!)))))
...
(define (do-ndash)
 (emit
  (let ((c (snoop-actual-char)))
   (if (and (char? c) (char=? c #\-)) (begin (get-actual-char) "&#x2014;") "&#x2013;"))))
...
(define (do-enquote) (ignorespaces)
 (cond
  ((and (char=? (snoop-actual-char) #\*)) (get-actual-char) (ignorespaces)
   (cond ((= *quote-level* 0) (set! *quote-level* (+ *quote-level* 1)) *quote-level*) (else false)))
  (else false))
 (cond ((not (char=? (get-actual-char) #\{)) (terror 'do-enquote "Missing {")) (else false)) (bgroup)
 (set! *quote-level* (+ *quote-level* 1)) (emit (if (odd? *quote-level*) "&#x201c;" "&#x2018;"))
 (add-aftergroup-to-top-frame
  (lambda () (emit (if (odd? *quote-level*) "&#x201d;" "&#x2019;"))
   (set! *quote-level* (- *quote-level* 1)) *quote-level*)))
...
(define (get-label)
 (let ((lbl (get-peeled-group)))
  (let
   ((i
     (or
      (let ((%position-v #\space) (%position-s lbl))
       (cond ((string? %position-s) (string-index %position-s %position-v))
        (else (list-position %position-v %position-s))))
     ...
     (let*
      ...
      (let %loop ()
       ...
       (unless %loop-returned
        (let
         ((c
           (let* ((%pop-old-stack s) (%pop-top-value (car %pop-old-stack)))
            (begin (set! s (cdr %pop-old-stack)) s) %pop-top-value)))
         (cond
          ((char-whitespace? c) (cond ((not whitep) (set! r (cons #\space r)) r) (else false))
...
(define (emit-ext-page-node-link-start extfile pageno node) (emit "<a ")
 (cond ((not extfile) (emit "class=hrefinternal ")) (else false)) (emit "href=\"")
 (cond
  ((not (and (not extfile) (or (not pageno) (= *html-page-count* pageno)))) (emit (or extfile *jobname*))
   (cond ((not (= pageno 0)) (emit *html-page-suffix*) (emit pageno)) (else false))
   (emit *output-extension*))
  (else false))
 (cond (node (emit "#") (emit node)) (else false)) (emit "\">"))

(define (emit-page-node-link-start pageno node) (emit-ext-page-node-link-start false pageno node))

(define (emit-page-link-start pageno) (emit-ext-page-node-link-start false pageno false))
...
(define (do-anchor-for-potential-label)
 (let
  ((node-name
    (let ((%type 'string) (%ee (list *html-node-prefix* "anchor_" (gen-temp-string))))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (tex-def-0arg "\\TIIPcurrentnodename" node-name) (emit-anchor node-name)))

(define (do-node) (set! *recent-node-name* (get-peeled-group)) *recent-node-name*)

(define (do-label-aux label)
 (let
  ((name (ctl-seq-no-arg-expand-once "\\TIIPcurrentnodename"))
   (value (ctl-seq-no-arg-expand-once "\\@currentlabel")))
  (set! value (tex-string-to-html-string value)) (!label label *html-page-count* name value)
...
(define (do-inputexternallabels)
 (let ((f (get-filename-possibly-braced)))
  (let
   ((fq-f
     (if (fully-qualified-pathname-p f) f
   ...
   (let
    ...
    (let ((ext-label-table (table-get f *external-label-tables*)))
     (cond
      ((not ext-label-table) (set! ext-label-table (make-table ':test equal?))
       (table-put! f *external-label-tables* ext-label-table) (table-get f *external-label-tables*))
      (else false))
     (cond
      ((file-exists? ext-label-file)
       (let ((%fluid-var-*label-source* fq-f) (%fluid-var-*label-table* ext-label-table))
        (fluid-let ((*label-table* %fluid-var-*label-table*) (*label-source* %fluid-var-*label-source*))
   ...
   (cond
    ((file-exists? ext-label-file)
     (let ((%fluid-var-*label-source* jobname))
      (fluid-let ((*label-source* %fluid-var-*label-source*)) (load-tex2page-data-file ext-label-file))))
    (else false)))))

(define (do-tag) (let ((tag-name (get-peeled-group))) (do-tag-aux tag-name (get-group))))

(define (do-definexref)
 (let ((tag (get-peeled-group))) (let ((value (get-group))) (get-token) (do-tag-aux tag value))))

(define (do-tag-aux tag-name tag-val)
 (let
  ((node-name
    (let ((%type 'string) (%ee (list *html-node-prefix* "tag_" (gen-temp-string))))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (tex-def-0arg "\\TIIPcurrentnodename" node-name) (tex-def-0arg "\\@currentlabel" tag-val)
  (emit-anchor node-name) (do-label-aux tag-name)))

(define (do-htmlpagelabel)
 (let ((label (get-peeled-group))) (!label label *html-page-count* false false)
  (write-label `(!label ,label ,*html-page-count* false false))))

(define (do-refexternal) (let ((ext-file (get-peeled-group))) (do-ref-aux (get-label) ext-file false)))

(define (do-ref-aux label ext-file link-text)
 (let ((label-ref (label-bound-p label ext-file)))
  (let
   ((label-text
     (cond (link-text (tex-string-to-html-string link-text)) (label-ref (label*-value label-ref))
      (else label))))
   (cond
    (label-ref
     (emit-ext-page-node-link-start (or ext-file (label*-src label-ref)) (label*-page label-ref)
    ...
    (%ee
     (list (or this-label-src *jobname*)
      (if (= this-label-pageno 0) ""
       (let ((%type 'string) (%ee (list *html-page-suffix* (write-to-string this-label-pageno))))
        (let ((%res (if (eq? %type 'string) "" null)))
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
           (let ((%a (car %ee)))
...
(define (do-htmlref)
 (let ((text (get-group))) (let ((lbl (get-peeled-group))) (do-ref-aux lbl false text))))

(define (do-htmlrefexternal)
 (let ((text (get-group)))
  (let ((extf (get-peeled-group))) (let ((lbl (get-peeled-group))) (do-ref-aux lbl extf text)))))

(define (do-hyperref)
 (let ((lbl (get-bracketed-text-if-any)))
  (if lbl (do-ref-aux lbl false (get-group))
   (let ((text (get-group)))
    (let ((lbl (begin (get-group) (get-group) (get-peeled-group)))) (do-ref-aux lbl false text))))))

(define (do-hypertarget) (let ((lbl (get-peeled-group))) (do-tag-aux lbl "hypertarget")))
...
(define (label-bound-p label . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (ext-file false))
  (when (< 0 %lambda-rest-arg-len) (set! ext-file (list-ref %lambda-rest-arg 0)))
  (let ((label-table (if ext-file (table-get ext-file *external-label-tables*) *label-table*)))
...
(define (flag-unresolved-xref xr)
 (let ((%push-added-value xr) (%push-old-stack *unresolved-xrefs*))
  (cond ((member %push-added-value %push-old-stack) %push-old-stack)
   (else (set! *unresolved-xrefs* (cons %push-added-value %push-old-stack)) *unresolved-xrefs*))))

(define (flag-missing-piece mp)
 (let ((%push-added-value mp) (%push-old-stack *missing-pieces*))
  (cond ((member %push-added-value %push-old-stack) %push-old-stack)
   (else (set! *missing-pieces* (cons %push-added-value %push-old-stack)) *missing-pieces*))))

(define (show-unresolved-xrefs-and-missing-pieces)
 (cond
  ((not (and (null? *unresolved-xrefs*) (null? *missing-pieces*))) (show-unresolved-xrefs)
   (show-missing-pieces) (write-log ':separation-newline) (write-log "Rerun: tex2page ")
   (write-log *main-tex-file*) (write-log ':separation-newline)
   (write-log "If problem persists, check for ") (write-log "missing \\label's and \\bibitem's"))
  (else false)))
...
(define (show-unresolved-xrefs)
 (cond
  ((not (null? *unresolved-xrefs*)) (write-log ':separation-newline)
   ...
   (cond ((> (length *unresolved-xrefs*) 1) (write-log "s")) (else false)) (write-log ": ")
   (set! *unresolved-xrefs* (reverse *unresolved-xrefs*)) (write-log (car *unresolved-xrefs*))
   (for-each (lambda (x) (write-log #\,) (write-log ':separation-space) (write-log x))
...
(define (show-missing-pieces)
 (cond
  ((not (null? *missing-pieces*)) (write-log ':separation-newline)
   (cond
    ((member ':document-title *missing-pieces*) (write-log "Document title not determined")
     (write-log ':separation-newline))
    (else false))
   (cond
    ((member ':last-page *missing-pieces*) (write-log "Last page not determined")
     (write-log ':separation-newline))
    (else false))
   (cond
    ((member ':last-modification-time *missing-pieces*) (write-log "Last modification time not determined")
     (write-log ':separation-newline))
    (else false))
   (cond
    ((member ':stylesheets *missing-pieces*) (write-log "Style sheets not determined")
     (write-log ':separation-newline))
    (else false))
   (cond
    ...
    (else false))
   (cond
    ((member ':html-head *missing-pieces*) (write-log "HTML header info not determined")
     (write-log ':separation-newline))
    (else false))
   (cond
    ((member ':toc *missing-pieces*) (write-log "Table of contents not determined")
     (write-log ':separation-newline))
    (else false))
   (cond
    ((member ':fresh-index *missing-pieces*) (write-log "Index not refreshed")
     (write-log ':separation-newline))
    ((member ':index *missing-pieces*) (write-log "Index not included") (write-log ':separation-newline)))
   (cond
    ((member ':fresh-bibliography *missing-pieces*) (write-log "Bibliography not refreshed")
     (write-log ':separation-newline))
    ((member ':bibliography *missing-pieces*) (write-log "Bibliography not included")
     (write-log ':separation-newline)))
   (cond
    ((member ':metapost *missing-pieces*) (write-log "MetaPost output not included")
     (write-log ':separation-newline))
    (else false)))
  (else false)))
...
(define (do-htmlpageref)
 (let ((label (get-peeled-group)))
  (let ((label-ref (label-bound-p label))) (emit "\"")
   (if label-ref (emit (maybe-label-page (label*-src label-ref) (label*-page label-ref)))
    (emit *log-file*))
   (emit "\""))))

(define (doc-internal-url url)
 (let ((n (let ((%length-arg url)) ((if (string? %length-arg) string-length length) %length-arg))))
  (cond
   ((and (> n 0) (char=? (string-ref url 0) #\#))
    (let ((label (substring url 1)))
     (let ((label-ref (label-bound-p label)))
      (if label-ref
       (if (label*-src label-ref) false (list (label*-page label-ref) (label*-name label-ref))) false))))
   (else false))))
     ...
     (let ((label-ref (label-bound-p label)))
      (if label-ref
       (let
        ((%type 'string)
         (%ee
          (list (maybe-label-page (label*-src label-ref) (label*-page label-ref)) "#"
           (label*-name label-ref))))
        (let ((%res (if (eq? %type 'string) "" null)))
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
         ...
         %res))
       url))))
   ((fully-qualified-url-p url) url) (else (ensure-url-reachable url) url))))

(define (do-url)
 (let ((url (get-url)))
  (let ((durl (doc-internal-url url)))
   (if durl (emit-page-node-link-start (car durl) (cadr durl)) (emit-link-start (fully-qualify-url url))))
...
(define (do-urlh)
 ...
 (bgroup)
 (tex2page-string
  (let ((%type 'string) (%ee (list "\\def\\\\{\\egroup\\endinput}" (get-token))))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
...
(define (do-urlp)
 (let ((link-text (get-token)))
  (let ((url (get-url)))
   (let ((durl (doc-internal-url url)))
    (if durl (emit-page-node-link-start (car durl) (cadr durl))
...
(define (do-htmladdimg)
 (let ((align-info (get-bracketed-text-if-any)))
  (let ((url (get-url))) (emit "<img src=\"") (do-img-src (fully-qualify-url url)) (emit "\"")
   (emit " style=\"border: 0\"") (cond (align-info (tex2page-string align-info)) (else false))
  ...
  (let*
   ...
   (let %loop ()
    (cond ((eat-word "height") (set! height (get-pixels)) height)
     ((eat-word "width") (set! width (get-pixels)) width) ((eat-word "depth") (get-pixels))
     (else (return)))
    (if %loop-returned %loop-result (%loop))))
  (emit "<img") (cond (height (emit " height=") (emit height)) (else false))
  (cond (width (emit " width=") (emit width)) (else false)) (emit " src=\"")
  (do-img-src (fully-qualify-url (get-filename-possibly-braced))) (emit "\">") (ignorespaces) (get-ctl-seq)
  (ignorespaces) (get-ctl-seq)))
...
(define (escape-opmac-index-entry x)
 (let ((y null))
  (let
   ((%dotimes-n (let ((%length-arg x)) ((if (string? %length-arg) string-length length) %length-arg)))
    (i 0))
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
    ...
    (let %loop () (if (>= i %dotimes-n) (return) false)
     (unless %loop-returned
      (let ((c (string-ref x i)))
       (case c ((#\") (set! y (cons c y)) (set! y (cons c y)) y)
        ((#\! #\@) (set! y (cons #\" y)) (set! y (cons c y)) y) (else (set! y (cons c y)) y))))
     (unless %loop-returned (set! i (+ i 1))) (if %loop-returned %loop-result (%loop)))))
   ...
   (let ((pageno (table-get n *index-table*)))
    ...
    (cond
     ((begin (set! *it* (table-get pageno *index-page-mention-alist*)) *it*)
      (let ((n (add1 *it*))) (emit (number-to-roman n)) (table-put! pageno *index-page-mention-alist* n)
       (table-get pageno *index-page-mention-alist*)))
     (else (table-put! pageno *index-page-mention-alist* 1) (table-get pageno *index-page-mention-alist*)))
    (emit-link-stop)))))

(define (do-see-also)
 (let ((other-entry (get-group))) (get-group) (emit "<em>see also</em> ") (tex2page-string other-entry)))

(define (html-length s)
 (let
  ((n (let ((%length-arg s)) ((if (string? %length-arg) string-length length) %length-arg))) (res 0) (i 0)
   (skip-tag false) (skip-entity false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (cond ((>= i n) (return res)) (else false))
    (unless %loop-returned
     (let ((c (string-ref s i))) (set! i (+ i 1))
      (cond (skip-tag (cond ((char=? c #\>) (set! skip-tag false) skip-tag) (else false)))
       (skip-entity (cond ((char=? c #\;) (set! skip-entity false) skip-entity) (else false)))
       ((char=? c #\<) (set! skip-tag true) skip-tag)
       ((char=? c #\&) (set! res (+ res 1)) (set! skip-entity true) skip-entity)
...
(define (do-llap)
 (let ((txt (tex-string-to-html-string (get-group))))
  (let ((html-len (html-length txt)))
   (let ((txt-len (sp-to-pixels (tex-length html-len ':ex)))) (emit "<span style=\"position: relative\">")
    (emit "<span style=\"position: absolute; left: -") (emit txt-len) (emit "pt\">") (emit txt)
    (emit "</span></span>")))))
 ...
 (let ((thing (get-bracketed-text-if-any)))
  (cond
   (thing (set! thing (string-trim thing))
    (cond ((not (string=? thing "")) (bgroup) (emit "<b>") (tex2page-string thing) (emit "</b>") (egroup))
     (else false)))
   (else false)))
 (emit "</dt><dd>"))
...
(define (do-regular-item) (do-end-para) (emit "<li>") (do-para)
 (let ((thing (get-bracketed-text-if-any)))
  (cond (thing (emit "<b>") (bgroup) (tex2page-string thing) (egroup) (emit "</b>") (emit-nbsp 2))
   (else false))))

(define (do-plain-item n) (do-end-para) (emit-newline)
 (let ((parindent (sp-to-pixels (the-dimen "\\parindent")))) (emit "<p style=\"margin-left: ")
  (emit (* n parindent)) (emit "pt; text-indent: 0pt\">") (emit "<span style=\"margin-left: ")
  (emit parindent) (emit "pt\"></span>") (emit "<span style=\"position: relative\">")
  (emit "<span class=item style=\"position: absolute; left: -") (emit parindent) (emit "pt\">")
  (tex2page-string (get-group)) (ignorespaces) (emit-nbsp 2) (emit "</span></span>")))

(define (do-textindent)
 (let ((parindent (sp-to-pixels (the-dimen "\\parindent")))) (do-noindent)
  (emit "<span style=\"margin-left: ") (emit parindent) (emit "pt\"></span>")
...
(define (do-proclaim)
 (let ((head (tex-string-to-html-string (get-till-char #\.))))
  (let ((body (begin (get-actual-char) (ignorespaces) (tex-string-to-html-string (get-till-par)))))
   (do-end-para) (emit "<div class=\"proclaim medskip\"><b>") (do-noindent) (emit head) (emit ".</b>")
   (emit-nbsp 2) (emit "<i>") (emit body) (emit "</i>") (do-end-para) (emit "</div>") (do-para))))

(define (do-item)
 (case (car *tabular-stack*) ((:description) (do-description-item))
  ((:itemize :enumerate) (do-regular-item)) (else (do-plain-item 1))))

(define (do-itemize) (do-end-para) (set! *tabular-stack* (cons ':itemize *tabular-stack*)) (emit "<ul")
 (cond
  ((or (tex2page-flag-boolean "\\TIIPslides") (tex2page-flag-boolean "\\TZPslides"))
   (emit " class=incremental"))
  (else false))
 (emit ">") (emit-newline))

(define (do-enditemize) (do-end-para) (pop-tabular-stack ':itemize) (emit "</ul>") (do-noindent))

(define (do-enumerate) (do-end-para) (set! *tabular-stack* (cons ':enumerate *tabular-stack*)) (emit "<ol")
 (cond
  ((or (tex2page-flag-boolean "\\TIIPslides") (tex2page-flag-boolean "\\TZPslides"))
   (emit " class=incremental"))
  (else false))
...
(define (do-endenumerate) (pop-tabular-stack ':enumerate) (do-end-para) (emit "</ol>") (do-noindent))

(define (do-opmac-list-style) (ignorespaces) (set! *opmac-list-style* (get-actual-char))
 *opmac-list-style*)

(define (do-bigskip type) (do-end-para) (emit "<div class=")
 (emit (case type ((:medskip) "medskip") ((:bigskip) "bigskip") (else "smallskip"))) (emit "></div>")
 (emit-newline) (emit "<p style=\"margin-top: 0pt; margin-bottom: 0pt\">") (set! *in-para-p* true)
 (emit-newline))

(define (do-hspace) (ignorespaces) (cond ((eqv? (snoop-actual-char) #\*) (get-actual-char)) (else false))
 (get-group) (emit-nbsp 3))

(define (do-vspace) (ignorespaces) (cond ((eqv? (snoop-actual-char) #\*) (get-actual-char)) (else false))
  ...
  (lambda ()
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (ignorespaces ':all)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (case (string->symbol (string-append ":" (string-upcase (scm-get-token))))
       ((:image :display-image) (tex-def-0arg "\\TZPmathtext" "0") (tex-def-0arg "\\TZPmathimage" "1"))
       ((:no-image :no-display-image) (tex-def-0arg "\\TZPmathtext" "1")
        (tex-def-0arg "\\TZPmathimage" "0"))))
     (if %loop-returned %loop-result (%loop)))))))

(define (do-htmldoctype)
 (let ((d (get-peeled-group))) (cond ((string=? d "") (set! d 'none) d) (else false))
  (write-aux `(!doctype ,d))))
 ...
 (call-with-input-string/buffered (ungroup (get-group))
  (lambda ()
   (let*
    ...
    (let %loop () (ignorespaces ':all)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (let ((directive (string->symbol (string-append ":" (string-upcase (scm-get-token))))))
       (!colophon directive) (write-aux `(!colophon ,directive))))
     (if %loop-returned %loop-result (%loop)))))))

(define (output-colophon)
 (let
  ((colophon-mentions-last-mod-time-p
  ...
  (cond
   ((or colophon-mentions-last-mod-time-p colophon-mentions-tex2page-p) (do-end-para)
    (emit "<div class=\"rightline colophon\">")
    (cond
     ((and colophon-mentions-last-mod-time-p *last-modification-time* (> *last-modification-time* 0))
      (tex2page-string *last-modified*) (emit ": ") (emit (seconds-to-human-time *last-modification-time*))
      (emit "<br>"))
     (else false))
    (cond
     (colophon-mentions-tex2page-p (emit "<div class=\"rightline advertisement\">")
      ...
      (cond (colophon-links-to-tex2page-website-p (emit-link-stop)) (else false)) (emit "</div>"))
     (else false))
    (emit "</div>") (emit-newline))
   (else false))))
...
(define (point-to-adjacent-pages)
 (let ((prev-page (if (= *html-page-count* 0) false (sub1 *html-page-count*))))
  (let ((next-page (if (= *html-page-count* *last-page-number*) false (add1 *html-page-count*))))
   (cond
    ((not (= *last-page-number* 0)) (cond (prev-page (emit-page-link-start prev-page)) (else false))
     (emit "&#x3c;&#xb7;&#xb7;&#xb7;Prev ") (cond (prev-page (emit-link-stop)) (else false)) (emit "||")
     (cond (next-page (emit-page-link-start next-page)) (else false))
     (emit " Next&#xb7;&#xb7;&#xb7;&#x3e;") (cond (next-page (emit-link-stop)) (else false)))
    (else false)))))

(define (output-head-or-foot-line head-or-foot)
 (cond
  ((not (or (tex2page-flag-boolean "\\TIIPsinglepage") (tex2page-flag-boolean "\\TZPsinglepage")))
   (do-end-para) (emit "<div class=navigation>")
   (cond
    ((or (tex2page-flag-boolean "\\TIIPtexlayout") (tex2page-flag-boolean "\\TZPtexlayout")) (bgroup)
     (tex-let "\\folio" "\\TIIPfolio" false)
  ...
  (let ((last-page-not-determined-p (< *last-page-number* 0)))
   (let ((last-page-p (= *html-page-count* *last-page-number*)))
    (let ((toc-page-p (and *toc-page* (= *html-page-count* *toc-page*))))
     (let ((index-page-p (and *index-page* (= *html-page-count* *index-page*))))
      (let ((prev-page (if first-page-p false (sub1 *html-page-count*))))
       (let ((next-page (if last-page-p false (add1 *html-page-count*))))
        (cond
         ((not
           (and first-page-p (or last-page-p (and (eq? head-or-foot ':head) last-page-not-determined-p))))
          (emit "[") (emit *navigation-sentence-begin*) (emit "<span")
          (cond (first-page-p (emit " class=disable")) (else false)) (emit ">")
          (cond ((not first-page-p) (emit-page-link-start 0)) (else false)) (emit *navigation-first-name*)
          (cond ((not first-page-p) (emit-link-stop)) (else false)) (emit ", ")
          (cond ((not first-page-p) (emit-page-link-start prev-page)) (else false))
          (emit *navigation-previous-name*) (cond ((not first-page-p) (emit-link-stop)) (else false))
          (emit "</span>") (emit "<span") (cond (last-page-p (emit " class=disable")) (else false))
          (emit ">") (cond (first-page-p (emit "<span class=disable>")) (else false)) (emit ", ")
          (cond (first-page-p (emit "</span>")) (else false))
          (cond ((not last-page-p) (emit-page-link-start next-page)) (else false))
          (emit *navigation-next-name*) (cond ((not last-page-p) (emit-link-stop)) (else false))
          ...
          (cond
           ((or *toc-page* *index-page*) (emit "<span")
            ...
            (cond
             (*toc-page* (emit "<span") (cond (toc-page-p (emit " class=disable")) (else false)) (emit ">")
              (cond
               ((not toc-page-p)
                (emit-page-node-link-start *toc-page*
                 (let ((%type 'string) (%ee (list *html-node-prefix* "toc")))
                  (let ((%res (if (eq? %type 'string) "" null)))
                   ...
                   %res))))
               (else false))
              (emit *navigation-contents-name*) (cond ((not toc-page-p) (emit-link-stop)) (else false))
              (emit "</span>"))
             (else false))
            (cond
             (*index-page* (emit "<span") (cond (index-page-p (emit " class=disable")) (else false))
              (emit ">") (emit "<span")
              (cond ((not (and *toc-page* (not toc-page-p))) (emit " class=disable")) (else false))
              (emit ">") (cond (*toc-page* (emit "; ") (emit-nbsp 2)) (else false)) (emit "</span>")
              (cond
               ((not index-page-p)
                (emit-page-node-link-start *index-page*
                 (let ((%type 'string) (%ee (list *html-node-prefix* "index_start")))
                  (let ((%res (if (eq? %type 'string) "" null)))
                   ...
                   %res))))
               (else false))
              (emit *navigation-index-name*) (cond ((not index-page-p) (emit-link-stop)) (else false))
              (emit "</span>"))
             (else false)))
           (else false))
...
(define (do-eject) (ignorespaces)
 (cond
  ((or (tex2page-flag-boolean "\\TIIPslides") (tex2page-flag-boolean "\\TZPslides")) (do-end-para)
   (emit "</div>") (emit-newline) (emit "<div class=slide>") (do-para))
  ((or (tex2page-flag-boolean "\\TIIPsinglepage") (tex2page-flag-boolean "\\TZPsinglepage")) true)
  (else
   (cond
    ((not (and (not (snoop-actual-char)) (eqv? *current-source-file* *main-tex-file*)))
     (cond ((not (> *last-page-number* 0)) (flag-missing-piece ':last-modification-time)) (else false))
     (do-end-page)
     (tex-def-count 0 (begin (set! *html-page-count* (+ *html-page-count* 1)) *html-page-count*) true)
     (set! *html-page*
      (let
       ((%type 'string)
        (%ee
         (list *aux-dir/* *jobname* *html-page-suffix* (write-to-string *html-page-count*)
          *output-extension*)))
       (let ((%res (if (eq? %type 'string) "" null)))
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
...
(define (output-html-preamble)
 (cond ((string? *doctype*) (emit "<!DOCTYPE ") (emit *doctype*) (emit ">") (emit-newline)) (else false))
 (emit "<html lang=") (emit (resolve-defs "\\TZPlang")) (emit ">") (emit-newline) (emit "<!--")
 (emit-newline) (emit "Generated from ") (emit *main-tex-file*) (emit " by tex2page, ") (emit "v. ")
 (emit *tex2page-version*) (emit-newline) (emit *tex2page-copyright-notice*) (emit-newline)
 (emit "(running on ") (emit *scheme-version*) (emit ")") (emit-newline) (emit *tex2page-website*)
 (emit-newline) (emit "-->") (emit-newline) (emit "<head>") (emit-newline)
 (emit "<meta charset=\"utf-8\">") (emit-newline) (output-external-title) (link-stylesheets) (link-scripts)
 (emit "<meta name=robots content=\"index,follow\">") (emit-newline) (for-each emit *html-head*)
 (emit "</head>") (emit-newline) (emit "<body>") (emit-newline) (emit "<div")
 (cond
  ((or (tex2page-flag-boolean "\\TIIPslides") (tex2page-flag-boolean "\\TZPslides")) (emit " class=slide"))
  (else false))
 (emit ">") (emit-newline))

(define (output-html-postamble) (do-end-para) (emit "</div>") (emit-newline) (emit "</body>")
 (emit-newline) (emit "</html>") (emit-newline))

(define (redirect-if-needed)
 (cond
  (*redirect-url* (emit "If not redirected in ") (emit *redirect-delay*) (emit " sec, go to ")
   (emit-link-start (fully-qualify-url *redirect-url*)) (emit *redirect-url*) (emit-link-stop))
  (else false)))

(define (check-tex2page-lisp)
 (let ((cl-p (not null)) (doc-expects-cl-p (tex2page-flag-boolean "\\TZPcommonlisp")))
  (cond
   ((not (eqv? cl-p doc-expects-cl-p)) (write-log ':separation-newline) (write-log "! Document ")
    (write-log *main-tex-file*) (write-log " appears to require ")
    (write-log (if doc-expects-cl-p "Common Lisp" "Scheme")) (write-log " version of TeX2page."))
   (else false))))

(define (do-start) (check-tex2page-lisp) (set! *footnote-list* null) (output-html-preamble)
 (output-head-or-foot-line ':head) (do-para) (redirect-if-needed))

(define (do-end-page) (do-end-para) (output-footnotes) (do-bigskip ':smallskip)
 (output-head-or-foot-line ':foot) (do-para)
 (let ((colophon-on-last-page-p (tex2page-flag-boolean "\\TZPcolophonlastpage")))
  (cond
   ((or (and (not colophon-on-last-page-p) (= *html-page-count* 0))
     (and colophon-on-last-page-p (= *html-page-count* *last-page-number*)))
    (output-colophon))
   (else false)))
...
(define (output-stats) (write-log ':separation-newline)
 (cond
  (*main-tex-file* (write-log "Output written on ") (write-log *aux-dir/*) (write-log *jobname*)
   (write-log *output-extension*) (cond ((> *html-page-count* 0) (write-log ", ...")) (else false))
   (write-log " (") (write-log (add1 *html-page-count*)) (write-log " page")
   (cond ((not (= *html-page-count* 0)) (write-log #\s)) (else false))
   (cond
    ((> *img-file-tally* 0) (write-log ", ") (write-log *img-file-tally*) (write-log " image")
...
(define (do-endinput) (toss-back-char *invisible-space*) (toss-back-string "\\TIIPendinput"))

(define (do-bye) (check-tex2page-lisp) (note-down-tex2page-flags)
 (cond
  ((not (null? *tex-if-stack*))
   (let ((n (length *tex-if-stack*)))
    (trace-if true "(\\end occurred when " n " \\if" (if (> n 1) "s were" " was") " incomplete)")))
  (else false))
 (cond
  ((not (null? *tex-env*)) (trace-if true "\\end occurred inside a group at level " (length *tex-env*)))
  (else false))
 (perform-postludes)
 (cond ((not (or (>= *last-page-number* 0) (= *html-page-count* 0))) (flag-missing-piece ':last-page))
  (else false))
 (!last-page-number *html-page-count*) (write-aux `(!last-page-number ,*last-page-number*)) (do-end-page)
 (cond (*last-modification-time* (write-aux `(!last-modification-time ,*last-modification-time* 1900)))
  (else false))
 (for-each (lambda (th) (th)) *afterbye*) (close-all-open-streams) (call-external-programs-if-necessary)
  ...
  ((hsize
    (cond
     ((begin (set! *it* (find-def "\\TZPhsize")) *it*)
      (tex2page-string
       (let ((%type 'string) (%ee (list "\\TIIPhsize=" (tdef*-expansion *it*))))
        (let ((%res (if (eq? %type 'string) "" null)))
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
           (let ((%a (car %ee)))
         ...
         %res)))
      (the-dimen "\\TIIPhsize"))
     ((or (tex2page-flag-boolean "\\TIIPtexlayout") (tex2page-flag-boolean "\\TZPtexlayout"))
      (the-dimen "\\hsize"))
     (else false))))
  (cond
...
(define (note-down-tex2page-flags) (write-aux `(!lang ,(resolve-defs "\\TZPlang")))
 (write-aux `(!head-line ,(the-toks "\\headline"))) (write-aux `(!foot-line ,(the-toks "\\footline")))
 (cond
  ((begin (set! *it* (find-def "\\TZPtitle")) *it*)
   (let ((d *it*)) (write-aux `(!preferred-title ,(tex-string-to-html-string (tdef*-expansion d))))))
  (else false))
 (cond ((tex2page-flag-boolean "\\TZPcolophonlastpage") (write-aux `(!colophon ':last-page))) (else false))
 (cond
 ...
 (cond
  ((begin (set! *it* (ctl-seq-no-arg-expand-once "\\TZPredirect")) *it*)
   ...
   (let ((url *it*) (seconds (ctl-seq-no-arg-expand-once "\\TZPredirectseconds")))
    (write-aux `(!html-redirect ,url ,seconds))))
  (else false))
 (cond
  ((tex2page-flag-boolean "\\TZPslides") (write-aux '(!slides)) (write-aux '(!single-page))
   (let ((slidy-css-file "slidy.css"))
    (cond
     ((not (file-exists? slidy-css-file))
      (set! slidy-css-file "http://www.w3.org/Talks/Tools/Slidy2/styles/slidy.css") slidy-css-file)
     (else false))
    (cond ((null? *stylesheets*) (flag-missing-piece ':stylesheets)) (else false))
   ...
   (let ((slidy-js-file "slidy.js"))
    (cond
     ((not (file-exists? slidy-js-file))
      (set! slidy-js-file "http://www.w3.org/Talks/Tools/Slidy2/scripts/slidy.js") slidy-js-file)
     (else false))
 ...
 (cond
  ((tex2page-flag-boolean "\\TZPtexlayout") (write-aux '(!tex-like-layout)) (newline *css-stream*)
   (display "body { margin-top: " *css-stream*)
   (display (sp-to-ems (+ (tex-length 0.5 ':in) (the-dimen "\\voffset"))) *css-stream*)
   (display "em; }" *css-stream*) (newline *css-stream*) (display "body { margin-left: " *css-stream*)
   (display (sp-to-ems (+ (tex-length 0.8 ':in) (the-dimen "\\hoffset"))) *css-stream*)
   (display "em; }" *css-stream*) (newline *css-stream*)
   (cond
    ((or (tex2page-flag-boolean "\\TZPrightjustify") (not (tex2page-flag-boolean "\\TZPraggedright")))
   ...
   (display (sp-to-pixels (the-dimen "\\belowdisplayskip")) *css-stream*) (display "pt; }" *css-stream*)
   (newline *css-stream*) (display ".navigation { color: black; font-style: normal; }" *css-stream*)
   (newline *css-stream*))
  (else false))
 (set-text-width)
 (cond ((not (tex2page-flag-boolean "\\TZPtextext")) (write-aux `(!tex-text 1))) (else false)))

(define (insert-missing-end) (write-log ':separation-newline) (write-log "! Missing \\end inserted.")
 (write-log ':separation-newline))

(define do-diacritic
 (let
  ((top-diacritics
    '(:grave :acute :circumflex :umlaut :tilde :macron :breve :hacek :hungarianumlaut :ring)))
  (lambda (diac)
   (let ((x (get-token-or-peeled-group)))
    (cond ((and (string=? x "\\i") (member diac top-diacritics)) (emit "i"))
     ((and (string=? x "\\j") (member diac top-diacritics)) (emit "j")) (else (tex2page-string x))))
   (emit
    (case diac ((:grave) "&#x300;") ((:acute) "&#x301;") ((:circumflex) "&#x302;") ((:umlaut) "&#x308;")
     ((:tilde) "&#x303;") ((:macron) "&#x304;") ((:dot) "&#x307;") ((:breve) "&#x306;")
     ((:hacek) "&#x30c;") ((:hungarianumlaut) "&#x30b;") ((:cedilla) "&#x327;") ((:dotunder) "&#x323;")
     ((:barunder) "&#x331;") ((:tieafter) "&#x361;") ((:ring) "&#x30a;") ((:ogonek) "&#x328;")
     (else (error 'ecase "0xdeadc0de")))))))

(define (do-backslash-equal)
 (cond
  ((not (and (not (null? *tabular-stack*)) (eqv? (car *tabular-stack*) ':tabbing)))
   (do-diacritic ':macron))
  (else false)))

(define (do-mathdg)
 (let
  ((%fluid-var-*math-mode-p* true) (%fluid-var-*in-display-math-p* true) (%fluid-var-*tabular-stack* null)
   (%fluid-var-*ligatures-p* false))
  (fluid-let
   ((*ligatures-p* %fluid-var-*ligatures-p*) (*tabular-stack* %fluid-var-*tabular-stack*)
    (*in-display-math-p* %fluid-var-*in-display-math-p*) (*math-mode-p* %fluid-var-*math-mode-p*))
   (do-end-para) (emit "<div class=") (emit *display-justification*) (emit "><table><tr><td>")
   (tex2page-string (get-group)) (emit "</td></tr></table></div>") (do-para))))

(define (do-mathg)
 (let
  ((%fluid-var-*math-mode-p* true) (%fluid-var-*in-display-math-p* false) (%fluid-var-*tabular-stack* null)
   (%fluid-var-*ligatures-p* false))
  (fluid-let
   ((*ligatures-p* %fluid-var-*ligatures-p*) (*tabular-stack* %fluid-var-*tabular-stack*)
    (*in-display-math-p* %fluid-var-*in-display-math-p*) (*math-mode-p* %fluid-var-*math-mode-p*))
...
(define (dump-tex-preamble o)
 (case *tex-format*
  ((:latex) (display "\\documentclass{" o) (display (if *using-chapters-p* "report" "article") o)
   (display "}" o) (newline o) (display *imgpreamble* o) (newline o)
   (cond
    ((member ':includegraphics *imgpreamble-inferred*) (display "\\ifx\\includegraphics\\UNDEFINED" o)
     (display "\\usepackage{graphicx}\\fi" o) (newline o))
    (else false))
   (cond
    ((member ':epsfbox *imgpreamble-inferred*) (display "\\ifx\\epsfbox\\UNDEFINED" o)
     (display "\\usepackage{epsfig}\\fi" o) (newline o))
    (else false))
   (display "\\thispagestyle{empty}" o) (newline o) (display "\\begin{document}" o) (newline o))
  (else (display *imgpreamble* o) (newline o)
   (cond
    ((member ':includegraphics *imgpreamble-inferred*) (display "\\ifx\\resetatcatcode\\UNDEFINED" o)
     (display "\\input miniltx \\fi" o) (newline o) (display "\\ifx\\includegraphics\\UNDEFINED" o)
     (display "\\input graphicx.sty \\fi" o) (newline o))
    (else false))
   (cond
    ((member ':epsfbox *imgpreamble-inferred*) (display "\\ifx\\epsfbox\\UNDEFINED" o)
     (display "\\ifx\\pdfoutput\\UNDEFINED\\input epsf \\else" o) (display "\\input supp-pdf " o)
     (display "\\def\\epsfbox#1{\\convertMPtoPDF{#1}{1}{1}}\\fi" o) (newline o))
    (else false))
   (display "\\nopagenumbers" o) (newline o))))

(define (dump-tex-postamble o)
 (case *tex-format* ((:latex) (display "\\end{document}" o) (newline o))
  (else (display "\\bye" o) (newline o))))

(define (skipping-img-file) (set! *img-file-count* (+ *img-file-count* 1)) *img-file-count*)

(define (next-html-image-file-stem) (set! *img-file-count* (+ *img-file-count* 1))
 (let ((%type 'string) (%ee (list *subjobname* *img-file-suffix* (write-to-string *img-file-count*))))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
    (if (null? %ee) %res
     (let ((%a (car %ee)))
  ...
  (let ((img-file-stem (next-html-image-file-stem)))
   (let
    ...
    (let
     ((o
       (let*
        ((%f aux-tex-file) (%ee (list ':direction ':output ':if-exists ':supersede))
         (%direction (memv ':direction %ee)) (%if-exists (memv ':if-exists %ee))
         (%if-does-not-exist ':error) (%if-does-not-exist-from-user (memv ':if-does-not-exist %ee)))
        (when %direction (set! %direction (cadr %direction)))
        (when %if-exists (set! %if-exists (cadr %if-exists)))
        ...
        (cond
         ((eqv? %direction ':output)
          ...
          (open-output-file %f))
         ((and (not %if-does-not-exist) (not (file-exists? %f))) false) (else (open-input-file %f))))))
     (let ((%with-open-file-res (begin (dump-tex-preamble o) (p o) (dump-tex-postamble o))))
      (cond (o ((if (input-port? o) close-input-port close-output-port) o)) (else false))
      %with-open-file-res))
...
(define (do-math-fragment s . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (display-p false))
  (when (< 0 %lambda-rest-arg-len) (set! display-p (list-ref %lambda-rest-arg 0)))
  (cond (display-p (emit "<div class=\"mathdisplay ") (emit *display-justification*) (emit "\">"))
   (else false))
  (let
   ((old-math-mode-p *math-mode-p*) (old-in-display-math-p *in-display-math-p*)
    (old-tabular-stack *tabular-stack*))
   (set! *math-mode-p* true) (set! *in-display-math-p* display-p) (set! *tabular-stack* null)
   (cond (display-p (emit "<table style=\"margin-left:auto; margin-right:auto\"><tr><td>")) (else false))
   (bgroup) (toss-back-char #\}) (toss-back-string s)
   (add-aftergroup-to-top-frame
    (lambda () (set! *math-mode-p* old-math-mode-p) (set! *in-display-math-p* old-in-display-math-p)
     (set! *tabular-stack* old-tabular-stack) (cond (display-p (emit "</td></tr></table>")) (else false))
...
(define (tex-math-delim-string type)
 (let ((top null) (mid null) (bot null) (ext null))
  (case type
   ((:lparen) (set! top "&#x239b;") (set! bot "&#x239d;") (set! ext "&#x239c;") (set! mid ext) mid)
   ((:lbrack) (set! top "&#x23a1;") (set! bot "&#x23a3;") (set! ext "&#x23a2;") (set! mid ext) mid)
   ((:lbrace) (set! top "&#x23a7;") (set! mid "&#x23a8;") (set! bot "&#x23a9;") (set! ext "&#x23aa;") ext)
   ((:lvert) (set! ext "&#x239c;") (set! top ext) (set! mid ext) (set! bot ext) bot)
   ((:nulldelim) (set! ext "") (set! top ext) (set! mid ext) (set! bot ext) bot)
   ((:rparen) (set! top "&#x239e;") (set! bot "&#x23a0;") (set! ext "&#x239f;") (set! mid ext) mid)
   ((:rbrack) (set! top "&#x23a4;") (set! bot "&#x23a6;") (set! ext "&#x23a5;") (set! mid ext) mid)
   ((:rbrace) (set! top "&#x23ab;") (set! mid "&#x23ac;") (set! bot "&#x23ad;") (set! ext "&#x23ae;") ext)
   ((:rvert) (set! ext "&#x239f;") (set! top ext) (set! mid ext) (set! bot ext) bot)
   (else (error 'ecase "0xdeadc0de")))
  (let
   ((%type 'string)
    (%ee
     (list "<table class=mathdelim><tr><td>" top "</td></tr>"
      (cond
       ((odd? *math-height*)
        (let
         ((%type 'string)
              ...
              (let*
               ((%loop-returned false) (%loop-result 0)
                (return
                 (lambda %args (set! %loop-returned true)
                  (set! %loop-result (and (pair? %args) (car %args))))))
               (let %loop () (if (>= i %dotimes-n) (return r) false)
                (unless %loop-returned
                 (set! r
                  (let ((%type 'string) (%ee (list r "<tr><td>" ext "</td></tr>")))
                   (let ((%res (if (eq? %type 'string) "" null)))
                    (let %concatenate-loop ((%ee %ee))
                     (if (null? %ee) %res
                      (let ((%a (car %ee)))
...
(define (do-math)
 (let ((display-p false))
  (cond ((eqv? (snoop-actual-char) #\$) (set! display-p true) (get-actual-char)) (else false))
  (let ((o (open-output-string)))
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (dump-till-char #\$ o)
     (cond ((not display-p) (return))
      (else
       (let ((c (get-actual-char)))
        (cond ((not c) (terror 'do-math "Display math should end with $$.")) ((char=? c #\$) (return))
...
(define (dump-till-char d o)
 (let ((nesting 0) (escape-p false) (c false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (set! c (get-actual-char))
    (cond ((not c) (terror 'dump-till-char "Missing " d ".")) (else false))
    (cond ((and (char=? c d) (= nesting 0)) (return)) (else false)) (unless %loop-returned (display c o))
    (unless %loop-returned
     (cond (escape-p (set! escape-p false) escape-p) ((char=? c #\{) (set! nesting (+ nesting 1)) nesting)
    ...
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop ()
      (let ((c (snoop-actual-char))) (cond ((not c) (terror 'dump-till-ctl-seq)) (else false))
       (cond
        ((= (catcode c) **escape**) (let ((x (get-ctl-seq))) (if (string=? x cs) (return) (display x o))))
                ...
                (let ((g (get-grouped-environment-name-if-any)))
                 (cond
                  ((and g (or *dumping-nontex-p* (= env-nesting 0))
                   ...
                   (return))
                  (else (display x o) (cond (g (display #\{ o) (display g o) (display #\} o)) (else false))
                   (cond ((and g (string=? g env)) (set! env-nesting (- env-nesting 1)) env-nesting)
                    (else false))))))
               (else (display x o)))))
            ((and (= (catcode c) **comment**) (not *dumping-nontex-p*)) (do-comment) (write-char #\% o)
             (newline o))
...
(define (dump-imgdef f)
 (let
  ...
  (let
   ...
   (let
    ((%with-open-file-res
      (begin (dump-tex-preamble o) (display (ungroup (get-group)) o) (dump-tex-postamble o))))
    (cond (o ((if (input-port? o) close-input-port close-output-port) o)) (else false))
    %with-open-file-res))))
  ...
  (let ((%fluid-var-*not-processing-p* true) (r *imgpreamble*))
   (fluid-let ((*not-processing-p* %fluid-var-*not-processing-p*))
    (let*
     ...
     (let %loop ()
      (let ((c (snoop-actual-char)))
       (cond ((not c) (terror 'do-img-preamble "Missing \\endimgpreamble")) (else false))
       (cond
        ((= (catcode c) **escape**)
         (let ((x (get-ctl-seq)))
          (cond ((member x '("\\endimgpreamble" "\\endgifpreamble" "\\endmathpreamble")) (return r))
           (else false))
          (set! r
...
(define (do-wlog) (let ((output (tex-write-output-string (get-peeled-group)))) (write-log output true)))
...
(define (do-ifeof)
 (let ((i (get-number)))
  (let ((c (table-get i *input-streams*))) (cond ((eq? c ':free) (terror 'do-ifeof)) (else false))
   (if
    (not
     (let*
      ((%read-char-port c) (%read-char-res (if %read-char-port (read-char %read-char-port) (read-char))))
      (when (eof-object? %read-char-res) (set! %read-char-res false)) %read-char-res))
    (do-iftrue) (do-iffalse)))))

(define (do-iffalse) (set! *tex-if-stack* (cons false *tex-if-stack*)) *tex-if-stack*)

(define (do-iftrue) (set! *tex-if-stack* (cons true *tex-if-stack*)) *tex-if-stack*)
         ...
         (set! one2
          (cond
           ((begin (set! *it* (find-def one)) *it*)
            (let ((d *it*)) (or (tdef*-expansion d) (tdef*-prim d))))
           ((begin (set! *it* (find-math-def one)) *it*) *it*) (else "UnDeFiNeD")))
         one2)
        (else false))
       (cond
        ((ctl-seq-p two)
         (set! two2
          (cond
           ((begin (set! *it* (find-def two)) *it*)
            (let ((d *it*)) (or (tdef*-expansion d) (tdef*-prim d))))
           ((begin (set! *it* (find-math-def two)) *it*) *it*) (else "UnDeFiNeD")))
         two2)
        (else false))
       (if (or (eqv? one2 two2) (and (string? one2) (string? two2) (string=? one2 two2))) (do-iftrue)
        (do-iffalse)))))))))
    ...
    (if (ctl-seq-p x)
     (cond
      ((begin (set! *it* (resolve-defs x)) *it*)
       (let ((z *it*)) (toss-back-char *invisible-space*) (toss-back-string z)))
      (else (return x)))
     (return x)))
   (if %loop-returned %loop-result (%loop)))))
...
(define (do-if)
 (let ((one (do-if-get-atomic)))
  (let ((two (do-if-get-atomic)))
   (if (or (string=? one two) (and (ctl-seq-p one) (ctl-seq-p two))) (do-iftrue) (do-iffalse)))))

(define (do-ifmmode) (set! *tex-if-stack* (cons *math-mode-p* *tex-if-stack*)) *tex-if-stack*)
...
(define (do-ifnum)
 (let ((one (get-number)))
  (let ((rel (string-ref (get-raw-token/is) 0)))
   (let ((two (get-number)))
    (if
     ((case rel ((#\<) <) ((#\=) =) ((#\>) >) (else (terror 'do-ifnum "Missing = for \\ifnum."))) one two)
     (do-iftrue) (do-iffalse))))))
      ...
      (let ((outer-loop-done false))
       (let*
        ((%loop-returned false) (%loop-result 0)
         (return
          (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
        (let %loop () (cond (outer-loop-done (return)) (else false))
         (unless %loop-returned
          (let ((clause ""))
           (let*
            ((%loop-returned false) (%loop-result 0)
            ...
            (let %loop ()
             (let ((c (snoop-actual-char)))
              (cond ((not c) (terror 'read-ifcase-clauses "Incomplete \\ifcase.")) (else false))
              (cond
               ((= (catcode c) **escape**)
                (let ((x (get-ctl-seq)))
                 (cond
                  ((string=? x "\\or") (ignorespaces)
                   (cond (elsep (terror 'read-ifcase-clauses "Extra \\or.")) (else false))
                   (set! or-clauses (cons clause or-clauses)) (return))
                  ((string=? x "\\else") (ignorespaces)
                   (cond (elsep (terror 'read-ifcase-clauses "Extra \\else.")) (else false))
                   (set! or-clauses (cons clause or-clauses)) (set! elsep true) (return))
                  ((string=? x "\\fi") (ignorespaces)
                   (cond (elsep (set! else-clause clause) else-clause)
                    (else (set! or-clauses (cons clause or-clauses)) or-clauses))
 ...
 (let ((iffoo (get-ctl-seq)))
  (let
   ...
   (let
    ...
    (tex-def-thunk iffoo
     (lambda () (set! *tex-if-stack* (cons (> (the-count foo-register) 0) *tex-if-stack*)) *tex-if-stack*)
     false)
    (tex-def-thunk
...
(define (find-img-file-extn)
 (case (tex2page-flag-value "\\TZPimageformat") ((#\g #\G) ".gif") ((#\j #\J) ".jpeg") (else ".png")))

(define (do-htmlimageformat) (tex-def-0arg "\\TZPimageformat" (get-peeled-group)))

(define (do-htmlimageconversionprogram) (tex-def-0arg "\\TZPimageconverter" (get-peeled-group)))

(define (do-htmlimgmagnification) true)

(define call-mp
 (let ((tex-prog-name false))
  (lambda (f) (cond ((not tex-prog-name) (set! tex-prog-name "tex") tex-prog-name) (else false))
   (system
    (let ((%type 'string) (%ee (list *metapost* " -tex=" tex-prog-name " " f)))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
...
(define call-tex
 (let ((tex-prog-name false) (tex-output-format false))
  (lambda (f)
   (cond
    ((not tex-prog-name)
     (let ((d (find-def "\\TZPtexprogname")))
      (cond (d (set! tex-prog-name (tdef*-expansion d)) tex-prog-name) (else false)))
     (cond ((not tex-prog-name) (set! tex-prog-name "luatex") tex-prog-name) (else false))
     (set! tex-output-format
      (if
       (or (eqv? (substring? "pdf" tex-prog-name) 0) (eqv? (substring? "xe" tex-prog-name) 0)
   ...
   (let
    ((dvi-file
      (let ((%type 'string) (%ee (list f (if (eq? tex-output-format ':pdf) ".pdf" ".dvi"))))
       (let ((%res (if (eq? %type 'string) "" null)))
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
          (let ((%a (car %ee)))
                   ...
                   (let %loop ()
                    (set! x
                     (let ((%read-line-res (read-line i)))
                      (when (eof-object? %read-line-res) (set! %read-line-res false)) %read-line-res))
                    (cond ((not x) (return true)) (else false))
                    (unless %loop-returned
                     (cond ((substring? "! I can't find file" x) (return false)) (else false)))
                    (if %loop-returned %loop-result (%loop)))))))
               (cond (i ((if (input-port? i) close-input-port close-output-port) i)) (else false))
               %with-open-file-res))))
           (cond
                ...
                ((ps-file
                  (let ((%type 'string) (%ee (list f ".ps")))
                   (let ((%res (if (eq? %type 'string) "" null)))
                    ...
                    %res))))
                (system
                 (let ((%type 'string) (%ee (list "dvips " dvi-file " -o " ps-file)))
                  (let ((%res (if (eq? %type 'string) "" null)))
                   (let %concatenate-loop ((%ee %ee))
                    (if (null? %ee) %res
                     (let ((%a (car %ee)))
...
(define (ps-to-img/gif/netpbm img-file)
 (system
  (let ((%type 'string) (%ee (list "ppmquant 256 < " img-file ".ppm.tmp > " img-file ".ppm")))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
    ...
    %res)))
 (system
  (let ((%type 'string) (%ee (list "ppmtogif -transparent rgb:ff/ff/ff < " img-file ".ppm > " img-file)))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
...
(define (ps-to-img/png/netpbm img-file)
 ...
 (system
  (let
   ((%type 'string)
    (%ee (list "pnmtopng -interlace -transparent \"#FFFFFF\" " " < " img-file ".ppm.tmp > " img-file)))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
 ...
 (system
  (let ((%type 'string) (%ee (list "ppmquant 256 < " img-file ".ppm.tmp > " img-file ".ppm")))
   (let ((%res (if (eq? %type 'string) "" null)))
    ...
    %res)))
 (system
  (let ((%type 'string) (%ee (list "ppmtojpeg --grayscale < " img-file ".ppm > " img-file)))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
...
(define (ps-to-img ps-file img-file)
 (case (tex2page-flag-value "\\TZPimageconverter")
  ((#\i #\I)
   (system
    (let ((%type 'string) (%ee (list "convert -transparent white -trim " ps-file " " img-file)))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
  ...
  (else
   (system
    (let
     ((%type 'string)
      (%ee (list *ghostscript* *ghostscript-options* " -sOutputFile=" img-file ".ppm.1 " ps-file)))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
      ...
      %res)))
   (system
    (let ((%type 'string) (%ee (list "pnmcrop " img-file ".ppm.1 > " img-file ".ppm.tmp")))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res)))
   (case (tex2page-flag-value "\\TZPimageformat") ((#\p #\P) (ps-to-img/png/netpbm img-file))
    ((#\j #\J) (ps-to-img/jpeg/netpbm img-file)) (else (ps-to-img/gif/netpbm img-file))))))
   ...
   ((fq-img-file
     (let ((%type 'string) (%ee (list *aux-dir/* img-file)))
      (let ((%res (if (eq? %type 'string) "" null)))
       ...
       %res))))
   (cond
    ((not (file-exists? fq-img-file)) (write-log ':separation-space) (write-log #\{)
     (write-log
      (let ((%type 'string) (%ee (list f ".tex")))
       (let ((%res (if (eq? %type 'string) "" null)))
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
          (let ((%a (car %ee)))
           ...
           (%concatenate-loop (cdr %ee)))))
        %res)))
     (write-log ':separation-space) (write-log "->") (write-log ':separation-space)
     (cond
...
(define (call-with-lazy-image-stream eps-file img-file-stem p)
 (let
  ...
  (if (file-exists? eps-file) (tex-to-img img-file-stem)
   (begin (set! *missing-eps-files* (cons (cons eps-file img-file-stem) *missing-eps-files*))
    *missing-eps-files*))))

(define (retry-lazy-image eps-file img-file-stem)
 (cond ((file-exists? eps-file) (tex-to-img img-file-stem))
  (else (write-log "! I can't find EPS file ") (write-log eps-file) (write-log ':separation-newline))))
...
(define (do-epsfig)
 (let ((%fluid-var-*imgpreamble-inferred* (cons ':epsfbox *imgpreamble-inferred*)))
  (fluid-let ((*imgpreamble-inferred* %fluid-var-*imgpreamble-inferred*))
   (call-with-html-image-stream (lambda (o) (display "\\epsfig{" o) (dump-groupoid o) (display #\} o))))))

(define (do-convertmptopdf)
 (let ((f (get-filename-possibly-braced)) (img-file-stem (next-html-image-file-stem))) (get-token)
  (get-token) (lazily-make-epsf-image-file f img-file-stem) (source-img-file img-file-stem)))

(define (do-includegraphics-web bracketed-text image-file) (emit "<img")
 (cond
  (bracketed-text
   (let ((height false) (width false)) (toss-back-string " enoughalready ")
    ...
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop ()
      (cond ((eat-word "height") (get-equal-sign) (set! height (get-pixels)) height)
       ((eat-word "width") (get-equal-sign) (set! width (get-pixels)) width)
       ((eat-word "enoughalready") (ignorespaces) (return)) (else (get-actual-char)))
      (if %loop-returned %loop-result (%loop))))
    (cond (height (emit " height=") (emit height)) (else false))
    (cond (width (emit " width=") (emit width)) (else false))))
  (else false))
 (emit " src=\"") (do-img-src (fully-qualify-url image-file)) (emit "\" ") (emit " alt=\"")
 (emit image-file) (emit "\"") (emit ">"))
    ...
    (let ((f (get-filename-possibly-braced)))
     (let
      ...
      (let ((ffull-ext (and ffull (file-extension ffull))))
       (cond
        ((and ffull-ext (member ffull-ext '(".jpg" ".jpeg" ".png"))) (do-includegraphics-web b1 ffull))
        (else
         (let
          ...
          (fluid-let ((*imgpreamble-inferred* %fluid-var-*imgpreamble-inferred*))
           (call-with-lazy-image-stream (or ffull f) img-file-stem
            (lambda (o) (display "\\includegraphics" o) (cond (starred-p (display #\* o)) (else false))
             (cond (b1 (display #\[ o) (display b1 o) (display #\] o)) (else false))
             (cond (b2 (display #\[ o) (display b2 o) (display #\] o)) (else false)) (display #\{ o)
    ...
    (let ((%fluid-var-*imgpreamble-inferred* (cons ':includegraphics *imgpreamble-inferred*)))
     (fluid-let ((*imgpreamble-inferred* %fluid-var-*imgpreamble-inferred*))
      (call-with-html-image-stream
       (lambda (o) (display "\\resizebox" o) (display arg1 o) (display arg2 o) (display arg3 o)))))))))

(define (do-mplibcode)
 (call-with-html-image-stream
  (lambda (o) (display "\\input luamplib.sty" o) (newline o) (display "\\mplibcode" o)
   (dump-till-end-env "mplibcode" o) (display "\\endmplibcode" o) (newline o))))
...
(define (do-mfpic-opengraphsfile) (set! *mfpic-file-stem* (get-filename-possibly-braced))
 ...
 (let
  ((f
    (let ((%type 'string) (%ee (list *mfpic-file-stem* *mfpic-tex-file-suffix*)))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
  ...
  (set! *mfpic-stream*
   (let*
    ...
    (cond
     ((eqv? %direction ':output)
      (when (and (eqv? %if-exists ':supersede) (file-exists? %f)) (delete-file %f)) (open-output-file %f))
     ((and (not %if-does-not-exist) (not (file-exists? %f))) false) (else (open-input-file %f)))))
  *mfpic-stream*)
 (set! *mfpic-file-num* 0) (display "\\input mfpic \\usemetapost " *mfpic-stream*) (newline *mfpic-stream*)
 (display "\\opengraphsfile{" *mfpic-stream*) (display *mfpic-file-stem* *mfpic-stream*)
 (display #\} *mfpic-stream*) (newline *mfpic-stream*)
...
(define (do-mfpic-closegraphsfile) (display "\\closegraphsfile" *mfpic-stream*) (newline *mfpic-stream*)
 (let ((%close-port-arg *mfpic-stream*))
  ((if (input-port? %close-port-arg) close-input-port close-output-port) %close-port-arg))
 (let
  ((tex-f
...
(define (do-mfpic) (display "\\mfpic" *mfpic-stream*) (dump-till-end-env "mfpic" *mfpic-stream*)
 ...
 (let
  ((f
    (let ((%type 'string) (%ee (list *mfpic-file-stem* "." (write-to-string *mfpic-file-num*))))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
      ...
      %res)))
   (img-file-stem (next-html-image-file-stem)))
  (lazily-make-epsf-image-file f img-file-stem) (source-img-file img-file-stem)))

(define (do-following-latex-env-as-image) (do-latex-env-as-image (ungroup (get-group)) ':display))
...
(define (do-hbox) (ignorespaces) (get-to) (eat-dimen) (ignorespaces)
 (let ((c (snoop-actual-char)))
  (case c ((#\{) (get-actual-char)) ((#\\) (get-ctl-seq))
   (else (get-actual-char) (toss-back-char #\}) (toss-back-char c))))
 (bgroup)
 (add-postlude-to-top-frame
  (let
   ((old-math-mode-p *math-mode-p*) (old-in-display-math-p *in-display-math-p*)
    (old-tabular-stack *tabular-stack*) (old-ligatures-p *ligatures-p*))
   (set! *math-mode-p* false) (set! *in-display-math-p* false) (set! *tabular-stack* null)
   (set! *ligatures-p* true)
   (lambda () (set! *math-mode-p* old-math-mode-p) (set! *in-display-math-p* old-in-display-math-p)
    (set! *tabular-stack* old-tabular-stack) (set! *ligatures-p* old-ligatures-p) *ligatures-p*))))
...
(define (do-tex-frac) (ignorespaces)
 (let
  ((inner-level-p (or (not *in-display-math-p*) (not (null? *tabular-stack*))))
   (%fluid-var-*tabular-stack* (cons ':frac *tabular-stack*)))
  (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*))
   (cond
    (inner-level-p (emit "<sup>") (tex2page-string (get-till-char #\/)) (emit "</sup>/<sub>")
     (get-actual-char) (ignorespaces) (tex2page-string (get-token)) (emit "</sub>"))
    (else (emit "</td><td><table class=leftline><tr><td class=centerline>")
     (tex2page-string (get-till-char #\/)) (get-actual-char) (ignorespaces) (emit "<hr noshade>")
     (tex2page-string (get-token)) (emit "</td></tr></table></td><td>"))))))

(define (do-eqno)
 (cond ((not *in-display-math-p*) (terror 'do-eqno "You can't use \\eqno in math mode")) (else false))
 (emit "</td><td width=10% class=rightline>"))

(define (do-eqalign type) (ignorespaces)
 (let ((c (get-actual-char))) (cond ((not c) (terror 'do-eqalign "Missing {")) (else false))
  (cond ((not (char=? c #\{)) (terror 'do-eqalign "Missing {")) (else false)) (bgroup)
  (set! *tabular-stack* (cons type *tabular-stack*))
  (add-postlude-to-top-frame
   (lambda () (emit "</td></tr>") (emit-newline) (emit "</table>") (emit-newline)
    (cond (*in-display-math-p* (emit "</td><td>")) (else false)) (pop-tabular-stack type)
    (set! *equation-position* 0) *equation-position*))
  (cond (*in-display-math-p* (emit "</td><td>")) (else false)) (emit-newline) (emit "<table><tr><td>")))

(define (do-noalign)
 (let ((type (car *tabular-stack*)))
  (let ((split-p (member type '(:eqalignno :displaylines))))
   (cond (split-p (egroup) (emit "</td></tr></table></div>") (emit-newline) (do-para)) (else false))
   (tex2page-string (get-group))
   (cond
    (split-p (do-end-para) (emit-newline) (emit "<div class=centerline><table><tr><td>")
     (toss-back-char #\{) (do-eqalign type))
    (else (emit "</td></tr>") (emit-newline) (emit "<tr><td>"))))))

(define (do-pmatrix) (ignorespaces)
 (let ((c (get-actual-char)))
  (cond ((or (not c) (not (char=? c #\{))) (terror 'do-pmatrix "Missing {")) (else false)) (bgroup)
  (set! *math-delim-left* ':lparen) (set! *math-delim-right* ':rparen) *math-delim-right*))
...
(define (eat-till-eol)
 (let*
  ((%loop-returned false) (%loop-result 0)
   (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
  (let %loop ()
   (let ((c (get-actual-char))) (cond ((or (not c) (char=? c #\newline)) (return)) (else false)))
   (if %loop-returned %loop-result (%loop)))))
...
(define (eat-till-char d)
 (let*
  ((%loop-returned false) (%loop-result 0)
   (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
  (let %loop () (let ((c (get-actual-char))) (cond ((or (not c) (char=? c d)) (return)) (else false)))
   (if %loop-returned %loop-result (%loop)))))

(define (do-comment) (eat-till-eol)
 (cond ((munched-a-newline-p) (toss-back-char #\newline) (toss-back-char #\newline)) (else false)))
...
(define (string=join ss sepc)
 (let ((res ""))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (cond ((null? ss) (return res)) (else false))
    (unless %loop-returned
     (let
      ((s
        (let* ((%pop-old-stack ss) (%pop-top-value (car %pop-old-stack)))
         (begin (set! ss (cdr %pop-old-stack)) ss) %pop-top-value)))
      (set! res
       (cond ((string=? res "") s)
        (else
 ...
 (let
  ...
  (let
   ...
   (if (not f) false
    (let ((f (string-trim f)))
     (cond
      ((= (let ((%length-arg f)) ((if (string? %length-arg) string-length length) %length-arg)) 0) false)
      ((file-exists? f) f) (else false)))))))
    ...
    (let ((%dolist-l *tex2page-inputs*) (dir false))
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (if (null? %dolist-l) (return) false)
       (unless %loop-returned (set! dir (car %dolist-l)))
       (unless %loop-returned (set! %dolist-l (cdr %dolist-l)))
       (unless %loop-returned
        (let
         ((f
           (let ((%type 'string) (%ee (list dir *directory-separator* file.tex)))
            (let ((%res (if (eq? %type 'string) "" null)))
             (let %concatenate-loop ((%ee %ee))
              (if (null? %ee) %res
               (let ((%a (car %ee)))
       ...
       (unless %loop-returned
        (let
         ((f
           (let ((%type 'string) (%ee (list dir *directory-separator* file)))
            (let ((%res (if (eq? %type 'string) "" null)))
             (let %concatenate-loop ((%ee %ee))
              (if (null? %ee) %res
               (let ((%a (car %ee)))
...
(define (initialize-scm-words) (set! *scm-keywords* (make-table ':test equal?))
 (set! *scm-builtins* (make-table ':test equal?)) (set! *scm-special-symbols* (make-table ':test equal?))
 (set! *scm-variables* (make-table ':test equal?))
 (for-each (lambda (s) (table-put! s *scm-keywords* true) (table-get s *scm-keywords*))
  '("=>" "and" "begin" "begin0" "case" "cond" "define" "define-macro" "define-syntax" "defmacro"
    "defstruct" "delay" "do" "else" "flet" "fluid-let" "if" "labels" "lambda" "let" "let*" "let-syntax"
    "let-values" "letrec" "letrec-syntax" "macrolet" "or" "quasiquote" "quote" "set!" "syntax-case"
    "syntax-rules" "unless" "unquote" "unquote-splicing" "when" "with" "with-handlers" "assert" "block"
    "decf" "defpackage" "defparameter" "defun" "defvar" "destructuring-bind" "do-all-symbols"
    "do-external-symbols" "do-symbols" "dolist" "dotimes" "ecase" "etypecase" "eval-when" "handler-bind"
    "handler-case" "incf" "loop" "multiple-value-bind" "multiple-value-setq" "pop" "prog1" "progn" "push"
    "setf" "setq" "typecase" "unwind-protect" "with-input-from-string" "with-open-file" "with-open-socket"
    "with-open-stream" "with-output-to-string" "with-slots"))
 true)

(define (actual-tex-filename f . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (check-timestamp-p false))
  (when (< 0 %lambda-rest-arg-len) (set! check-timestamp-p (list-ref %lambda-rest-arg 0)))
  (let ((doing-main-file-p (not *main-tex-file*)) (f (and f (find-tex-file f))))
   (cond
    (doing-main-file-p
     (cond
               ...
               (let %concatenate-loop ((%ee %ee))
                (if (null? %ee) %res
                 (let ((%a (car %ee)))
                  ...
                  (%concatenate-loop (cdr %ee)))))
               %res))))
           (write-log ':separation-newline) (write-log "Copying weirdly named TeX source file ")
           (write-log f) (write-log " to ") (write-log f-save) (write-log ':separation-newline)
           (system
   ...
   (cond
    ((and f check-timestamp-p)
     (cond ((not (member f *verb-written-files*)) (update-last-modification-time f)) (else false)))
    (else false))
   f)))
...
(define (ignore-tex-specific-text env)
 (let
  ...
  (let*
   ...
   (let %loop ()
    (let ((c (snoop-actual-char)))
     (cond ((not c) (terror 'ignore-tex-specific-text "Missing \\end" env)) (else false))
     (cond
      ((= (catcode c) **escape**)
       (let ((x (get-ctl-seq)))
        (cond ((string=? x endenv) (return))
         ((string=? x "\\end")
          (let ((g (get-grouped-environment-name-if-any)))
           (cond ((and g (string=? g env)) (return)) (else false)))))))
      (else (get-actual-char))))
    (if %loop-returned %loop-result (%loop))))))
...
(define (do-htmlheadonly) (cond ((null? *html-head*) (flag-missing-piece ':html-head)) (else false))
 (let ((s null) (s2 false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
          ...
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
             ...
             (%concatenate-loop (cdr %ee)))))
          %res)))
       (write-aux `(!html-head ,s2)) (set! s null)
       (let ((x (get-ctl-seq)))
        (cond ((string=? x "\\endhtmlheadonly") (return))
         ((string=? x "\\input")
          (let ((f (get-filename-possibly-braced))) (call-with-input-file/buffered f do-htmlheadonly)))
...
(define (resolve-defs x)
 (cond
  ((begin (set! *it* (find-def x)) *it*)
   (let ((y *it*))
    (cond ((begin (set! *it* (tdef*-defer y)) *it*) *it*) ((tdef*-thunk y) false)
     ((and (null? (tdef*-argpat y)) (not (tdef*-optarg y)) (begin (set! *it* (tdef*-expansion y)) *it*))
      *it*)
     ((and (inside-false-world-p) (not (if-aware-ctl-seq-p x))) false)
     (else (cond (*outer-p* (set! *outer-p* false) (toss-back-char *invisible-space*)) (else false))
...
(define (do-expandafter)
 (let ((first (get-raw-token/is)))
  (let ((second (get-raw-token/is))) (toss-back-char *invisible-space*)
   (cond ((ctl-seq-p second) (toss-back-string (expand-ctl-seq-into-string second)))
...
(define (resolve-expandafters)
 (let ((c (snoop-actual-char)))
  (cond
   ((= (catcode c) **escape**)
    (let ((x (get-ctl-seq)))
     (if (string=? x "\\expandafter") (do-expandafter)
...
(define (do-futurelet)
 (let ((first (get-raw-token/is)))
  (let ((second (get-raw-token/is)))
   (let ((third (get-raw-token))) (do-futurelet-aux first second third)))))

(define (do-futurelet-aux first second third) (tex-let-general first third false)
 (toss-back-char *invisible-space*) (toss-back-string third) (toss-back-char *invisible-space*)
 (toss-back-string second))

(define (set-start-time)
 (apply
  (lambda (s m h d mo y . %mvb-rest-arg) (plain-count "\\time" (+ (* 60 h) m)) (plain-count "\\day" d)
   (plain-count "\\month" mo) (plain-count "\\year" y))
  (decode-universal-time *start-time*)))

(define (initialize-globals) (tex-def-countdef "\\pageno" 0 0 true) (tex-def-count 10 21 true)
 (tex-def-count 11 9 true) (tex-def-count 14 9 true) (tex-def-count 15 9 true) (tex-def-count 16 -1 true)
 (tex-def-count 17 -1 true) (set-start-time) (plain-count "\\language" 256)
 (plain-count "\\secnumdepth" -2) (plain-count "\\tocdepth" -2) (plain-count "\\footnotenumber" 0)
 (plain-count "\\TIIPtabularborder" 1) (plain-count "\\TIIPnestedtabularborder" 0)
 (plain-count "\\TIIPobeyspacestrictly" 0) (plain-count "\\TIIPobeylinestrictly" 0)
 (plain-count "\\TIIPsettabscolumns" 0) (plain-count "\\errorcontextlines" 5)
 (plain-count "\\doublehyphendemerits" 10000) (plain-count "\\finalhyphendemerits" 5000)
 (plain-count "\\hyphenpenalty" 50) (plain-count "\\exhyphenpenalty" 50) (plain-count "\\pretolerance" 100)
 ...
 (plain-dimen "\\maxdepth" (tex-length 4 ':pt)) (plain-dimen "\\delimitershortfall" (tex-length 5 ':pt))
 ...
 (plain-dimen "\\smallskipamount" (tex-length 3 ':pt)) (plain-dimen "\\medskipamount" (tex-length 6 ':pt))
 (plain-dimen "\\bigskipamount" (tex-length 12 ':pt)) (plain-dimen "\\lastskip" 0)
 (plain-dimen "\\baselineskip" (tex-length 12 ':pt)) (plain-dimen "\\overfullrule" (tex-length 5 ':pt))
 ...
 (plain-toks "\\headline" "") (plain-toks "\\footline" "\\folio") (tex-def-dotted-count "figure" false)
 (tex-def-dotted-count "table" false) (tex-def-dotted-count "equation" false)
 (tex-gdef-0arg "\\TIIPcurrentnodename" "no value yet") (tex-gdef-0arg "\\@currentlabel" "no value yet")
 (tex-gdef-0arg "\\TZPcolophontimestamp" "1") (tex-gdef-0arg "\\TZPcolophoncredit" "1")
 (tex-gdef-0arg "\\TZPcolophonweblink" "1") (tex-gdef-0arg "\\TZPimageformat" "PNG")
 (tex-gdef-0arg "\\TZPimageconverter" "NetPBM") (tex-gdef-0arg "\\TZPredirectseconds" "0")
 (tex-gdef-0arg "\\TZPtextext" "1") (tex-gdef-0arg "\\TZPraggedright" "1") (tex-gdef-0arg "\\TZPlang" "en")
 (tex-gdef-0arg "\\TZPcommonlisp" (if null "0" "1")) (initialize-scm-words))

(define (find-def ctlseq)
 (or (ormap (lambda (fr) (table-get ctlseq (texframe*-definitions fr))) *tex-env*)
  (and *global-texframe* (table-get ctlseq (texframe*-definitions *global-texframe*)))
...
(define (do-magstep)
 (case (string->number (get-token-or-peeled-group)) ((1) "1000") ((2) "1200") ((3) "1440") ((4) "1728")
  ((5) "2074") ((6) "2488") (else "")))
...
(define (expand-the)
 (let ((ctlseq (get-ctl-seq)))
  (cond ((begin (set! *it* (find-dimendef ctlseq)) *it*) (scaled-point-to-tex-point (find-dimen *it*)))
...
(define (find-corresp-prim ctlseq) (let ((y (find-def ctlseq))) (or (and y (tdef*-defer y)) ctlseq)))

(define (find-corresp-prim-thunk ctlseq)
 (let ((y (find-def ctlseq))) (if (and y (tdef*-thunk y)) (tdef*-prim y) ctlseq)))
        ...
        ((frame
          (cond
           (globalp
            (for-each
             (lambda (fr)
              (table-rem lhs (if (ctl-seq-p lhs) (texframe*-definitions fr) (texframe*-cdefinitions fr))))
             *tex-env*)
            *global-texframe*)
           (else false))))
   ...
   (let
    ((argc
      (cond
       ((begin (set! *it* (get-bracketed-text-if-any)) *it*)
        (let ((s *it*))
         (cond ((begin (set! *it* (get-bracketed-text-if-any)) *it*) (set! optarg *it*) optarg))
    ...
    (let ((rhs (ungroup (get-token))))
     (let ((ok-to-def-p (or renewp (not (find-def lhs)))))
      ...
      (cond ((not ok-to-def-p) (trace-if (> (the-count "\\tracingcommands") 0) lhs " already defined"))
       (else false))))))))

(define (do-newcounter)
 (let ((counter-name (ungroup (get-group))))
  (let ((within (get-bracketed-text-if-any))) (tex-def-dotted-count counter-name within))))
  ...
  (let
   ...
   (let ((optarg false))
    (let
     ...
     (let
      ((beginning
        (let ((%type 'string) (%ee (list "\\begingroup " (ungroup (get-token)))))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
      ...
      (let
       ((ending
         (let ((%type 'string) (%ee (list (ungroup (get-token)) "\\endgroup")))
          (let ((%res (if (eq? %type 'string) "" null)))
           (let %concatenate-loop ((%ee %ee))
            (if (null? %ee) %res
             (let ((%a (car %ee)))
       ...
       (let ((ok-to-def-p (or renewp (not (find-def bs-envname)))))
        (tex-def bs-envname (latex-argnum-to-plain-argpat argc) beginning optarg false false false false)
        (tex-def
        ...
        (cond ((not ok-to-def-p) (trace-if true "{" envname "} already defined")) (else false))))))))))
             ...
             (let %concatenate-loop ((%ee %ee))
              (if (null? %ee) %res
               (let ((%a (car %ee)))
                ...
                (%concatenate-loop (cdr %ee)))))
             %res))))))
       (cond ((not numbered-like) (tex-def-dotted-count counter-name sec-num)) (else false))
       (tex-def
        (let ((%type 'string) (%ee (list "\\" env)))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
          ...
          %res))
        null
        (let ((%type 'string) (%ee (list "\\par\\begingroup\\TIIPtheorem{" counter-name "}{" caption "}")))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
          ...
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
             ...
             (%concatenate-loop (cdr %ee)))))
          %res))
        null "\\endgroup\\par" false false false false *global-texframe*))))))))

(define (do-theorem)
 (let ((counter-name (ungroup (get-group))))
  (let ((counter (table-get counter-name *dotted-counters*)))
   (let ((caption (ungroup (get-group)))) (cond ((not counter) (terror 'do-theorem)) (else false))
    (let ((new-counter-value (+ 1 (counter*-value counter))))
     (set!counter*-value counter new-counter-value) (counter*-value counter)
     (let
      ((thm-num
        (let ((sec-num (counter*-within counter)))
         (if sec-num
          (let
           ((%type 'string)
            (%ee (list (section-counter-value sec-num) "." (write-to-string new-counter-value))))
           (let ((%res (if (eq? %type 'string) "" null)))
            (let %concatenate-loop ((%ee %ee))
             (if (null? %ee) %res
              (let ((%a (car %ee)))
      ...
      (let
       ((lbl
         (let ((%type 'string) (%ee (list *html-node-prefix* "thm_" thm-num)))
          (let ((%res (if (eq? %type 'string) "" null)))
           (let %concatenate-loop ((%ee %ee))
            (if (null? %ee) %res
             (let ((%a (car %ee)))
              ...
              (%concatenate-loop (cdr %ee)))))
           %res))))
       (tex-def-0arg "\\TIIPcurrentnodename" lbl) (tex-def-0arg "\\@currentlabel" thm-num)
       (emit-anchor lbl) (emit-newline) (emit "<b>") (emit caption) (emit " ") (emit thm-num)
       (emit ".</b>") (emit-nbsp 2))))))))
...
(define (do-begin)
 (cond
  ...
  (else false))
 (let ((env *it*)) (toss-back-char *invisible-space*)
  (toss-back-string
   (let ((%type 'string) (%ee (list "\\" env)))
    (let ((%res (if (eq? %type 'string) "" null)))
     (let %concatenate-loop ((%ee %ee))
  ...
  (cond
   ((not
     (member env
      '("htmlonly" "cssblock" "document" "latexonly" "rawhtml" "texonly" "verbatim" "verbatim*")))
...
(define (do-end)
 (cond
  ((begin (set! *it* (get-grouped-environment-name-if-any)) *it*)
   (let ((env *it*)) (toss-back-char *invisible-space*)
    (cond ((not (member env '("htmlonly" "document"))) (do-end-para) (toss-back-string "\\endgroup"))
     (else false))
    (toss-back-string
     (let ((%type 'string) (%ee (list "\\end" env)))
      (let ((%res (if (eq? %type 'string) "" null)))
...
(define (latex-argnum-to-plain-argpat n)
 (let ((n n) (s null))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (cond ((<= n 0) (return s)) (else false)) (unless %loop-returned (set! n (- n 1)) n)
   ...
   ((lhs (get-ctl-seq))
    (imgdef-file-stem
     (let
      ((%type 'string)
       (%ee
        (list *subjobname* *img-file-suffix* *imgdef-file-suffix* (write-to-string *imgdef-file-count*))))
      (let ((%res (if (eq? %type 'string) "" null)))
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
         (let ((%a (car %ee)))
       ...
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
         (let ((%a (car %ee)))
          ...
          (%concatenate-loop (cdr %ee)))))
       %res))))
   (write-log #\() (write-log img-file) (write-log ':separation-space) (valid-img-file-p img-file)
   (emit "<img src=\"") (do-img-src (ensure-url-reachable img-file)) (emit "\"")
   (emit " style=\"border: 0\"") (emit " alt=\"")
   (cond (alt (emit alt)) (else (emit "[") (emit img-file) (emit "]"))) (emit "\">") (write-log #\))
   (write-log ':separation-space) true)))
    ...
    (cond
     ...
     (else
      (cond ((char=? c #\newline) (get-actual-char) (ignorespaces))
       ((char-whitespace? c) (ignorespaces) (set! c #\space) c) (else (get-actual-char)))
     ...
     (reverse
      (let ((s null) (nesting 0) (escape-p false))
       (let*
        ...
        (let %loop ()
         (let ((c (snoop-actual-char)))
          (cond ((not c) (terror 'get-till-char "File ended too soon")) (else false))
          (cond (escape-p (set! s (cons (get-actual-char) s)) (set! escape-p false) escape-p)
           ((char=? c c0) (return s))
           ((= (catcode c) **escape**) (set! s (cons (get-actual-char) s)) (set! escape-p true) escape-p)
           ((char=? c #\{) (set! s (cons (get-actual-char) s)) (set! nesting (+ nesting 1)) nesting)
           ((char=? c #\}) (set! s (cons (get-actual-char) s)) (set! nesting (- nesting 1)) nesting)
           ((> nesting 0) (set! s (cons (get-actual-char) s)) s)
           ((and (char-whitespace? c) (not (char=? c0 #\newline)) (char-whitespace? c0)) (return s))
...
(define (do-halign) (do-end-para) (ignorespaces)
 (let ((c (get-actual-char)))
  (cond ((or (not c) (not (char=? c #\{))) (terror 'do-halign "Missing {")) (else false)))
 (let ((%fluid-var-*tabular-stack* (cons ':halign *tabular-stack*)))
  (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*)) (bgroup) (emit "<table>")
   (let ((tmplt (get-halign-template)))
    (let*
     ...
     (let %loop () (ignorespaces)
      (let ((c (snoop-actual-char)))
       (cond ((not c) (terror 'do-halign "Eof inside \\halign")) (else false))
       (cond
        ((char=? c #\}) (get-actual-char) (emit "</table>") (emit-newline) (egroup) (do-para) (return))
...
(define (get-halign-template)
 (let ((s null))
  (let*
   ...
   (let %loop ()
    (let ((x (get-raw-token)))
     (cond ((not x) (terror 'get-halign-template "Eof in \\halign")) (else false))
     (cond ((string=? x "\\cr") (set! s (cons false s)) (return (reverse s)))
      ((string=? x "#") (set! s (cons true s)) s) ((string=? x "&") (set! s (cons false s)) s)
 ...
 (let ((tmplt tmplt) (ins " ") (outer-loop-done false))
  (let*
   ...
   (let %loop () (cond (outer-loop-done (return)) (else false))
    (unless %loop-returned
     (let ((x (get-raw-token)))
      (cond ((not x) (terror 'expand-halign-line "Eof in \\halign")) (else false))
      (cond
        ...
        (let ((r "{"))
         (let*
          ((%loop-returned false) (%loop-result 0)
           (return
            (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
          (let %loop () (cond ((null? tmplt) (terror 'expand-halign-line "Eof in \\halign")) (else false))
           (let ((y (car tmplt)))
            (case y
             ((()) (emit "<td>")
              (tex2page-string
                 ...
                 (let %concatenate-loop ((%ee %ee))
                  (if (null? %ee) %res
                   (let ((%a (car %ee)))
                    ...
                    (%concatenate-loop (cdr %ee)))))
                 %res)))
              (cond ((and (string=? x "\\cr") (string=? ins " ")) (emit-nbsp 1)) (else false))
              (emit "</td>")
              (if (string=? x "\\cr")
               (begin (emit "</tr>") (emit-newline) (begin (set! outer-loop-done true) outer-loop-done)
                (return))
               (begin
                (let* ((%pop-old-stack tmplt) (%pop-top-value (car %pop-old-stack)))
                 (begin (set! tmplt (cdr %pop-old-stack)) tmplt) %pop-top-value)
 ...
 (let ((settabs-spec ""))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop ()
    (let ((x (get-token/ps))) (cond ((not x) (terror 'do-settabs "Eof in \\settabs")) (else false))
     (cond
      ((string=? x "\\columns")
       (tex2page-string
        (let ((%type 'string) (%ee (list "\\TIIPsettabscolumns=" settabs-spec)))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
...
(define (do-tabalign) (emit-newline)
 (let ((table-width "") (num-cols (the-count "\\TIIPsettabscolumns")))
  ...
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (do-tabalign-row num-cols)
    (let ((x (get-token/ps)))
     (cond ((not x) (return)) ((or (string=? x "\\+") (string=? x "\\tabalign")) true)
...
(define (do-tabalign-row num-cols) (emit "<tr>") (emit-newline)
 (let ((cell-contents "") (cell-width ""))
  (cond
   ((> num-cols 0)
    (set! cell-width
     (let ((%type 'string) (%ee (list " width=" (write-to-string (/ 100.0 num-cols)) "%")))
      (let ((%res (if (eq? %type 'string) "" null)))
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
         (let ((%a (car %ee)))
  ...
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop ()
    (let ((x (get-token/ps))) (cond ((not x) (terror 'do-tablign "Eof in \\tabalign")) (else false))
     (cond
      ((or (string=? x "&") (string=? x "\\cr")) (emit "<td") (emit cell-width) (emit ">") (bgroup)
       (tex2page-string cell-contents) (egroup) (set! cell-contents "") (emit "</td>") (emit-newline)
...
(define (read-till-next-sharp k argpat) (ignorespaces)
 (let ((n (length argpat)) (ss null) (i false) (s false) (c false) (outer-loop-done false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop ()
    ...
    (unless %loop-returned
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (set! c (if (< i n) (list-ref argpat i) #\#))
       (cond ((char=? c #\#) (set! outer-loop-done true) (return)) (else false))
       (unless %loop-returned
        (let ((d (snoop-actual-char)))
         (cond
          ((and (char=? c #\space) (char-whitespace? d)) (ignorespaces) (set! i (+ i 1))
           (set! s (cons c s)) s)
          ((= (catcode d) **comment**) (do-comment))
          ((and (char=? c #\newline) (char-whitespace? d)
            (or (munched-a-newline-p) (begin (toss-back-char d) false)))
           (set! i (+ i 1)) (set! s (cons c s)) s)
          ((char=? c d) (get-actual-char) (set! i (+ i 1)) (set! s (cons c s)) s)
          ((= i k)
           (set! ss
            (if (and (char=? d #\{) (or (null? ss) (not (= (catcode (car ss)) **escape**))))
             (append (get-group-as-reversed-chars) ss)
             (begin (get-actual-char)
              (cond ((and (char-whitespace? d) (not (char=? d #\newline))) (ignorespaces)) (else false))
              (cons d ss))))
           (return))
    ...
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop ()
      (cond ((>= k n) (cond ((= k 0) (ignorespaces ':stop-after-first-newline)) (else false)) (return r))
       (else false))
      (unless %loop-returned
       (let ((c (list-ref argpat k)))
        (cond
         ((= (catcode c) **parameter**)
          (cond ((= k (sub1 n)) (ignorespaces) (set! r (cons (get-till-char #\{) r)) (return r))
           ((= k (- n 2)) (set! r (cons (ungroup (get-token)) r)) (return r))
           (else
            (let ((c2 (list-ref argpat (+ k 2))))
             (if (= (catcode c2) **parameter**)
              (begin (begin (set! k (+ k 2)) k) (begin (set! r (cons (ungroup (get-token)) r)) r))
              (apply (lambda (k2 s . %mvb-rest-arg) (set! k k2) (set! r (cons s r)) r)
               (read-till-next-sharp (+ k 2) argpat)))))))
         (else
          (let ((d (get-actual-char)))
           (cond ((not d) (terror 'read-macro-args "Eof before macro got enough args")) (else false))
           (cond
            ((char=? c #\space)
             (cond
              ((not (char-whitespace? d))
               (terror 'read-macro-args "Use of macro doesn't match its definition."))
              (else false)))
            ((char=? c d) true)
            (else (terror 'read-macro-args "Use of macro doesn't match its definition.")))
           (set! k (+ k 1)) k)))))
      (if %loop-returned %loop-result (%loop))))))))
       ...
       (let %loop ()
        (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
         (display
          (cond
           ((= (catcode c) **escape**)
            (let ((x (get-ctl-seq))) (toss-back-char *invisible-space*)
             (cond
              ((or (string=? x "\\the") (string=? x "\\number"))
               (let ((x2 (get-raw-token/is))) (toss-back-char *invisible-space*) (toss-back-string x2)
                (cond
                 ((ctl-seq-p x2)
                  (cond ((string=? x "\\the") (expand-the)) ((string=? x "\\number") (get-number))
                   (else 3735929054)))
                 (else x))))
              ((string=? x "\\noexpand")
               (let ((x2 (get-raw-token/is))) (toss-back-char *invisible-space*) x2))
              ((begin (set! *it* (find-def x)) *it*)
               (let ((y *it*))
                (cond
                 ((and (null? (tdef*-argpat y)) (not (tdef*-optarg y)) (not (tdef*-thunk y))
                   (not (tdef*-prim y)) (not (tdef*-defer y)))
                  (toss-back-char *invisible-space*) (toss-back-string (tdef*-expansion y)) "")
                 (else x))))
              (else x))))
   ...
   ((r
     (if (not optarg) null
      (begin (begin (set! k 2) k)
       (list (cond ((begin (set! *it* (get-bracketed-text-if-any)) *it*) *it*) (else optarg)))))))
   (let ((args (read-macro-args argpat k r)))
    (let ((rhs-n (let ((%length-arg rhs)) ((if (string? %length-arg) string-length length) %length-arg))))
                    ...
                    (let*
                     ((%loop-returned false) (%loop-result 0)
                      (return
                       (lambda %args (set! %loop-returned true)
                        (set! %loop-result (and (pair? %args) (car %args))))))
                     (let %loop () (cond ((>= j rhs-n) (return (reverse s))) (else false))
                      (unless %loop-returned
                       (let ((c (string-ref rhs j)))
                        (cond ((char-alphabetic? c) (set! j (+ j 1)) (set! s (cons c s)) s)
                         ((and (char=? c #\#) (> (length s) 1))
                          (return (append (reverse s) (cons #\space (aux j)))))
                         ((= (length s) 1) (return (append (reverse (cons c s)) (aux (add1 j)))))
                         (else (return (append (reverse s) (aux j)))))))
                      (if %loop-returned %loop-result (%loop))))))
                  ((char=? c #\#)
                   (if (= k (sub1 rhs-n)) (list #\#)
                    (let ((n (string-ref rhs (add1 k))))
                     (cond ((char=? n #\#) (cons #\# (aux (+ k 2))))
                      ((and (char-numeric? n) (<= (digit-to-int n) (length args)))
                       (append
                        (let ((%type 'list) (%ee (list (list-ref args (sub1 (digit-to-int n))))))
                         (let ((%res (if (eq? %type 'string) "" null)))
                          (let %concatenate-loop ((%ee %ee))
                           (if (null? %ee) %res
                            (let ((%a (car %ee)))
...
(define (do-verbatimescapechar) (ignorespaces)
 (let ((c1 (get-actual-char)))
  (let ((c2 (get-actual-char)))
   (cond ((not (= (catcode c1) **escape**)) (terror 'do-verbatimescapechar "Arg must be \\<char>"))
    (else false))
   (set! *esc-char-verb* c2) *esc-char-verb*)))

(define (do-verb-braced ign)
 (let ((%fluid-var-*catcodes* *catcodes*) (nesting 0))
  (fluid-let ((*catcodes* %fluid-var-*catcodes*)) (catcode #\\ 12) (catcode *esc-char-verb* 0)
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop ()
     (let ((c (get-actual-char)))
      (cond ((not c) (terror 'do-verb-braced "Eof inside verbatim")) (else false))
      (cond
       ((= (catcode c) **escape**) (toss-back-char c)
        (let
         ((x
           (let ((%fluid-var-*not-processing-p* true))
            (fluid-let ((*not-processing-p* %fluid-var-*not-processing-p*)) (get-ctl-seq)))))
       ...
       ((char=? c #\}) (cond ((= nesting 0) (return)) (else false)) (emit #\}) (set! nesting (- nesting 1))
        nesting)
       ((char=? c #\space) (emit (if *verb-visible-space-p* *verbatim-visible-space* #\space)))
       ((char=? c #\newline)
        (cond (*verb-display-p* (emit "&#xa0;") (emit-newline))
         (*verb-visible-space-p* (emit *verbatim-visible-space*)) (else (emit-newline))))
       ((and (char=? c #\-) (not *verb-display-p*)) (emit "&#x2011;")) (else (emit-html-char c))))
     (if %loop-returned %loop-result (%loop)))))))
...
(define (do-verb-delimed d)
 (let*
  ...
  (let %loop ()
   (let ((c (get-actual-char)))
    (cond ((not c) (terror 'do-verb-delimed "Eof inside verbatim")) (else false))
     ...
     (let ((d (get-actual-char)))
      (let ((%fluid-var-*verb-display-p* (munched-a-newline-p)))
       (fluid-let ((*verb-display-p* %fluid-var-*verb-display-p*))
        ...
        (cond (*outputting-external-title-p* false) (*verb-display-p* (emit "</pre>") (do-noindent))
         (else (emit "</code>"))))))))))
 (egroup))

(define (do-verbc) (ignorespaces) (bgroup)
 (let ((%fluid-var-*ligatures-p* false))
  (fluid-let ((*ligatures-p* %fluid-var-*ligatures-p*)) (emit "<code class=verbatim>")
...
(define (do-verbatiminput) (ignorespaces)
 (let ((f0 (get-filename-possibly-braced)))
  (let ((f (find-tex-file f0)))
   (cond
    ((and f (file-exists? f)) (do-end-para) (bgroup) (emit "<pre class=verbatim>")
            ...
            ((c
              (let*
               ((%read-char-port p)
                (%read-char-res (if %read-char-port (read-char %read-char-port) (read-char))))
               (when (eof-object? %read-char-res) (set! %read-char-res false)) %read-char-res)))
            (cond ((not c) (return)) (else false)) (emit-html-char c))
           (if %loop-returned %loop-result (%loop))))))
       (cond (p ((if (input-port? p) close-input-port close-output-port) p)) (else false))
       %with-open-file-res))
     (emit "</pre>") (egroup) (do-para))
    (else (non-fatal-error "File " f0 " not found"))))))

(define (get-char-definitely c0) (ignorespaces)
 (let ((c (get-actual-char))) (cond ((not c) (terror 'get-char-defnitely "Runaway argument")) (else false))
  (cond ((not (char=? c c0)) (terror 'get-char-defnitely "Missing" c0)) (else false))))

(define (get-char-optionally cc) (ignorespaces)
 (let ((c (snoop-actual-char))) (cond ((not c) false) ((member c cc) (get-actual-char) c) (else false))))

(define (get-unsigned-number-optionally) (ignorespaces)
 (let ((c (snoop-actual-char))) (cond ((not c) false) ((char-numeric? c) (get-integer 10)) (else false))))
...
(define (opmac-verbinput-skip-lines i n)
 (let ((%dotimes-n n) (_ 0))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (if (>= _ %dotimes-n) (return) false)
    (unless %loop-returned
     (let
      ((x
        (let ((%read-line-res (read-line i)))
         (when (eof-object? %read-line-res) (set! %read-line-res false)) %read-line-res)))
      (cond ((not x) (terror 'do-opmac-verbinput "\\verbinput file ended too soon")) (else false))))
    (unless %loop-returned (set! _ (+ _ 1))) (if %loop-returned %loop-result (%loop))))))
   ...
   (let %loop ()
    (let
     ((x
       (let ((%read-line-res (read-line i)))
        (when (eof-object? %read-line-res) (set! %read-line-res false)) %read-line-res)))
     (cond ((not x) (return)) (else false)) (emit-html-string x) (emit-newline))
    (if %loop-returned %loop-result (%loop))))
  (let ((%dotimes-n n) (_ 0))
   (let*
    ((%loop-returned false) (%loop-result 0)
        ...
        ((n
          (and f
           (let ((n (table-get f *opmac-verbinput-table*)))
            (or n
             (begin (begin (table-put! f *opmac-verbinput-table* 0) (table-get f *opmac-verbinput-table*))
              0))))))
        (cond
         ((and f (file-exists? f)) (do-end-para) (bgroup) (emit "<pre class=verbatim>")
          (let ((i (open-input-file f)))
           (let
            ((%with-open-file-res
              (cond
               ((and s1 n1 s2 n2 (char=? s1 #\-) (char=? s2 #\+)) (opmac-verbinput-skip-lines i (+ n n1))
                (opmac-verbinput-print-lines i n2) (set! n (+ n (+ n1 n2))) n)
               ((and (not s1) n1 s2 n2 (char=? s2 #\-)) (opmac-verbinput-skip-lines i (sub1 n1))
                (opmac-verbinput-print-lines i (add1 (- n2 n1))) (set! n n2) n)
               ((and (not s1) n1 s2 n2 (char=? s2 #\+)) (opmac-verbinput-skip-lines i (sub1 n1))
                (opmac-verbinput-print-lines i n2) (set! n (+ (sub1 n1) n2)) n)
               ((and s1 n1 (not s2) (not n2) (char=? s1 #\-)) (opmac-verbinput-print-lines i n1)
                (set! n n1) n)
               ((and s1 n1 (not s2) (not n2) (char=? s1 #\+)) (opmac-verbinput-skip-lines i n)
                (opmac-verbinput-print-lines i n1) (set! n1 (+ n1 1)) n1)
               ((and (not s1) n1 s2 (not n2) (char=? s2 #\-)) (opmac-verbinput-skip-lines i (sub1 n1))
                (opmac-verbinput-print-lines i true) (set! n 0) n)
               ((and s1 (not n1) (not s2) (not n2) (char=? s1 #\+)) (opmac-verbinput-skip-lines i n)
                (opmac-verbinput-print-lines i true) (set! n 0) n)
               ((and s1 (not n1) (not s2) (not n2) (char=? s1 #\-)) (opmac-verbinput-print-lines i true)
                (set! n 0) n)
               (else (terror 'do-opmac-verbinput "Malformed \\verbinput" s1 n1 s2 n2 f)))))
            (cond (i ((if (input-port? i) close-input-port close-output-port) i)) (else false))
            %with-open-file-res))
          (table-put! f *opmac-verbinput-table* n) (table-get f *opmac-verbinput-table*) (emit "</pre>")
          (egroup) (do-para))
         (else (non-fatal-error "File " f0 " not found")))))))))))
...
(define (dump-groupoid p) (ignorespaces)
 (let ((write-char write-char) (d (get-actual-char)))
  (cond ((not p) (set! write-char (lambda (x y) false)) write-char) (else false))
  (case d
   ((#\{)
     ...
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (set! c (get-actual-char))
       (cond ((not c) (terror 'dump-groupoid "Eof inside verbatim")) (else false))
       (cond ((= (catcode c) **escape**) (write-char c p) (write-char (get-actual-char) p))
        ((char=? c #\{) (write-char c p) (set! nesting (+ nesting 1)) nesting)
        ((char=? c #\})
         (cond ((= nesting 0) (return)) (else (write-char c p) (set! nesting (- nesting 1)) nesting)))
        (else (write-char c p)))
       (if %loop-returned %loop-result (%loop))))))
   (else
    (let*
     ...
     (let %loop ()
      (let ((c (get-actual-char)))
       (cond ((not c) (terror 'dump-groupoid "Eof inside verbatim")) (else false))
       (cond ((char=? c d) (return)) (else false)) (write-char c p))
      (if %loop-returned %loop-result (%loop))))))))
...
(define (do-string)
 (let ((c (snoop-actual-char)))
  (cond ((not c) false)
   ((= (catcode c) **escape**) (get-actual-char) (toss-back-char *invisible-space*)
    (toss-back-string "\\TIIPbackslash"))
   ((= (catcode c) **comment**) (eat-till-eol) (do-string)) (else (toss-back-char (get-actual-char))))))

(define (do-verbatim-latex env) (do-end-para) (bgroup) (emit "<pre class=verbatim>")
 (let ((%fluid-var-*verb-visible-space-p* (eat-star)))
  (fluid-let ((*verb-visible-space-p* %fluid-var-*verb-visible-space-p*))
   (cond
...
(define (do-scm-set-specialsymbol)
 (let ((sym (get-peeled-group)))
  (let ((xln (get-group))) (table-put! sym *scm-special-symbols* xln)
  ...
  (lambda ()
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (ignorespaces ':all) (cond ((not (snoop-actual-char)) (return)) (else false))
     (unless %loop-returned (table-put! (scm-get-token) *scm-special-symbols* false)
...
(define (scm-output-next-chunk)
 (let ((c (snoop-actual-char)))
  (cond ((and *slatex-math-escape* (char=? c *slatex-math-escape*)) (scm-escape-into-math))
   ((char=? c #\;) (scm-output-comment) (do-end-para)) ((char=? c #\") (scm-output-string))
   ((char=? c #\#) (scm-output-hash))
   ((char=? c #\,) (get-actual-char) (emit "<span class=keyword>") (scm-emit-html-char c)
    (let ((c (snoop-actual-char)))
     (cond ((char=? c #\@) (get-actual-char) (scm-emit-html-char c)) (else false)))
    (emit "</span>"))
   ((or (char=? c #\') (char=? c #\`)) (get-actual-char) (emit "<span class=keyword>")
    (scm-emit-html-char c) (emit "</span>"))
   ((or (char-whitespace? c) (member c *scm-token-delims*)) (get-actual-char) (scm-emit-html-char c))
 ...
 (let
  ...
  (cond
   (yes-p (set! *slatex-math-escape* c)
    (set! *scm-token-delims* (cons *slatex-math-escape* *scm-token-delims*)) *scm-token-delims*)
   (else (set! *slatex-math-escape* false) (set! *scm-token-delims* (remove c *scm-token-delims*))
    *scm-token-delims*))))

(define (scm-escape-into-math) (get-actual-char)
 (let ((math-text (get-till-char *slatex-math-escape*))) (get-actual-char)
  (cond
...
(define (scm-output-slatex-comment)
 (let ((s (get-line))) (emit "<span class=comment>")
  ...
  (let ((%fluid-var-*catcodes* *catcodes*))
   (fluid-let ((*catcodes* %fluid-var-*catcodes*)) (catcode #\\ 0) (catcode *esc-char-verb* 11)
    (tex2page-string s)))
...
(define (scm-output-verbatim-comment) (emit "<span class=comment>")
 (let*
  ...
  (let %loop ()
   (let ((c (get-actual-char)))
    (cond ((or (not c) (char=? c #\newline)) (emit "</span>") (scm-emit-html-char c) (return))
     (else false))
    (cond
     ((and (char-whitespace? c) (let ((c2 (snoop-actual-char))) (or (not c2) (char=? c2 #\newline))))
      (emit "</span>") (scm-emit-html-char (get-actual-char)) (return))
     (else (scm-emit-html-char c))))
   (if %loop-returned %loop-result (%loop)))))

(define (scm-output-comment)
 ((if (tex2page-flag-boolean "\\TZPslatexcomments") scm-output-slatex-comment
   scm-output-verbatim-comment)))

(define (scm-output-extended-comment) (get-actual-char) (emit "<span class=comment>")
 (scm-emit-html-char #\#) (scm-emit-html-char #\|)
 (let*
  ((%loop-returned false) (%loop-result 0)
   (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
  (let %loop ()
    ...
    (cond ((not c) (return))
     ((char=? c #\|)
      (let ((c2 (snoop-actual-char)))
       (cond ((not c2) (scm-emit-html-char c) (return)) ((char=? c2 #\#) (get-actual-char) (return))
        (else (scm-emit-html-char c)))))
     (else (scm-emit-html-char c))))
   (if %loop-returned %loop-result (%loop))))
 (scm-emit-html-char #\|) (scm-emit-html-char #\#) (emit "</span>"))

(define (scm-output-string) (get-actual-char) (emit "<span class=selfeval>") (scm-emit-html-char #\")
 (let ((esc-p false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop ()
    (let ((c (get-actual-char)))
     (case c
      ((#\") (cond ((not esc-p) (return)) (else false)) (scm-emit-html-char c) (set! esc-p false) esc-p)
...
(define (scm-output-hash) (get-actual-char)
 (let ((c (snoop-actual-char)))
  (cond ((not c) (emit "<span class=selfeval>") (scm-emit-html-char #\#) (emit "</span>"))
...
(define (scm-output-token s)
 (case (scm-get-type s)
  ...
  ((:keyword) (emit "<span class=keyword>") (scm-display-token s) (emit "</span>"))
  ((:global) (emit "<span class=global>") (scm-display-token s) (emit "</span>"))
  ((:selfeval) (emit "<span class=selfeval>") (scm-display-token s) (emit "</span>"))
  ((:builtin) (emit "<span class=builtin>") (scm-display-token s) (emit "</span>"))
  ((:background) (scm-display-token s))
  (else (emit "<span class=variable>") (scm-display-token s) (emit "</span>"))))

(define (scm-display-token s)
 (let ((n (let ((%length-arg s)) ((if (string? %length-arg) string-length length) %length-arg))) (k 0))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (cond ((not (< k n)) (return)) (else false))
    (unless %loop-returned (scm-emit-html-char (string-ref s k)))
    (unless %loop-returned (set! k (+ k 1)) k) (if %loop-returned %loop-result (%loop))))))

(define (do-scm-braced result-p) (get-actual-char)
 (let ((display-p (munched-a-newline-p)))
  (cond
   ((not display-p) (emit "<code class=scheme") (cond (result-p (emit "response")) (else false))
    (emit ">"))
   (else (do-end-para) (emit "<pre class=scheme>")))
  (bgroup)
  (let ((%fluid-var-*catcodes* *catcodes*) (%fluid-var-*verb-display-p* display-p) (nesting 0) (c false))
   (fluid-let ((*verb-display-p* %fluid-var-*verb-display-p*) (*catcodes* %fluid-var-*catcodes*))
    ...
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop () (set! c (snoop-actual-char))
      (cond ((not c) (terror 'do-scm-braced "Eof inside verbatim")) (else false))
      (cond
       ((= (catcode c) **escape**)
        (let ((x (get-ctl-seq)))
         (cond ((member x '("\\ " "\\{" "\\}")) (scm-emit-html-char (string-ref x 1)))
          (else
           (let ((%fluid-var-*catcodes* *catcodes*))
            (fluid-let ((*catcodes* %fluid-var-*catcodes*)) (catcode #\\ 0)
             (do-tex-ctl-seq-completely x)))))))
       ((char=? c #\{) (get-actual-char) (scm-emit-html-char c) (set! nesting (+ nesting 1)) nesting)
       ((char=? c #\}) (get-actual-char)
        (case nesting ((0) (return)) (else (scm-emit-html-char c) (set! nesting (- nesting 1)) nesting)))
       (else (scm-output-next-chunk)))
      (if %loop-returned %loop-result (%loop))))))
  (egroup) (cond ((not display-p) (emit "</code>")) (else (emit "</pre>") (do-noindent)))))
 ...
 (let ((d (get-actual-char)))
  (let ((display-p (munched-a-newline-p)))
   (cond
    ...
    (else (do-end-para) (emit "<pre class=scheme>")))
   (let
    ((%fluid-var-*verb-display-p* display-p) (%fluid-var-*scm-token-delims* (cons d *scm-token-delims*))
     (c false))
    (fluid-let
     ((*scm-token-delims* %fluid-var-*scm-token-delims*) (*verb-display-p* %fluid-var-*verb-display-p*))
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
        (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
      (let %loop () (set! c (snoop-actual-char))
       (cond ((not c) (terror 'do-scm-delimed "Eof inside verbatim")) (else false))
       (cond ((char=? c d) (get-actual-char) (return)) (else false))
       (unless %loop-returned (scm-output-next-chunk)) (if %loop-returned %loop-result (%loop))))))
   (cond ((not display-p) (emit "</code>")) (else (emit "</pre>") (do-noindent))))))

(define (do-scm . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (result-p false))
  (when (< 0 %lambda-rest-arg-len) (set! result-p (list-ref %lambda-rest-arg 0)))
  (cond (*outputting-external-title-p* (do-verb))
   (else (ignorespaces) (bgroup)
    (let ((%fluid-var-*ligatures-p* false))
     (fluid-let ((*ligatures-p* %fluid-var-*ligatures-p*))
      ((if (char=? (snoop-actual-char) #\{) do-scm-braced do-scm-delimed) result-p)))
    (egroup)))))

(define (do-scminput) (ignorespaces) (do-end-para) (bgroup) (emit "<pre class=scheme>")
 (let ((f (add-dot-tex-if-no-extension-provided (get-filename-possibly-braced))) (c false))
 ...
 (let
  ...
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (if (>= i %dotimes-n) (return true) false)
    (unless %loop-returned (cond ((not (char=? (string-ref s i) #\.)) (return false)) (else false)))
    (unless %loop-returned (set! i (+ i 1))) (if %loop-returned %loop-result (%loop))))))
...
(define (scm-get-type s)
 (cond ((table-get s *scm-special-symbols*) ':special-symbol) ((table-get s *scm-keywords*) ':keyword)
  ((table-get s *scm-builtins*) ':builtin) ((table-get s *scm-variables*) ':variable)
  ...
  ((string-is-all-dots-p s) ':background) ((char=? (string-ref s 0) #\#) ':selfeval)
  ((string->number s) ':selfeval) (else ':variable)))

(define (eat-star)
 (let ((c (snoop-actual-char))) (if (and (not (not c)) (char=? c #\*)) (get-actual-char) false)))

(define (do-cr z) (ignorespaces)
 (case (car *tabular-stack*)
  ((:tabular) (get-bracketed-text-if-any) (egroup) (emit "</td></tr>") (emit-newline)
   (emit "<tr><td valign=top ") (do-tabular-multicolumn))
  ((:eqnarray*) (emit "</td></tr>") (emit-newline) (set! *equation-position* 0)
   (emit "<tr><td class=rightline>"))
  ((:eqnarray) (emit "</td>")
   (cond
    (*equation-numbered-p* (emit "<td>(") (emit *equation-number*) (bump-dotted-counter "equation")
     (emit ")</td>"))
    (else (set! *equation-numbered-p* true) *equation-numbered-p*))
   (emit "</tr>") (emit-newline) (set! *equation-position* 0) (emit "<tr><td class=rightline>"))
  ((:ruled-table) (emit "</td></tr>") (emit-newline) (emit "<tr><td>"))
  ((:minipage :tabbing) (get-bracketed-text-if-any) (emit "<br>") (emit-newline))
  ((:eqalign :eqalignno :displaylines :pmatrix :mathbox)
   (cond
    ((not (char=? (snoop-actual-char) #\})) (set! *math-height* (+ *math-height* 1)) (emit "</td></tr>")
     (emit-newline) (emit "<tr><td class=centerline>") (set! *equation-position* 0) (emit-newline))
    (else false)))
  ((:header) (emit #\space))
  (else
   (cond
    ((and (eqv? *tex-format* ':latex) (string=? z "\\\\")) (get-bracketed-text-if-any)
     (let ((c (snoop-actual-char)))
      (cond ((and (not (not c)) (char=? c #\*)) (get-actual-char)) (else false)))
     (emit "<br>") (emit-newline))
    (else false)))))

(define (do-ruledtable) (set! *tabular-stack* (cons ':ruled-table *tabular-stack*))
 (emit "<table border=2><tr><td>") (emit-newline))

(define (do-endruledtable) (emit-newline) (emit "</td></tr></table>") (emit-newline)
 (pop-tabular-stack ':ruled-table))

(define (do-tabular . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (mathp false))
  (when (< 0 %lambda-rest-arg-len) (set! mathp (list-ref %lambda-rest-arg 0))) (do-end-para)
  (get-bracketed-text-if-any) (bgroup)
  (cond
   ((not mathp)
    (add-postlude-to-top-frame
     (let ((old-math-mode-p *math-mode-p*) (old-in-display-math-p *in-display-math-p*))
      (set! *math-mode-p* false) (set! *in-display-math-p* false)
      (lambda () (set! *math-mode-p* old-math-mode-p) (set! *in-display-math-p* old-in-display-math-p)
       *in-display-math-p*))))
   (else false))
  (let
     ...
     (if
      (let ((%position-v #\|) (%position-s (get-group)))
       (cond ((string? %position-s) (string-index %position-s %position-v))
        (else (list-position %position-v %position-s))))
      1 0)))
   (set! *tabular-stack* (cons ':tabular *tabular-stack*)) (emit "<table border=") (emit border-width)
   (emit "><tr><td valign=top ") (do-tabular-multicolumn))))

(define (do-end-tabular) (egroup) (do-end-para) (emit "</td></tr></table>") (pop-tabular-stack ':tabular)
 (egroup))

(define (do-tabular-colsep) (egroup) (emit "</td><td valign=top ") (do-tabular-multicolumn))
...
(define (do-tabular-multicolumn)
 (let*
  ((%loop-returned false) (%loop-result 0)
   (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
  (let %loop () (ignorespaces)
   (let ((c (snoop-actual-char))) (cond ((not (and (char? c) (char=? c #\\))) (return)) (else false))
    (let ((x (get-ctl-seq)))
     (cond ((string=? x "\\hline") true)
      ((string=? x "\\multicolumn")
       (let ((n (ungroup (get-token)))) (get-token) (emit " colspan=") (emit n) (return)))
      (else (toss-back-char *invisible-space*) (toss-back-string x) (return)))))
   (if %loop-returned %loop-result (%loop))))
 (emit ">") (bgroup))
...
(define (do-ruledtable-colsep) (emit-newline) (emit "</td><td") (ignorespaces)
 (let ((c (snoop-actual-char)))
  (cond
   ((char=? c #\\)
    (let ((x (get-ctl-seq)))
     (if (string=? x "\\multispan") (let ((n (ungroup (get-token)))) (emit " colspan=") (emit n))
      (toss-back-string x))))
   (else false)))
 (emit ">") (emit-newline))
...
(define (do-romannumeral . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (upcase-p false))
  (when (< 0 %lambda-rest-arg-len) (set! upcase-p (list-ref %lambda-rest-arg 0)))
  (let ((n (get-number-or-false))) (cond (n (emit (number-to-roman n upcase-p))) (else false)))))
  ...
  ((not (inside-false-world-p))
   (let ((c1 (get-tex-char-spec)))
    (let ((c2 (begin (get-equal-sign) (get-tex-char-spec))))
     (let ((fr (top-texframe)))
      (table-put! c1 ((case kase ((:uccode) texframe*-uccodes) ((:lccode) texframe*-lccodes)) fr) c2)
      (table-get c1 ((case kase ((:uccode) texframe*-uccodes) ((:lccode) texframe*-lccodes)) fr))))))
  (else false)))

(define (tex-char-downcase c)
 (or (ormap (lambda (fr) (table-get c (texframe*-lccodes fr))) *tex-env*) (char-downcase c)))

(define (tex-char-upcase c)
 (or (ormap (lambda (fr) (table-get c (texframe*-uccodes fr))) *tex-env*) (char-upcase c)))

(define (do-flipcase kase)
 (emit (map 'string (case kase ((:uccode) tex-char-upcase) ((:lccode) tex-char-downcase)) (get-token))))
...
(define (do-stepcounter)
 (let ((counter-name (get-peeled-group))) (set-latex-counter-aux counter-name true 1)))
...
(define (set-latex-counter-aux counter-name addp new-value)
 (cond
  ((begin (set! *it* (table-get counter-name *dotted-counters*)) *it*)
   (let ((counter *it*))
    (if addp
     (begin (set!counter*-value counter (+ (counter*-value counter) new-value)) (counter*-value counter))
   ...
   (let
    ...
    (cond
     ...
     ((find-count count-seq)
      (tex-gdef-count count-seq (if addp (+ new-value (get-gcount count-seq)) new-value)))
     (else false))))))
 ...
 (let ((y false))
  (cond
   ((begin (set! *it* (find-def z)) *it*) (set! y *it*)
    (cond ((begin (set! *it* (tdef*-defer y)) *it*) (set! y *it*) (toss-back-string y))
     ((begin (set! *it* (tdef*-thunk y)) *it*) (set! y *it*) (y))
     (else (expand-tex-macro (tdef*-optarg y) (tdef*-argpat y) (tdef*-expansion y) (tdef*-catcodes y)))))
...
(define (tex-math-bb c)
 (case c ((#\C) "&#x2102;") ((#\H) "&#x210d;") ((#\N) "&#x2115;") ((#\P) "&#x2119;") ((#\Q) "&#x211a;")
  ((#\R) "&#x211d;") ((#\Z) "&#x2124;")
  (else
   (let
    ((%type 'string)
     (%ee
      (list "&#x" (write-to-string (+ 120120 (- (char->integer c) (char->integer #\A))) ':base 16) ";")))
    (let ((%res (if (eq? %type 'string) "" null)))
     (let %concatenate-loop ((%ee %ee))
      (if (null? %ee) %res
       (let ((%a (car %ee)))
...
(define (tex-math-cal c)
 (let
  ((%type 'string)
   (%ee (list "&#x" (write-to-string (+ 120016 (- (char->integer c) (char->integer #\A))) ':base 16) ";")))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
    (if (null? %ee) %res
     (let ((%a (car %ee)))
 ...
 (let
  ((%type 'string)
   (%ee
    (list "&#x"
     (write-to-string
      (if (char-upper-case? c) (+ 120172 (- (char->integer c) (char->integer #\A)))
...
(define (emit-math-alpha-char c)
 (case *math-font* ((:rm) (emit c)) ((:bf) (emit "<b>") (emit c) (emit "</b>"))
...
(define (do-tex-char c)
 (cond ((= (catcode c) **comment**) (do-comment)) ((inside-false-world-p) true)
  ((= (catcode c) **space**) (emit-space c)) ((char=? c #\{) (bgroup)) ((char=? c #\}) (egroup))
  ((= (catcode c) **math**) (do-math)) ((char=? c #\-) (do-hyphen)) ((char=? c #\`) (do-lsquo))
  ((char=? c #\') (do-rsquo)) ((char=? c #\~) (emit-nbsp 1)) ((char=? c #\!) (do-excl))
  ((char=? c #\?) (do-quest)) ((or (char=? c #\<) (char=? c #\>) (char=? c #\")) (emit-html-char c))
  ((= (catcode c) **alignment**)
   (cond
    (*tabular-stack*
     (case (car *tabular-stack*)
      ((:pmatrix :eqalign :displaylines :mathbox) (emit "&#xa0;</td><td class=centerline>&#xa0;"))
      ((:eqalignno) (set! *equation-position* (add1 *equation-position*)) (emit "</td><td")
       (cond ((= *equation-position* 2) (emit " width=30% class=rightline")) (else false)) (emit ">"))
      ((:eqnarray :eqnarray*) (set! *equation-position* (add1 *equation-position*)) (emit "</td><td")
       (cond ((= *equation-position* 1) (emit " class=centerline width=2%")) (else false)) (emit ">"))
  ...
  (else
   (cond
    (*math-mode-p*
     (case c ((#\^) (do-sup)) ((#\_) (do-sub))
      ((#\+ #\=) (cond ((not *math-script-mode-p*) (emit #\space)) (else false)) (emit c)
       (cond ((not *math-script-mode-p*) (emit #\space)) (else false)))
      (else (if (char-alphabetic? c) (emit-math-alpha-char c) (emit c)))))
    ((and *in-small-caps-p* (char-lower-case? c)) (emit "<small>") (emit (char-upcase c))
     (emit "</small>"))
    (else (emit c))))))
...
(define (do-tex-ctl-seq-completely x)
 (cond ((begin (set! *it* (resolve-defs x)) *it*) (tex2page-string *it*))
  ((begin (set! *it* (do-tex-prim (find-corresp-prim x))) *it*)
   (cond ((eq? *it* ':encountered-undefined-command) (emit x)) (else false)))))
      ...
      (cond ((not c) (return true))
       ...
       ((= (catcode c) **escape**)
        (case (do-tex-ctl-seq (get-ctl-seq)) ((:encountered-endinput) (return true))
         ((:encountered-bye) (return ':encountered-bye)) (else true)))
       (else (get-actual-char) (do-tex-char c))))
     (if %loop-returned %loop-result (%loop)))))))

(define (check-input-file-timestamp-p f)
 (cond ((let ((e (file-extension f))) (and e (member e '(".t2p" ".bbl" ".ind")))) false)
  (*inputting-boilerplate-p* false) (*ignore-timestamp-p* false) ((> *html-only* 0) false)
  ((and (>= (let ((%length-arg f)) ((if (string? %length-arg) string-length length) %length-arg)) 3)
...
(define (tex2page-file-if-exists f) (cond ((file-exists? f) (tex2page-file f)) (else false)))
   ...
   (else
    (cond
     ((string-ci=? e ".tex")
      (set! f
       (substring f 0
        (- (let ((%length-arg f)) ((if (string? %length-arg) string-length length) %length-arg)) 4)))
      f)
     (else false))
...
(define (do-input) (ignorespaces)
 (let ((f (get-filename-possibly-braced)))
  (let ((boilerplate-index *inputting-boilerplate-p*))
   (cond
    ((eqv? *inputting-boilerplate-p* 0) (set! *inputting-boilerplate-p* false) *inputting-boilerplate-p*)
    (else false))
   (let ((%fluid-var-*inputting-boilerplate-p* (and boilerplate-index (add1 boilerplate-index))))
    (fluid-let ((*inputting-boilerplate-p* %fluid-var-*inputting-boilerplate-p*))
     (cond ((ignorable-tex-file-p f) false) ((member f '("miniltx" "miniltx.tex")) (catcode #\@ 11) false)
      ((member f '("texinfo" "texinfo.tex"))
       (cond ((begin (set! *it* (actual-tex-filename "texi2p")) *it*) (tex2page-file *it*))
        (else (terror 'do-input "File texi2p.tex not found"))))
      ((begin (set! *it* (actual-tex-filename f (check-input-file-timestamp-p f))) *it*)
       (tex2page-file *it*))
      (else (write-log #\() (write-log f) (write-log ':separation-space) (write-log "not found)")
       (write-log ':separation-space))))))))

(define (do-includeonly) (ignorespaces)
 (cond ((eq? *includeonly-list* true) (set! *includeonly-list* null) *includeonly-list*) (else false))
 (let ((c (get-actual-char)))
  (cond ((or (not c) (not (char=? c #\{))) (terror 'do-includeonly)) (else false)))
   ...
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (ignorespaces)
     (let ((c (snoop-actual-char))) (cond ((not c) (terror 'do-includeonly)) (else false))
      (cond ((= (catcode c) **comment**) (eat-till-eol)) ((char=? c #\,) (get-actual-char))
       ((char=? c #\}) (get-actual-char) (return)) ((member c *filename-delims*) (terror 'do-includeonly))
       (else (set! *includeonly-list* (cons (get-filename) *includeonly-list*)) *includeonly-list*)))
     (if %loop-returned %loop-result (%loop)))))))
 ...
 (let ((f (ungroup (get-group))))
  (cond
   ((or (eq? *includeonly-list* true) (member f *includeonly-list*))
    (let
     ((%fluid-var-*subjobname* (file-stem-name f)) (%fluid-var-*img-file-count* 0)
      (%fluid-var-*imgdef-file-count* 0))
     (fluid-let
      ((*imgdef-file-count* %fluid-var-*imgdef-file-count*) (*img-file-count* %fluid-var-*img-file-count*)
       (*subjobname* %fluid-var-*subjobname*))
      (tex2page-file (actual-tex-filename f (check-input-file-timestamp-p f))))))
   (else false))))
...
(define (call-with-html-output-going-to p th)
 (let ((%fluid-var-*html* (make-ostream* ':stream p))) (fluid-let ((*html* %fluid-var-*html*)) (th))))
 ...
 (let
  ((run-bibtex-p
    (cond ((not *using-bibliography-p*) false)
     ...
     (*source-changed-since-last-run-p* (flag-missing-piece ':fresh-bibliography) true) (else false)))
   (run-makeindex-p
       ...
       (file-exists?
        (let ((%type 'string) (%ee (list *aux-dir/* *jobname* *index-file-suffix* ".idx")))
         (let ((%res (if (eq? %type 'string) "" null)))
          ...
          %res))))
      false)
     ((member ':fresh-index *missing-pieces*) false) ((member ':index *missing-pieces*) true)
     (*source-changed-since-last-run-p* (flag-missing-piece ':fresh-index) true) (else false))))
  (cond
   (run-bibtex-p (write-log ':separation-newline) (write-log "Running: bibtex ") (write-log *aux-dir/*)
    (write-log *jobname*) (write-log *bib-aux-file-suffix*) (write-log #\space)
    (system
     (let ((%type 'string) (%ee (list "bibtex " *aux-dir/* *jobname* *bib-aux-file-suffix*)))
      (let ((%res (if (eq? %type 'string) "" null)))
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
         (let ((%a (car %ee)))
    ...
    (cond
     ((not
       (file-exists?
        (let ((%type 'string) (%ee (list *aux-dir/* *jobname* *bib-aux-file-suffix* ".bbl")))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
   ...
   (else false))
  (cond
   (run-makeindex-p (write-log ':separation-newline) (write-log "Running: makeindex ")
    (write-log *aux-dir/*) (write-log *jobname*) (write-log *index-file-suffix*) (write-log #\space)
    (system
     (let ((%type 'string) (%ee (list "makeindex " *aux-dir/* *jobname* *index-file-suffix*)))
      (let ((%res (if (eq? %type 'string) "" null)))
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
         (let ((%a (car %ee)))
    ...
    (cond
     ((not
       (file-exists?
        (let ((%type 'string) (%ee (list *aux-dir/* *jobname* *index-file-suffix* ".ind")))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
       ...
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
         (let ((%a (car %ee)))
          ...
          (%concatenate-loop (cdr %ee)))))
       %res)))
    (%ee (list ':if-does-not-exist false)) (%if-does-not-exist (cadr (memv ':if-does-not-exist %ee))))
   (cond ((and (not %if-does-not-exist) (not (file-exists? %f))) false) (else (load %f))))
  (for-each
   (lambda (f)
    (cond
     ((file-exists? f) (write-log ':separation-newline) (write-log "Running: metapost ") (write-log f)
      (write-log ':separation-newline) (call-mp f))
     (else false)))
   *mp-files*)
  (for-each
   (lambda (eps-file+img-file-stem)
    (retry-lazy-image (car eps-file+img-file-stem) (cdr eps-file+img-file-stem)))
   *missing-eps-files*)))
...
(define (first-file-that-exists . ff)
 (let ((%dolist-l ff) (f false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (if (null? %dolist-l) (return) false) (unless %loop-returned (set! f (car %dolist-l)))
    (unless %loop-returned (set! %dolist-l (cdr %dolist-l)))
    (unless %loop-returned (cond ((file-exists? f) (return f)) (else false)))
    (if %loop-returned %loop-result (%loop))))))
...
(define (file-in-home f)
 (let ((home (getenv "HOME")))
  (and home
   (let
    ((slash-already-p
      (let ((n (let ((%length-arg home)) ((if (string? %length-arg) string-length length) %length-arg))))
       (and (>= n 0) (let ((c (string-ref home (sub1 n)))) (or (char=? c #\/) (char=? c #\\)))))))
...
(define (make-target-dir)
 (let
  ...
  (cond
   (hdir-file
    (let ((hdir (call-with-input-file/buffered hdir-file (lambda () (get-filename-possibly-braced)))))
     (cond
      ((not (= (let ((%length-arg hdir)) ((if (string? %length-arg) string-length length) %length-arg)) 0))
       (system
        ...
        ((probe
          (let ((%type 'string) (%ee (list hdir "/probe")))
           (let ((%res (if (eq? %type 'string) "" null)))
            ...
            %res))))
        (cond
         ((file-exists? probe) (ensure-file-deleted probe) (set! *aux-dir* hdir)
          (set! *aux-dir/*
...
(define (start-css-file)
 ...
 (let
  ((css-file
    (let ((%type 'string) (%ee (list *aux-dir/* *jobname* *css-file-suffix*)))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
      ...
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (cond ((file-exists? label-file) (load-tex2page-data-file label-file) (delete-file label-file))
   (else false)))
 (cond
      ...
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (cond ((file-exists? aux-file) (load-tex2page-data-file aux-file) (delete-file aux-file)) (else false)))
 (start-css-file)
 (cond ((not (null? *toc-list*)) (set! *toc-list* (reverse *toc-list*)) *toc-list*) (else false))
 (cond ((not (null? *stylesheets*)) (set! *stylesheets* (reverse *stylesheets*)) *stylesheets*)
  (else false))
 (cond ((not (null? *html-head*)) (set! *html-head* (reverse *html-head*)) *html-head*) (else false)))
 ...
 (let ((s (file-or-directory-modify-seconds f)))
  (cond
   ((and s (or (not *last-modification-time*) (> s *last-modification-time*)))
    (set! *source-changed-since-last-run-p* true) (set! *last-modification-time* s)
    (cond
     ((and
       (or (not (tex2page-flag-boolean "\\TZPcolophondisabletimestamp"))
        (tex2page-flag-boolean "\\TZPcolophontimestamp"))
       (not (tex2page-flag-boolean "\\TZPcolophonlastpage")) (> *html-page-count* 1))
...
(define (!toc-entry level number page label header)
 (set! *toc-list*
  (cons (make-tocentry* ':level level ':number number ':page page ':label label ':header header)
   *toc-list*))
 *toc-list*)
...
(define (!index index-number html-page-number) (table-put! index-number *index-table* html-page-number)
...
(define (!last-modification-time s . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (epoch 1900))
  ...
  (set! *last-modification-time*
   (case epoch ((1900) s) ((1970) (+ s 2208988800)) (else (error 'ecase "0xdeadc0de"))))
...
(define (!script jsf)
 (cond
  ...
  (else (write-log "! Can't find script ") (write-log jsf) (write-log ':separation-newline))))

(define (!using-chapters) (set! *using-chapters-p* true) *using-chapters-p*)

(define (!definitely-latex) (set! *tex-format* ':latex)
 (cond ((< (get-gcount "\\secnumdepth") -1) (tex-gdef-count "\\secnumdepth" 3)) (else false)))
...
(define (!colophon x)
 (case x ((:last-page) (tex-def-0arg "\\TZPcolophonlastpage" "1"))
  ((:no-timestamp) (tex-def-0arg "\\TZPcolophondisabletimestamp" "1")
   (tex-def-0arg "\\TZPcolophontimestamp" "0"))
  ((:dont-credit-tex2page) (tex-def-0arg "\\TZPcolophondisablecredit" "1")
   (tex-def-0arg "\\TZPcolophoncredit" "0"))
  ((:dont-link-to-tex2page-website) (tex-def-0arg "\\TZPcolophondisableweblink" "1")
   (tex-def-0arg "\\TZPcolophonweblink" "0"))))

(define (!tex-text n) (cond ((= n 0) (set! *ligatures-p* false) *ligatures-p*) (else false)))

(define (!opmac-iis lhs sub)
 (cond
  ((not *opmac-index-sub-table*) (set! *opmac-index-sub-table* (make-table ':test equal?))
   *opmac-index-sub-table*)
  (else false))
 (table-put! lhs *opmac-index-sub-table* sub) (table-get lhs *opmac-index-sub-table*))

(define (fully-qualified-url-p u) (or (substring? "//" u) (char=? (string-ref u 0) #\/)))

(define (fully-qualified-pathname-p f)
 (let ((n (let ((%length-arg f)) ((if (string? %length-arg) string-length length) %length-arg))))
  (if (= n 0) true
   (let ((c0 (string-ref f 0)))
    (cond ((char=? c0 #\/) true) ((= n 1) false)
     ((and (char-alphabetic? c0) (char=? (string-ref f 1) #\:)) true) (else false))))))

(define (ensure-url-reachable f . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (deletep false))
  (when (< 0 %lambda-rest-arg-len) (set! deletep (list-ref %lambda-rest-arg 0)))
  (cond
...
(define (!stylesheet css)
 (cond
  ...
  (else (write-log "! Can't find stylesheet ") (write-log css) (write-log ':separation-newline))))

(define (!html-head s) (set! *html-head* (cons s *html-head*)) *html-head*)

(define (!html-redirect url seconds) (set! *redirect-url* url) (set! *redirect-delay* seconds)
 (!html-head
  (let ((%type 'string) (%ee (list "<meta http-equiv=\"refresh\" content=\"" seconds ";" url "\">")))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
...
(define (!default-title title) (cond ((not *title*) (set! *title* title) *title*) (else false)))

(define (!preferred-title title) (set! *title* title) *title*)

(define (!infructuous-calls-to-tex2page n) (set! *infructuous-calls-to-tex2page* n)
...
(define (load-tex2page-data-file f)
 (let
  ((i
    (let*
     ((%f f) (%ee (list ':direction ':input ':if-does-not-exist false)) (%direction (memv ':direction %ee))
      (%if-exists (memv ':if-exists %ee)) (%if-does-not-exist ':error)
      (%if-does-not-exist-from-user (memv ':if-does-not-exist %ee)))
     (when %direction (set! %direction (cadr %direction)))
     (when %if-exists (set! %if-exists (cadr %if-exists)))
  ...
  (let
   ((%with-open-file-res
     (cond
      (i
       (let
        ((%fluid-var-*current-source-file* f) (%fluid-var-*input-line-no* 0) (e false) (directive false))
        (fluid-let
         ((*input-line-no* %fluid-var-*input-line-no*)
          (*current-source-file* %fluid-var-*current-source-file*))
         (let*
          ...
          (let %loop ()
           (set! e
            (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res))
           (cond ((not e) (return)) (else false))
           (unless %loop-returned (set! directive (car e)) directive)
           (unless %loop-returned (set! *input-line-no* (+ *input-line-no* 1)) *input-line-no*)
           (unless %loop-returned
            (apply
             (case directive ((!colophon) !colophon) ((!default-title) !default-title)
              ((!definitely-latex) !definitely-latex) ((!doctype) !doctype)
              ((!external-labels) !external-labels) ((!foot-line) !foot-line) ((!head-line) !head-line)
              ((!html-head) !html-head) ((!html-redirect) !html-redirect) ((!index) !index)
              ((!index-page) !index-page) ((!infructuous-calls-to-tex2page) !infructuous-calls-to-tex2page)
              ((!label) !label) ((!lang) !lang) ((!last-modification-time) !last-modification-time)
              ((!last-page-number) !last-page-number) ((!opmac-iis) !opmac-iis)
              ((!preferred-title) !preferred-title) ((!script) !script) ((!single-page) !single-page)
              ((!slides) !slides) ((!stylesheet) !stylesheet) ((!tex-like-layout) !tex-like-layout)
              ((!tex-text) !tex-text) ((!toc-entry) !toc-entry) ((!toc-page) !toc-page)
              ((!using-chapters) !using-chapters) ((!using-external-program) !using-external-program)
              (else (terror 'load-tex2page-data-file "Fatal aux file error " directive "; I'm stymied.")))
             (cdr e)))
           (if %loop-returned %loop-result (%loop)))))))
      (else false))))
   (cond (i ((if (input-port? i) close-input-port close-output-port) i)) (else false))
...
(define (tex2page-help not-a-file)
 (cond ((not not-a-file) (set! not-a-file "--missing-arg") not-a-file) (else false))
 (write-aux `(!infructuous-calls-to-tex2page ,(add1 *infructuous-calls-to-tex2page*)))
 (cond
  ((not
...
For more information on how to use tex2page, please see")
  ...
  (else
   (cond
    ((string=? not-a-file "--missing-arg") (write-log "! Missing command-line argument.")
     (write-log ':separation-newline))
    (else false))
   (cond
    ((> *infructuous-calls-to-tex2page* 0) (write-log "You have called TeX2page") (write-log #\space)
     (write-log (add1 *infructuous-calls-to-tex2page*)) (write-log #\space)
     (write-log "times without a valid input document.") (write-log ':separation-newline))
    (else false))
   (cond
    ((>= *infructuous-calls-to-tex2page* 4) (write-log "I can't go on meeting you like this.")
     (write-log ':separation-newline) (write-log "Good bye!") (write-log ':separation-newline))
    (else
     (write-log
    ...
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
       ...
       (%concatenate-loop (cdr %ee)))))
    %res)))
 (emit "<span style=\"color: red\">&#x2388;&#xa0;") (for-each emit-html-string ss) (emit-link-stop)
 (emit "</span>"))

(define (do-math-ctl-seq s)
 (cond ((begin (set! *it* (find-math-def s)) *it*) ((tdef*-thunk *it*)))
  (else
   (cond ((not *math-needs-image-p*) (set! *math-needs-image-p* true) *math-needs-image-p*) (else false))
   (emit (substring s 1)))))
...
(define (tex-defsym-math-prim cs str)
 (tex-def cs null false false (lambda () (emit str)) cs false *math-primitive-texframe*))
...
(tex-def-prim "\\rm" (lambda () (cond (*math-mode-p* (do-switch ':rm)) (else false))))
...
(define (do-not) (ignorespaces)
 (let ((c (snoop-actual-char)))
  (if (= (catcode c) **escape**)
   (let ((x (get-ctl-seq)))
    (emit
     (cond ((string=? x "\\leq") "&#x2270;") ((string=? x "\\le") "&#x2270;")
      ((string=? x "\\prec") "&#x2280;") ((string=? x "\\preceq") "&#x22e0;")
      ((string=? x "\\subset") "&#x2284;") ((string=? x "\\subseteq") "&#x2288;")
      ((string=? x "\\sqsubseteq") "&#x22e2;") ((string=? x "\\geq") "&#x2271;")
      ((string=? x "\\ge") "&#x2271;") ((string=? x "\\succ") "&#x2281;")
      ((string=? x "\\succeq") "&#x22e1;") ((string=? x "\\supset") "&#x2285;")
      ((string=? x "\\supseteq") "&#x2289;") ((string=? x "\\sqsupseteq") "&#x22e3;")
      ((string=? x "\\equiv") "&#x2262;") ((string=? x "\\sim") "&#x2241;")
      ((string=? x "\\simeq") "&#x2244;") ((string=? x "\\approx") "&#x2249;")
      ((string=? x "\\cong") "&#x2247;") ((string=? x "\\asymp") "&#x226d;")
      (else (toss-back-string x) "/"))))
   (case c
    ((#\< #\> #\=) (get-actual-char)
     (emit (cond ((char=? c #\<) "&#x226e;") ((char=? c #\>) "&#x226f;") ((char=? c #\=) "&#x2260;"))))
    (else (emit "/"))))))
...
(tex-def-math-prim "\\sqrt" (lambda () (emit "&#x221a;(") (tex2page-string (get-token)) (emit ")")))
...
(for-each texbook-18-2
 '("arccos" "arcsin" "arctan" "arg" "cos" "cosh" "cot" "coth" "csc" "deg" "det" "dim" "exp" "gcd" "hom"
   "inf" "ker" "lg" "lim" ("liminf" "lim inf") ("limsup" "lim sup") "ln" "log" "max" "min" "Pr"
   ("TIIPsec" "sec") "sin" "sinh" "sup" "tan" "tanh"))
...
(tex-def-prim "\\MF"
 (let
  ((mf
    (let ((%type 'string) (%ee (list "<span style=\"" "font-family: sans-serif" "\">METAFONT</span>")))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
 ...
 (let
  ((ams
    (let
     ((%type 'string)
      (%ee
       (list "<span style=\"font-family: cursive;\">" "A" "<span style=\"" "position: relative; "
        "top: 0.5ex; " "margin-left: -.1667em; " "margin-right: -.075em" "\">M</span>" "S</span>")))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
...
(define (do-ifdefined)
 (let ((x (get-raw-token/is)))
  (if (or (not (ctl-seq-p x)) (and (ctl-seq-p x) (or (find-def x) (find-math-def x)))) (do-iftrue)
   (do-iffalse))))
        ...
        (let*
         ...
         (let %loop ()
          (cond ((eat-word "height") (set! height (get-pixels)) height)
           ...
           ((eat-word "width") (set! width (get-pixels)) width) (else (return)))
          (if %loop-returned %loop-result (%loop))))
        (cond
         ((not (file-exists? fq-img-file)) (write-log ':separation-space) (write-log #\{)
          (write-log pdf-file) (write-log ':separation-space) (write-log "->")
          (write-log ':separation-space) (write-log img-file) (write-log #\})
          (write-log ':separation-space) (ps-to-img pdf-file img-file))
         (else false))
        (write-log #\() (write-log img-file) (write-log ':separation-space) (emit "<img src=\"")
        (do-img-src (ensure-url-reachable img-file ':delete)) (emit "\"")
        (cond (height (emit " height=") (emit height)) (else false))
        (cond
         (rotated (set! rotated (- rotated)) (emit " style=\"transform: rotate(") (emit rotated)
          (emit "deg)\""))
         (else false))
        (cond (width (emit " width=") (emit width)) (else false)) (emit " alt=\"") (emit img-file)
        (emit "\"") (emit ">") (write-log #\)) (write-log ':separation-space)))))))))
  ...
  (let*
   ...
   (let %loop ()
    (cond ((eat-word "height") (set! height (get-pixels)) height)
     ...
     ((eat-word "width") (set! width (get-pixels)) width) (else (return)))
    (if %loop-returned %loop-result (%loop))))
  (emit "<img src=\"") (do-img-src (fully-qualify-url img-file)) (emit "\"")
  (cond (height (emit " height=") (emit height)) (else false))
  (cond
   (rotated (set! rotated (- rotated)) (emit " style=\"transform: rotate(") (emit rotated) (emit "deg)\""))
   (else false))
  (cond (width (emit " width=") (emit width)) (else false)) (emit " alt=\"") (emit img-file) (emit "\"")
  (emit ">")))
...
(tex-def-prim "\\htmlonly" (lambda () (set! *html-only* (+ *html-only* 1)) *html-only*))

(tex-def-prim "\\endhtmlonly" (lambda () (set! *html-only* (- *html-only* 1)) *html-only*))
...
(define (do-iffileexists)
 (let ((file (actual-tex-filename (get-filename-possibly-braced))))
  (let ((then-e (ungroup (get-group))))
   (let ((else-e (ungroup (get-group)))) (tex2page-string (if file then-e else-e))))))
 ...
 (let ((f (actual-tex-filename (get-filename-possibly-braced))))
  (let ((then-txt (ungroup (get-group))))
   (let ((else-txt (ungroup (get-group))))
    (cond (f (tex2page-string then-txt) (tex2page-file f)) (else (tex2page-string else-txt)))))))
...
(define (do-futurenonspacelet)
 (let ((first (get-raw-token/is)))
  (let ((second (get-raw-token/is)))
   (let ((third (get-raw-token/is))) (do-futurelet-aux first second third)))))
 ...
 (call-with-input-string/buffered (ungroup (get-group))
  (lambda ()
   (let*
    ...
    (let %loop () (ignorespaces ':all)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (let ((s (scm-get-token))) (table-put! s *scm-builtins* false) (table-put! s *scm-variables* false)
       (table-put! s *scm-keywords* true) (table-get s *scm-keywords*)))
     (if %loop-returned %loop-result (%loop)))))))
 ...
 (call-with-input-string/buffered (ungroup (get-group))
  (lambda ()
   (let*
    ...
    (let %loop () (ignorespaces ':all)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (let ((s (scm-get-token))) (table-put! s *scm-keywords* false) (table-put! s *scm-variables* false)
       (table-put! s *scm-builtins* true) (table-get s *scm-builtins*)))
     (if %loop-returned %loop-result (%loop)))))))
 ...
 (call-with-input-string/buffered (ungroup (get-group))
  (lambda ()
   (let*
    ...
    (let %loop () (ignorespaces ':all)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (let ((s (scm-get-token))) (table-put! s *scm-builtins* false) (table-put! s *scm-keywords* false)
       (table-put! s *scm-variables* true) (table-get s *scm-variables*)))
     (if %loop-returned %loop-result (%loop)))))))
...
(define (do-scm-slatex-lines env display-p result-p)
 (let
  ...
  (fluid-let ((*catcodes* %fluid-var-*catcodes*))
   ...
   (cond ((string=? env "tt") (do-tex-ctl-seq-completely "\\tthook") (catcode #\\ 12)) (else false))
   (emit "<div class=leftline><pre class=scheme") (cond (result-p (emit "response")) (else false))
   (emit ">")
   (let
    ((%fluid-var-*ligatures-p* false) (%fluid-var-*verb-display-p* true)
     (%fluid-var-*not-processing-p* true) (c false))
    (fluid-let
     ((*not-processing-p* %fluid-var-*not-processing-p*) (*verb-display-p* %fluid-var-*verb-display-p*)
...
(tex-def-prim "\\schemedisplay" (lambda () (do-scm-slatex-lines "schemedisplay" true false)))

(tex-def-prim "\\schemeresponse" (lambda () (do-scm-slatex-lines "schemeresponse" true ':result)))
...
(define (do-index-help idx-entry) (set! *index-count* (+ *index-count* 2))
 (!index *index-count* *html-page-count*) (write-aux `(!index ,*index-count* ,*html-page-count*))
 (let
  ((tag
    (let ((%type 'string) (%ee (list *html-node-prefix* "index_" (write-to-string *index-count*))))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
...
(define (do-index)
 (let ((idx-entry (ungroup (get-group)))) (ignorespaces)
  (cond ((not (substring? "|)" idx-entry)) (do-index-help idx-entry)) (else false))))

(tex-def-prim "\\index" do-index)

(define (do-theindex) (bgroup) (tex2page-string "\\let\\endtheindex\\egroup")
 (tex2page-string "\\let\\indexspace\\relax") (tex2page-string "\\let\\item\\indexitem")
 (tex2page-string "\\let\\subitem\\indexsubitem") (tex2page-string "\\let\\subsubitem\\indexsubsubitem")
 (tex2page-string "\\let\\(\\expandhtmlindex"))

(tex-def-prim "\\theindex" do-theindex)

(define (do-indexitem indent) (set! *index-page-mention-alist* (make-table)) (emit "<br>") (emit-newline)
 (emit-nbsp (* indent 4)))
...
(define (do-inputindex . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (insert-heading-p false))
  (when (< 0 %lambda-rest-arg-len) (set! insert-heading-p (list-ref %lambda-rest-arg 0)))
  (set! *using-index-p* true)
  (cond
   (insert-heading-p
    (tex2page-string (if *using-chapters-p* "\\chapter*{\\indexname}" "\\section*{\\indexname}"))
    (emit-newline))
   (else false))
  (emit-anchor
   (let ((%type 'string) (%ee (list *html-node-prefix* "index_start")))
     ...
     (let %concatenate-loop ((%ee %ee))
      (if (null? %ee) %res
       (let ((%a (car %ee)))
        ...
        (%concatenate-loop (cdr %ee)))))
     %res)))
  (!index-page *html-page-count*) (write-aux `(!index-page ,*html-page-count*))
  (let
   ((ind-file
     (let ((%type 'string) (%ee (list *aux-dir/* *jobname* *index-file-suffix* ".ind")))
      (let ((%res (if (eq? %type 'string) "" null)))
   ...
   (cond ((file-exists? ind-file) (tex2page-file ind-file))
    (else (flag-missing-piece ':index) (non-fatal-error "Index not generated; rerun TeX2page"))))))

(tex-def-prim "\\inputindex" (lambda () (do-inputindex false)))

(define (do-xrdef) (let ((tag (get-peeled-group))) (do-tag-aux tag (write-to-string *html-page-count*))))
 ...
 (let ((label-ref (label-bound-p (get-peeled-group))))
  (if label-ref
   (let ((pageno (label*-page label-ref)))
    (emit-ext-page-node-link-start (label*-src label-ref) pageno false) (emit pageno) (emit-link-stop))
   (non-fatal-error "***"))))
 ...
 (let ((f (get-filename-possibly-braced)))
  (cond
   ((not *eval-for-tex-only-p*)
    (let ((epsf-x-size (get-dimen "\\epsfxsize")) (epsf-y-size (get-dimen "\\epsfysize")))
     (cond
      ((and (= epsf-x-size 0) (= epsf-y-size 0))
       (let ((img-file-stem (next-html-image-file-stem))) (lazily-make-epsf-image-file f img-file-stem)
        (source-img-file img-file-stem)))
      (else (cond ((not (= epsf-x-size 0)) (tex2page-string "\\epsfxsize=0pt")) (else false))
       ...
       (let ((%fluid-var-*imgpreamble-inferred* (cons ':epsfbox *imgpreamble-inferred*)))
        (fluid-let ((*imgpreamble-inferred* %fluid-var-*imgpreamble-inferred*))
         (call-with-html-image-stream
          (lambda (o)
           (cond
            ((not (= epsf-x-size 0)) (display "\\epsfxsize=" o) (display epsf-x-size o) (display "sp" o)
             (newline o))
            (else false))
           (cond
            ((not (= epsf-y-size 0)) (display "\\epsfysize=" o) (display epsf-y-size o) (display "sp" o)
             (newline o))
            (else false))
           (display "\\epsfbox{" o) (display f o) (display #\} o)))))))))
   (else false))))
    ...
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop ()
      (set! x (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false)) %read-res))
      (cond ((not x) (return)) (else false)) (unless %loop-returned (eval1 x))
      (if %loop-returned %loop-result (%loop))))))))
...
(define (do-eval kind)
 (let
  ((s
    (if *outer-p*
     (ungroup
      (let ((%fluid-var-*catcodes* *catcodes*) (%fluid-var-*expand-escape-p* true))
       (fluid-let ((*expand-escape-p* %fluid-var-*expand-escape-p*) (*catcodes* %fluid-var-*catcodes*))
        (catcode #\\ 12) (catcode *esc-char-verb* 0) (get-group))))
     (tex-write-output-string (ungroup (get-group))))))
  (cond
   ((not (inside-false-world-p)) (cond ((> *html-only* 0) (set! kind ':html) kind) (else false))
    (case kind ((:quiet) (do-eval-string s))
     (else (tex2page-string (cl-with-output-to-string ((current-output-port)) (do-eval-string s))))))
   (else false))))
...
(define (do-thebibliography) (do-end-para) (get-group)
 (cond
  ((eq? *tex-format* ':latex)
   (tex2page-string (if *using-chapters-p* "\\chapter*{\\bibname}" "\\section*{\\refname}")))
  (else false))
 (bgroup) (set! *bibitem-num* 0) (tex2page-string "\\let\\em\\it") (tex2page-string "\\def\\newblock{ }")
 (emit "<table>") (munch-newlines) (emit-newline))

(tex-def-prim "\\beginthebibliography" do-thebibliography)

(tex-def-prim "\\endthebibliography" (lambda () (emit "</table>") (egroup) (do-para)))

(define (do-bibitem) (do-end-para)
 (let ((bibmark (get-bracketed-text-if-any)))
  (cond ((not (= *bibitem-num* 0)) (emit "</td></tr>") (emit-newline)) (else false))
  (emit "<tr><td class=\"rightline bibitem\">") (set! *bibitem-num* (+ *bibitem-num* 1))
  (let ((bibitem-num-s (write-to-string *bibitem-num*)))
   (let
    ...
    (let
     ((node-name
       (let ((%type 'string) (%ee (list *html-node-prefix* "bib_" bibitem-num-s)))
        (let ((%res (if (eq? %type 'string) "" null)))
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
           (let ((%a (car %ee)))
     ...
     (cond ((not bibmark) (set! bibmark bibitem-num-s) bibmark) (else false))
     (tex-def-0arg "\\@currentlabel" bibmark) (emit-anchor node-name) (emit "[") (tex2page-string bibmark)
     (emit "]") (emit-nbsp 2) (do-label-aux key) (emit "</td><td>"))))))
      ...
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (write-bib-aux "\\bibdata{") (write-bib-aux bibdata) (write-bib-aux "}") (write-bib-aux #\newline)
  (cond ((file-exists? bbl-file) (set! *bibitem-num* 0) (tex2page-file bbl-file) (emit-newline))
...
(define (do-bibliographystyle-help s) (write-bib-aux "\\bibstyle{") (write-bib-aux s) (write-bib-aux "}")
 (write-bib-aux #\newline))

(define (do-bibliographystyle) (do-bibliographystyle-help (ungroup (get-token))))
...
(define (do-cite-help delim . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (extra-text false))
  (when (< 0 %lambda-rest-arg-len) (set! extra-text (list-ref %lambda-rest-arg 0)))
  (let
   ((closing-delim
     (cond ((char=? delim #\{) #\}) ((char=? delim #\[) #\])
      (else (terror 'do-cite-help "faulty delim" delim)))))
   (ignorespaces)
   (cond ((not (char=? (get-actual-char) delim)) (terror 'do-cite "missing" delim)) (else false))
   (emit "[")
   (let ((first-key-p true) (key false))
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop () (set! key (get-csv closing-delim)) (cond ((not key) (return)) (else false))
      (unless %loop-returned
       (cond (first-key-p (set! first-key-p false) first-key-p) (else (emit ",") (emit-nbsp 1))))
      (unless %loop-returned (write-bib-aux "\\citation{")) (unless %loop-returned (write-bib-aux key))
      (unless %loop-returned (write-bib-aux "}")) (unless %loop-returned (write-bib-aux #\newline))
      (unless %loop-returned
       (do-ref-aux
        ...
        false false))
      (if %loop-returned %loop-result (%loop))))
    (cond (extra-text (emit ",") (emit-nbsp 1) (tex2page-string extra-text)) (else false))
    (cond ((not (char=? (get-actual-char) closing-delim)) (terror 'do-cite "missing" closing-delim))
     (else false))
    (cond (first-key-p (terror 'do-cite "empty \\cite")) (else false)))
   (emit "]"))))

(define (do-cite)
 (if (tex2page-flag-boolean "\\TZPopmac") (do-cite-help #\[ false)
...
(define (do-nocite) (ignorespaces)
 (let ((delim (if (tex2page-flag-boolean "\\TZPopmac") #\[ #\{)))
  (let ((closing-delim (if (char=? delim #\{) #\} #\])))
   (cond ((not (char=? (get-actual-char) delim)) (terror 'do-nocite "missing" delim)) (else false))
   (let ((key false))
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
          ...
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
             ...
             (%concatenate-loop (cdr %ee)))))
          %res))))
      (unless %loop-returned (write-bib-aux #\newline)) (if %loop-returned %loop-result (%loop)))))
   (cond ((not (char=? (get-actual-char) closing-delim)) (terror 'do-nocite "missing" closing-delim))
    (else false)))))

(tex-def-prim "\\nocite" do-nocite)

(define (do-color) (let ((model (color-model-to-keyword (get-bracketed-text-if-any)))) (do-switch model)))
...
(define (do-colorbox)
 (let ((model (get-bracketed-text-if-any)))
  (let ((color (get-group)))
   (let ((text (get-peeled-group))) (toss-back-char #\}) (toss-back-string text) (toss-back-string color)
...
(define (do-pagecolor)
 (let ((model (color-model-to-keyword (get-bracketed-text-if-any))))
  (let ((color (read-color model))) (display "body { background-color: " *css-stream*)
...
(define (do-definecolor)
 (let ((name (get-peeled-group)))
  (let ((model (color-model-to-keyword (get-peeled-group))))
   (set! *color-names* (cons (cons name (read-color model)) *color-names*)) *color-names*)))
 ...
 (let
  ((header
    (let ((%fluid-var-*tabular-stack* (list ':header)))
     (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*))
      (tex-string-to-html-string (get-till-par))))))
  (let ((nonum-p *opmac-nonum-p*) (notoc-p *opmac-notoc-p*)) (set! *opmac-nonum-p* false)
   (set! *opmac-notoc-p* false) (do-heading-help seclvl false nonum-p notoc-p false header))))

(tex-def-prim "\\chap" (lambda () (do-opmac-heading 0)))

(define (do-opmac-sec) (if *math-mode-p* (toss-back-string "\\TIIPsec") (do-opmac-heading 1)))
...
(define (do-opmac-item)
 (cond
  ((= (the-count "\\TIIPopmacliststarted") 0) (plain-count "\\TIIPopmacliststarted" 1 false)
   ...
   (emit (case *opmac-list-style* ((#\o #\- #\x #\X) "u") (else "o"))) (emit "l style=\"list-style-type: ")
   (emit
    (case *opmac-list-style* ((#\o) "disc") ((#\O) "circle") ((#\-) "'-'") ((#\n #\N) "decimal")
     ((#\i) "lower-roman") ((#\I) "upper-roman") ((#\a) "lower-alpha") ((#\A) "upper-alpha")
     ((#\x #\X) "square") (else "disc")))
   (emit "\">"))
  (else false))
 (do-regular-item))

(define (do-opmac-enditems) (egroup) (do-end-para) (pop-tabular-stack ':opmac-itemize) (emit "</")
 (emit (case *opmac-list-style* ((#\o #\- #\x #\X) "u") (else "o"))) (emit "l>") (do-noindent))

(tex-def-prim "\\enditems" do-opmac-enditems)

(define (do-opmac-ii retainp)
 (let ((lhs (get-word)))
  (let ((sub (and *opmac-index-sub-table* (table-get lhs *opmac-index-sub-table*))))
   (if retainp (toss-back-string lhs) (ignorespaces))
   (do-index-help
    (cond (sub sub) (else (string=join (map escape-opmac-index-entry (string=split lhs #\/)) #\!)))))))
  ...
  (let ((rhs (get-peeled-group)))
   (let ((lhs-list (map escape-opmac-index-entry (string=split lhs #\/))))
    (let ((rhs-list (map escape-opmac-index-entry (string=split rhs #\/))))
     (let ((sub ""))
      (cond ((not (= (length lhs-list) (length rhs-list))) (terror 'do-opmac-iis "Malformed \\iis."))
       (else false))
      (let*
       ((%loop-returned false) (%loop-result 0)
        (return
         (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
       (let %loop () (cond ((null? lhs-list) (return true)) (else false))
        (unless %loop-returned
         (let
          ((additive
            (let
             ((%type 'string)
              (%ee
               (list
                (let* ((%pop-old-stack lhs-list) (%pop-top-value (car %pop-old-stack)))
                 (begin (set! lhs-list (cdr %pop-old-stack)) lhs-list) %pop-top-value)
                "@"
                (let* ((%pop-old-stack rhs-list) (%pop-top-value (car %pop-old-stack)))
                 (begin (set! rhs-list (cdr %pop-old-stack)) rhs-list) %pop-top-value))))
             (let ((%res (if (eq? %type 'string) "" null)))
              (let %concatenate-loop ((%ee %ee))
               (if (null? %ee) %res
                (let ((%a (car %ee)))
          ...
          sub))
        (if %loop-returned %loop-result (%loop))))
      (cond ((not *opmac-index-sub-table*) (flag-missing-piece ':fresh-index)) (else false))
...
(define (do-opmac-usebibtex)
 (let ((bibfile (ungroup (get-token))))
  (let ((bibstyle (ungroup (get-token)))) (do-bibliographystyle-help bibstyle)
         ...
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
           (let ((%a (car %ee)))
            ...
            (%concatenate-loop (cdr %ee)))))
         %res))))
     (cond ((not key0) (terror 'do-opmac-bib "Improper \\bib entry")) (else false))
     (tex-def-0arg "\\TIIPcurrentnodename" node-name) (tex-def-0arg "\\@currentlabel" bibitem-num-s)
     (emit-anchor node-name) (emit "[") (tex2page-string bibitem-num-s) (emit "]") (emit-nbsp 2)
     (do-label-aux key))))))
...
(define (do-math-font f)
 (lambda ()
  (let ((%fluid-var-*math-font* f))
   (fluid-let ((*math-font* %fluid-var-*math-font*)) (tex2page-string (get-token))))))
...
(tex-def-prim "\\abstract"
 (lambda () (tex2page-string "\\quote") (tex2page-string "\\centerline{\\bf\\abstractname}\\par")))
...
(tex-def-prim "\\description"
 (lambda () (do-end-para) (set! *tabular-stack* (cons ':description *tabular-stack*))
  (emit "<dl><dt></dt><dd>")))

(tex-def-prim "\\displaymath" (lambda () (do-latex-env-as-image "displaymath" ':display)))

(tex-def-prim "\\document" probably-latex)

(tex-def-prim "\\documentclass" do-documentclass)

(tex-def-prim "\\dontuseimgforhtmlmath" (lambda () (tex-def-0arg "\\TZPmathtext" "1")))

(tex-def-prim "\\dontuseimgforhtmlmathdisplay" (lambda () (tex-def-0arg "\\TZPmathtext" "1")))
...
(tex-def-prim "\\enddescription"
 (lambda () (pop-tabular-stack ':description) (do-end-para) (emit "</dd></dl>") (do-noindent)))
...
(tex-def-prim "\\endhtmlimg" (lambda () (terror 'tex-def-prim "Unmatched \\endhtmlimg")))
...
(define (do-table/figure type) (do-end-para) (bgroup)
 ...
 (let
  ((tbl-tag
    (let
     ((%type 'string)
      (%ee (list *html-node-prefix* (if (eqv? type 'table) "tbl_" "fig_") (gen-temp-string))))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (tex-def-0arg "\\TIIPcurrentnodename" tbl-tag) (emit-anchor tbl-tag) (emit-newline)
  (emit "<div class=\"") (emit type) (emit " ") (emit *display-justification*)
  (emit "\"><table width=100%><tr><td class=") (emit *display-justification*) (emit ">")))
...
(tex-def-prim "\\ignorenextinputtimestamp"
 (lambda ()
  (cond ((not *inputting-boilerplate-p*) (set! *inputting-boilerplate-p* 0) *inputting-boilerplate-p*)
   (else false))))

(tex-def-prim "\\imgpreamble" do-img-preamble)

(tex-def-prim "\\IMGtabbing" (lambda () (do-latex-env-as-image "tabbing" ':display)))

(tex-def-prim "\\IMGtabular" (lambda () (do-latex-env-as-image "tabular" ':display)))
...
(tex-def-prim "\\noad" (lambda () (tex-def-0arg "\\TZPcolophondisablecredit" "1")))

(tex-def-prim "\\node" do-node)

(tex-def-prim "\\nonumber" do-nonumber)

(tex-def-prim "\\noslatexlikecomments" (lambda () (tex-def-0arg "\\TZPslatexcomments" "0")))

(tex-def-prim "\\notimestamp" (lambda () (tex-def-0arg "\\TZPcolophondisabletimestamp" "1")))
...
(tex-def-prim "\\schemebox" (lambda () (do-scm-slatex-lines "schemebox" false false)))

(tex-def-prim "\\schemeresponsebox" (lambda () (do-scm-slatex-lines "schemeresponsebox" false ':result)))
...
(tex-def-prim "\\slatexlikecomments" (lambda () (tex-def-0arg "\\TZPslatexcomments" "1")))
...
(tex-def-prim "\\textsc"
 (lambda ()
  (let ((%fluid-var-*in-small-caps-p* true))
   (fluid-let ((*in-small-caps-p* %fluid-var-*in-small-caps-p*)) (tex2page-string (get-group))))))
...
(define (do-verbatim-eplain)
 (let ((%fluid-var-*inside-eplain-verbatim-p* true) (%fluid-var-*catcodes* *catcodes*))
  (fluid-let
   ((*catcodes* %fluid-var-*catcodes*) (*inside-eplain-verbatim-p* %fluid-var-*inside-eplain-verbatim-p*))
   (catcode #\\ 11) (catcode *esc-char-verb* 0)
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (cond ((not *inside-eplain-verbatim-p*) (return)) (else false))
     (unless %loop-returned
      (let ((c (get-actual-char)))
       (cond ((not c) (terror 'do-verbatim-eplain "Eof inside verbatim")) (else false))
       (cond
        ((= (catcode c) **escape**) (toss-back-char c)
         (let ((x (get-ctl-seq)))
          (cond ((string=? x "\\ ") (emit " ")) (else (do-tex-ctl-seq-completely x)))))
        ((char=? c #\space) (emit "&#xa0;")) ((char=? c #\newline) (emit "<br>") (emit-newline))
        (else (emit-html-char c)))))
     (if %loop-returned %loop-result (%loop)))))))

(define (do-verbatim) (if (eqv? *tex-format* ':latex) (do-verbatim-latex "verbatim") (do-verbatim-eplain)))
...
(define (do-writenumberedcontentsline)
 (let ((toc (get-peeled-group)))
  (cond ((not (string=? toc "toc")) (terror 'do-writenumberedcontentsline "only #1=toc supported"))
   (else false))
...
(define (do-latex-intext-math)
 (do-intext-math (let ((o (open-output-string))) (dump-till-ctl-seq "\\)" o) (get-output-string o))))

(tex-def-prim "\\(" do-latex-intext-math)

(define (do-latex-display-math)
 (do-display-math (let ((o (open-output-string))) (dump-till-ctl-seq "\\]" o) (get-output-string o))))
...
(tex-def-prim "\\>"
 (lambda ()
  (cond ((and (not (null? *tabular-stack*)) (eqv? (car *tabular-stack*) ':tabbing)) (emit-nbsp 3))
   (else false))))
...
(let ((tex *tex-logo*))
 (let
  ((bib
    (let
     ((%type 'string)
      (%ee (list "B" "<span style=\"" "text-transform: uppercase" "\"><small>ib</small></span>")))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
   ...
   (let
    ((latex
      (let
       ((%type 'string)
        (%ee
         (list "L" "<span style=\"" "position: relative; " "bottom: 0.3ex; " "margin-left: -0.36em; "
          "margin-right: -0.15em; " "text-transform: uppercase" "\"><small>a</small></span>" tex)))
       (let ((%res (if (eq? %type 'string) "" null)))
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
          (let ((%a (car %ee)))
    ...
    (let
     ((xe
       (let
        ((%type 'string)
         (%ee
          (list "X" "<span style=\"" "text-transform: uppercase; " "position: relative; " "top: 0.5ex; "
           "margin-left: -0.125em; " "margin-right: -0.1667em" "\">&#x1dd;</span>")))
        (let ((%res (if (eq? %type 'string) "" null)))
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
           (let ((%a (car %ee)))
      ...
      (let
       ((ii/e
         (let
          ((%type 'string)
           (%ee
            (list "<span style=\"" "margin-left: .05em" "\">2<span>" "<span style=\""
             "position: relative; " "top: .5ex" "\">&#x3b5;</span>")))
          (let ((%res (if (eq? %type 'string) "" null)))
           (let %concatenate-loop ((%ee %ee))
            (if (null? %ee) %res
             (let ((%a (car %ee)))
...
(tex-def-prim "\\ProvidesFile" (lambda () (get-group) (get-bracketed-text-if-any)))

(tex-def-prim "\\DeclareGraphicsRule" (lambda () (get-group) (get-group) (get-group) (get-group)))
...
(tex-def-prim "\\usepackage" (lambda () (get-bracketed-text-if-any) (get-group) (probably-latex)))
...
(define (tex2page tex-file) (cond ((not (= *write-log-index* 0)) (newline)) (else false))
 (cond ((or (not tex-file) (string=? tex-file "")) (set! tex-file "--missing-arg") tex-file) (else false))
 (let
  ((%fluid-var-*afterassignment* false) (%fluid-var-*afterbye* null) (%fluid-var-*afterpar* null)
   ...
   (%fluid-var-*basic-style* "") (%fluid-var-*bib-aux-stream* false) (%fluid-var-*bibitem-num* 0)
   ...
   (%fluid-var-*equation-number* false) (%fluid-var-*equation-numbered-p* true)
   (%fluid-var-*equation-position* 0) (%fluid-var-*esc-char-verb* #\|)
   (%fluid-var-*eval-for-tex-only-p* false) (%fluid-var-*external-label-tables* (make-table ':test equal?))
   (%fluid-var-*footnote-list* null) (%fluid-var-*footnote-sym* 0)
   (%fluid-var-*global-texframe* (make-texframe* ':catcodes *catcodes*))
   (%fluid-var-*graphics-file-extensions* '(".eps")) (%fluid-var-*html* false)
   (%fluid-var-*html-head* null) (%fluid-var-*html-only* 0) (%fluid-var-*html-page* false)
   (%fluid-var-*html-page-count* 0) (%fluid-var-*img-file-count* 0) (%fluid-var-*img-file-tally* 0)
   (%fluid-var-*imgdef-file-count* 0) (%fluid-var-*imgpreamble* "")
   ...
   (%fluid-var-*in-small-caps-p* false) (%fluid-var-*includeonly-list* true) (%fluid-var-*index-count* 0)
   (%fluid-var-*index-page* false) (%fluid-var-*index-stream* false)
   ...
   (%fluid-var-*jobname* "texput") (%fluid-var-*label-stream* false) (%fluid-var-*label-source* false)
   (%fluid-var-*label-table* (make-table ':test equal?)) (%fluid-var-*last-modification-time* false)
   (%fluid-var-*last-page-number* -1) (%fluid-var-*latex-probability* 0) (%fluid-var-*ligatures-p* true)
   (%fluid-var-*loading-external-labels-p* false) (%fluid-var-*log-file* false)
   (%fluid-var-*log-stream* false) (%fluid-var-*main-tex-file* false) (%fluid-var-*math-delim-left* false)
   (%fluid-var-*math-delim-right* false) (%fluid-var-*math-height* 0) (%fluid-var-*math-mode-p* false)
   (%fluid-var-*mfpic-file-num* false) (%fluid-var-*mfpic-file-stem* false)
   (%fluid-var-*mfpic-stream* false) (%fluid-var-*missing-eps-files* null)
   (%fluid-var-*missing-pieces* null) (%fluid-var-*mp-files* null) (%fluid-var-*not-processing-p* false)
   (%fluid-var-*opmac-active-tt-char* false) (%fluid-var-*opmac-index-sub-table* false)
   (%fluid-var-*opmac-list-style* #\o) (%fluid-var-*opmac-nonum-p* false)
   (%fluid-var-*opmac-notoc-p* false) (%fluid-var-*opmac-verbinput-table* (make-table ':test equal?))
   (%fluid-var-*outer-p* true) (%fluid-var-*output-streams* (make-table))
   (%fluid-var-*outputting-external-title-p* false) (%fluid-var-*outputting-to-non-html-p* false)
   ...
   (%fluid-var-*redirect-delay* false) (%fluid-var-*redirect-url* false) (%fluid-var-*reg-num-count* 255)
   (%fluid-var-*scm-builtins* false) (%fluid-var-*scm-dribbling-p* false) (%fluid-var-*scm-keywords* false)
   (%fluid-var-*scm-special-symbols* false) (%fluid-var-*scm-variables* false) (%fluid-var-*scripts* null)
   (%fluid-var-*section-counter-dependencies* (make-table)) (%fluid-var-*section-counters* (make-table))
   (%fluid-var-*slatex-math-escape* false) (%fluid-var-*source-changed-since-last-run-p* false)
   (%fluid-var-*start-time* (current-seconds)) (%fluid-var-*stylesheets* null)
   (%fluid-var-*subjobname* false) (%fluid-var-*tabular-stack* null) (%fluid-var-*temp-string-count* 0)
   (%fluid-var-*temporarily-use-utf8-for-math-p* false) (%fluid-var-*tex-env* null)
   (%fluid-var-*tex-format* ':plain) (%fluid-var-*tex-if-stack* null)
   (%fluid-var-*tex-output-format* false)
   (%fluid-var-*tex2page-inputs* (string=split (getenv "TEX2PAGEINPUTS") *path-separator*))
  ...
  (fluid-let
   ((*write-log-possible-break-p* %fluid-var-*write-log-possible-break-p*)
    (*write-log-index* %fluid-var-*write-log-index*) (*verb-written-files* %fluid-var-*verb-written-files*)
    (*verb-visible-space-p* %fluid-var-*verb-visible-space-p*) (*verb-stream* %fluid-var-*verb-stream*)
    ...
    (*tex2page-inputs* %fluid-var-*tex2page-inputs*) (*tex-output-format* %fluid-var-*tex-output-format*)
    (*tex-if-stack* %fluid-var-*tex-if-stack*) (*tex-format* %fluid-var-*tex-format*)
    (*tex-env* %fluid-var-*tex-env*)
    (*temporarily-use-utf8-for-math-p* %fluid-var-*temporarily-use-utf8-for-math-p*)
    (*temp-string-count* %fluid-var-*temp-string-count*) (*tabular-stack* %fluid-var-*tabular-stack*)
    (*subjobname* %fluid-var-*subjobname*) (*stylesheets* %fluid-var-*stylesheets*)
    (*start-time* %fluid-var-*start-time*)
    (*source-changed-since-last-run-p* %fluid-var-*source-changed-since-last-run-p*)
    ...
    (*scm-special-symbols* %fluid-var-*scm-special-symbols*) (*scm-keywords* %fluid-var-*scm-keywords*)
    (*scm-dribbling-p* %fluid-var-*scm-dribbling-p*) (*scm-builtins* %fluid-var-*scm-builtins*)
    (*reg-num-count* %fluid-var-*reg-num-count*) (*redirect-url* %fluid-var-*redirect-url*)
    ...
    (*output-streams* %fluid-var-*output-streams*) (*outer-p* %fluid-var-*outer-p*)
    (*opmac-verbinput-table* %fluid-var-*opmac-verbinput-table*)
    (*opmac-notoc-p* %fluid-var-*opmac-notoc-p*) (*opmac-nonum-p* %fluid-var-*opmac-nonum-p*)
    ...
    (*missing-pieces* %fluid-var-*missing-pieces*) (*missing-eps-files* %fluid-var-*missing-eps-files*)
    (*mfpic-stream* %fluid-var-*mfpic-stream*) (*mfpic-file-stem* %fluid-var-*mfpic-file-stem*)
    (*mfpic-file-num* %fluid-var-*mfpic-file-num*) (*math-mode-p* %fluid-var-*math-mode-p*)
    (*math-height* %fluid-var-*math-height*) (*math-delim-right* %fluid-var-*math-delim-right*)
    ...
    (*last-modification-time* %fluid-var-*last-modification-time*) (*label-table* %fluid-var-*label-table*)
    (*label-source* %fluid-var-*label-source*) (*label-stream* %fluid-var-*label-stream*)
    (*jobname* %fluid-var-*jobname*) (*inside-appendix-p* %fluid-var-*inside-appendix-p*)
    ...
    (*includeonly-list* %fluid-var-*includeonly-list*) (*in-small-caps-p* %fluid-var-*in-small-caps-p*)
    (*in-para-p* %fluid-var-*in-para-p*) (*in-display-math-p* %fluid-var-*in-display-math-p*)
    (*in-alltt-p* %fluid-var-*in-alltt-p*) (*imgpreamble-inferred* %fluid-var-*imgpreamble-inferred*)
    (*imgpreamble* %fluid-var-*imgpreamble*) (*imgdef-file-count* %fluid-var-*imgdef-file-count*)
    (*img-file-tally* %fluid-var-*img-file-tally*) (*img-file-count* %fluid-var-*img-file-count*)
    (*html-page-count* %fluid-var-*html-page-count*) (*html-page* %fluid-var-*html-page*)
    (*html-only* %fluid-var-*html-only*) (*html-head* %fluid-var-*html-head*) (*html* %fluid-var-*html*)
    (*graphics-file-extensions* %fluid-var-*graphics-file-extensions*)
    (*global-texframe* %fluid-var-*global-texframe*) (*footnote-sym* %fluid-var-*footnote-sym*)
    (*footnote-list* %fluid-var-*footnote-list*)
    (*external-label-tables* %fluid-var-*external-label-tables*)
    (*eval-for-tex-only-p* %fluid-var-*eval-for-tex-only-p*) (*esc-char-verb* %fluid-var-*esc-char-verb*)
    (*equation-position* %fluid-var-*equation-position*)
    (*equation-numbered-p* %fluid-var-*equation-numbered-p*)
    (*equation-number* %fluid-var-*equation-number*) (*dumping-nontex-p* %fluid-var-*dumping-nontex-p*)
    ...
    (*basic-style* %fluid-var-*basic-style*) (*aux-stream* %fluid-var-*aux-stream*)
    (*aux-dir/* %fluid-var-*aux-dir/*) (*aux-dir* %fluid-var-*aux-dir*) (*afterpar* %fluid-var-*afterpar*)
    ...
    (*main-tex-file* (set! *subjobname* *jobname*)
     ...
     (let ((%fluid-var-*html-only* (add1 *html-only*)))
      (fluid-let ((*html-only* %fluid-var-*html-only*))
       (tex2page-file-if-exists (file-in-home ".tex2page.t2p")) (tex2page-file-if-exists ".tex2page.t2p")
       (let
        ((f
          (actual-tex-filename
     ...
     (cond ((not (eqv? (tex2page-file *main-tex-file*) ':encountered-bye)) (insert-missing-end))
      (else false))
     (do-bye))
