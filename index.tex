...
files (e.g., \p{btxmac.tex}, \p{eplain.tex}~\cite{eplain}, \p{opmac.tex}~\cite{opmac},
\p{epsf.tex}).
...
original, more sophisticated \p{\vrule}-based definition as seen
by \TeX.
...
\ytem{i} The link in the index goes directly to the spot where
...
\ytem{ii} An index entry could link to the same HTML page
...
\index{TZPcolophondisabletimestamp@\p{\TZPcolophondisabletimestamp}}
...
\index{TZPcolophondisablecredit@\p{\TZPcolophondisablecredit}}
...
\index{TZPcolophondisableweblink@\p{\TZPcolophondisableweblink}}
...
\xrdef{mathflag}
\index{TZPmathtext@\p{\TZPmathtext}}
...
graphic, and then Ghostscript~\cite{gs} and either \ytem{i} the
NetPBM library~\cite{netpbm}, or \ytem{ii} the ImageMagick
...
\index{TZPrightjustify@\p{\TZPrightjustify}}
...
\p{\input opmac}. The flag need be set only if somehow the
loading of \p{opmac} occurs in a \TeX-only portion of your
document.
...
environment, the \p{\path} macro from \p{path.sty},
Eplain’s \p{\verbatim}, and OPmac’s
\p{\activettchar}.
...
\ytem{i} background punctuation;

\ytem{ii} self-evaluating atoms (numbers, booleans, characters, strings);

\ytem{iii} syntactic keywords;

\ytem{iv} builtin variables;

\ytem{v} global or special variables, viz., identifiers that begin and end with an asterisk;

\ytem{vi} other variables; and
...
you can either \ytem{i} have  MetaPost convert the spec into EPS and
thence to PDF using a utility like \p{epstopdf}; or \ytem{ii}
have MetaPost directly convert the spec to PNG. Let’s start with a MetaPost file
\urlh{mpexample.html}{\p{lambda.mp}}.

\ytem{i} At the OS shell, type
...
\ytem{ii} To generate PNG, in \p{lambda.mp} set
...
\ytem{i} Setting  \p{\tracingcommands=1} tells \TeX2page to log all calls
to atomic commands.

\ytem{ii} Setting  \p{\tracingmacros=1} tells
...
\ytem{iii} The command \p{\tracingall} turns {\em on} both
\p{\tracingcommands} and \p{\tracingmacros}.

\index{errmessage@\p{\errmessage}}

\ytem{iv} The command \p{\errmessage} can be used to generate
...
\ytem{i} \p{|meta[jobname].hdir} in the working directory, i.e.,
a file with the same basename as the input document but with
extension \p{.hdir}.  For \p{story.tex}, this would
be \p{story.hdir}.

\ytem{ii} \p{.tex2page.hdir} in the working directory.

\ytem{iii} \p{.tex2page.hdir} in the user’s \p{HOME} directory.
...
\ytem{i} Type \p{./configure --help} at your OS command line
to get the list of dialects supported.
If your dialect \p{|meta[D]} is one of them,

\ytem{ii} Type  \p{./configure --dialect=|meta[D]}
