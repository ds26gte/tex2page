...
(define (decode-universal-time s)
 (let ((ht (and s (seconds->date s))))
  (cond
   (ht (list false (date-minute ht) (date-hour ht) (date-day ht) (date-month ht) (date-year ht)))
   ...
   (string-append (vector-ref *week-day-names* dow) ", " (vector-ref *short-month-names* (- mo 1))
    ...
    (if tz
     (string-append ", " (let ((h (modulo h 12))) (number->string (if (= h 0) 12 h))) ":"
      (if (< m 10) "0" "") (number->string m) " " (if (<= 0 h 11) "a" "p") "m UTC"
      (if (> tz 0) "+" "−") (number->string (abs tz)))
     "")))))
...
(define list-position
 (lambda (x s)
  (let loop ((s s) (i 0))
   (cond ((null? s) false) ((eq? (car s) x) i) (else (loop (cdr s) (+ i 1)))))))
     ...
     (lambda (s . ff)
      (let ((s-s (symbol->string s)) (n (length ff)))
       (let* ((n+1 (+ n 1)) (vv (make-vector n+1)))
        (let loop ((i 1) (ff ff))
         (if (< i n+1)
          (let ((f (car ff))) (vector-set! vv i (if (pair? f) (cadr f) (not 't)))
           (loop (+ i 1) (cdr ff)))
          0))
        (let*
         ((ff-without-colons (map (lambda (f) (symbol->string (if (pair? f) (car f) f))) ff))
          (ff-with-colons
           (map (lambda (f) (string->symbol (string-append ":" f))) ff-without-colons)))
         `(begin
              ...
              (if (>= i n+1) procs
               (loop (+ i 1)
                (let* ((f-s (list-ref ff-without-colons (- i 1))))
                 (cons
                  `(define ,(string->symbol (string-append s-s "-" f-s))
...
(define (lassoc k al equ?)
 (let loop ((al al))
  (if (null? al) false (let ((c (car al))) (if (equ? (car c) k) c (loop (cdr al)))))))

(define-syntax rassoc
 (lambda (%so)
  (datum->syntax %so
   (let ((%so-d (syntax->datum %so)))
    (apply (lambda (k al . z) `(scheme-rassoc ,k ,al ,(if (null? z) 'eqv? (cadr z))))
     (cdr %so-d))))))

(define (scheme-rassoc k al equ)
 (let loop ((al al))
  (if (null? al) false (let ((c (car al))) (if (equ (cdr c) k) c (loop (cdr al)))))))
...
(define (number-to-roman n . upcase?) (set! upcase? (and (pair? upcase?) (car upcase?)))
 ...
 (let
  ((roman-digits
    '((1000 #\m 100) (500 #\d 100) (100 #\c 10) (50 #\l 10) (10 #\x 1) (5 #\v 1) (1 #\i 0)))
...
(define (read-6hex i)
 (let*
  ...
  (string-append "#"
   (case (string-length htmlcolor) ((1) "00000") ((2) "0000") ((3) "000") ((4) "00") ((5) "0")
    (else ""))
   htmlcolor)))
...
(define *tex2page-copyright-notice*
 (let
  ((%type 'string)
   (%ee (list "Copyright (C) 1997-" (substring *tex2page-version* 0 4) " Dorai Sitaram")))
...
(define *catcodes*
 (list (cons #\\ 0) (cons #\space 10) (cons #\% 14) (cons (integer->char 0) 9) (cons #\return 5)
  (cons #\newline 5) (cons #\{ 1) (cons #\} 2) (cons #\$ 3) (cons #\& 4) (cons #\# 6) (cons #\^ 7)
  (cons #\_ 8) (cons #\tab 10) (cons #\~ 13)))
        ...
        ((not (null? ll))
         (let ((border "__________________________..."))
          (let ((only-1-p (= (length ll) 1)))
           (let ((nf (caar ll)))
            (let ((ll (reverse ll)))
             (let ((n1 (caar ll))) (write-log "Likely error context: ")
              (write-log *current-source-file*) (write-log ", line")
              (cond ((not only-1-p) (write-log "s")) (else false)) (write-log " ") (write-log n1)
              (cond ((not only-1-p) (write-log "-") (write-log nf)) (else false)) (write-log ":")
      ...
      ((and c (char-ci=? c #\e))
       (let ((texedit-string (retrieve-env "TEXEDIT")))
        ...
        (cond
         ...
         (else false))
        (cond
         ((not texedit-string) (display "Ill-formed TEXEDIT; using EDITOR.") (newline)
          (cond
...
(define (terror where . args) (write-log ':separation-newline) (write-log "! ")
 (for-each write-log args) (write-log ':separation-newline) (write-log *current-source-file*)
...
(define (snoop-actual-char)
 (let ((c (snoop-char)))
  (cond ((not c) c) ((invisible-space-p c) (get-char) (snoop-actual-char))
...
(define (get-actual-char)
 (let ((c (get-char)))
  (cond ((not c) c) ((invisible-space-p c) (get-actual-char))
   ((char=? c #\return)
...
(define (ignorespaces . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (newlines 1.5))
       ...
       (cond ((not c) (return))
        ...
        ((char=? c #\newline)
         (if newline-active-p (return)
          (case newlines ((0) (return)) ((1) (get-actual-char) (return))
           ((1.5)
            (case num-newlines-read
             ((0) (get-actual-char) (set! num-newlines-read (+ num-newlines-read 1))
              num-newlines-read)
             (else (toss-back-char #\newline) (return))))
...
(define (get-ctl-seq) (ignorespaces 1.5)
 (let ((bs (get-actual-char)))
  (cond ((not (esc-char-p bs)) (terror 'get-ctl-seq "Missing control sequence (" bs ")"))
   (else false))
  (let ((c (get-char)))
   (cond ((not c) "\\ ") ((invisible-space-p c) "\\ ")
         ...
         (reverse
          (let ((s (list c #\\)))
           (let*
            ...
            (let %loop ()
             (let ((c (snoop-char)))
              (cond ((not c) (return s)) ((invisible-space-p c) (return s))
               ((char-tex-alphabetic-p c) (get-char) (set! s (cons c s)) s)
               (else (ignorespaces 0) (return s))))
             (if %loop-returned %loop-result (%loop)))))))))
      (let ((%res (if (eq? %type 'string) "" null)))
       (let %concatenate-loop ((%ee %ee))
        (if (null? %ee) %res
...
(define (get-group-as-reversed-chars) (ignorespaces 1.5)
    ...
    (let %loop ()
     (let ((c (get-actual-char)))
      ...
      (cond (escape-p (set! s (cons c s)) (set! escape-p false) escape-p)
       ...
       ((char=? c #\{) (set! s (cons c s)) (set! nesting (+ nesting 1)) (set! escape-p false)
        escape-p)
...
(define (get-bracketed-text-if-any) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (if (or (not c) (not (char=? c #\[))) false
   (begin (get-actual-char)
    (let
          ...
          (let*
           ...
           (let %loop ()
            (let ((c (get-actual-char)))
             ...
             (cond (escape-p (set! s (cons c s)) (set! escape-p false) escape-p)
              ((esc-char-p c) (set! s (cons c s)) (set! escape-p true) escape-p)
...
(define (eat-alphanumeric-string) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (if (char=? c #\") (begin (get-actual-char) (eat-till-char #\"))
   (let*
    ...
    (let %loop () (set! c (snoop-actual-char))
     (cond ((not (or (char-alphabetic? c) (char-numeric? c) (member c '(#\:)))) (return))
      (else false))
...
(define (get-filename . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (bracedp false))
  (when (< 0 %lambda-rest-arg-len) (set! bracedp (list-ref %lambda-rest-arg 0))) (ignorespaces 1.5)
  (cond
   (bracedp
    (let ((c (snoop-actual-char)))
        ...
        (let*
         ...
         (let %loop ()
          (let ((c (snoop-actual-char)))
           (cond ((not c) (return s))
            ((and (not bracedp)
              (or (char-whitespace? c) (comment-char-p c) (member c *filename-delims*)))
             (ignorespaces 0) (return s))
...
(define (get-filename-possibly-braced) (ignorespaces 1.5)
 (let ((c (snoop-actual-char))) (get-filename (and (char? c) (char=? c #\{)))))

(define (get-word) (ignorespaces 1.5)
 (let
  ((%type 'string)
   (%ee
    (list
      ...
      (let ((s null))
       (let*
        ...
        (let %loop ()
         (let ((c (snoop-actual-char)))
          (cond ((not c) (return s))
           ((or (char-whitespace? c) (comment-char-p c) (esc-char-p c)) (return s))
           (else (get-actual-char) (set! s (cons c s)) s)))
         (if %loop-returned %loop-result (%loop)))))))))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
...
(define (get-integer base) (ignorespaces 1.5)
 (let ((s null) (c false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return
   ...
   (let %loop () (set! c (snoop-actual-char)) (cond ((not c) (return)) (else false))
    (unless %loop-returned
     (cond
      ((not (or (char-numeric? c) (and (= base 16) (char-alphabetic? c)))) (ignorespaces 1.5)
       (return))
      (else false)))
...
(define (get-real) (ignorespaces 1.5)
...
(define (get-number-or-false) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (cond ((esc-char-p c) (get-number-corresp-to-ctl-seq (get-ctl-seq)))
   ...
   ((char=? c #\`) (get-actual-char) (ignorespaces 1.5)
    (char->integer
     (if (esc-char-p (snoop-actual-char)) (string-ref (get-ctl-seq) 1) (get-actual-char))))
...
(define (get-url) (ignorespaces 1.5)
 (let ((c (get-actual-char)))
            ...
            (cond
             ((comment-char-p c)
              (let ((c1 (snoop-actual-char)))
               (if (and (char? c1) (char-whitespace? c1)) (ignorespaces 1.5)
                (begin (set! s (cons c s)) s))))
             ((char=? c #\{) (set! nesting (+ nesting 1)) (set! s (cons c s)) s)
             ((char=? c #\})
              (cond ((= nesting 0) (return s))
               (else (set! nesting (- nesting 1)) (set! s (cons c s)) s)))
             (else (set! s (cons c s)) s)))
           (if %loop-returned %loop-result (%loop)))))))))
    (let ((%res (if (eq? %type 'string) "" null)))
     (let %concatenate-loop ((%ee %ee))
...
(define (get-csv closing-delim) (ignorespaces 1.5)
 (let
  ((rev-lbl
    (let ((s null))
     (let*
      ...
      (let %loop ()
       (let ((c (get-actual-char)))
        (cond
         ((not c) (terror 'get-csv "Runaway argument of \\cite, " "\\nocite, \\expandhtmlindex?"))
         (else false))
...
(define (get-raw-token/is) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (cond ((not c) c) ((esc-char-p c) (get-ctl-seq))
...
(define (get-token) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (cond ((not c) c) ((esc-char-p c) (get-ctl-seq)) ((char=? c #\{) (get-group))
...
(define (eat-word word) (ignorespaces 1.5)
 (let ((r null))
  (let
   ((%dotimes-n
     (let ((%length-arg word)) ((if (string? %length-arg) string-length length) %length-arg)))
    (i 0))
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
       ...
       (let*
        ((%loop-returned false) (%loop-result 0)
         (return
          (lambda %args (set! %loop-returned true)
           (set! %loop-result (and (pair? %args) (car %args))))))
        (let %loop () (ignorespaces 1.5)
         (let ((c (snoop-actual-char)))
          (cond ((not c) (return)) ((and (esc-char-p c) firstp) (get-ctl-seq) (return))
...
(define (eat-integer)
 (let ((%fluid-var-*not-processing-p* true))
  (fluid-let ((*not-processing-p* %fluid-var-*not-processing-p*)) (ignorespaces 1.5) (get-equal-sign)
   (get-number))))
...
(define (catcode c . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (n false) (globalp false))
  ...
  (cond
   ((not n)
    (cond ((begin (set! *it* (assoc c *catcodes*)) *it*) (cdr *it*)) ((char-alphabetic? c) 11)
     (else 12)))
...
(define (egroup) (cond ((null? *tex-env*) (terror 'egroup "Too many }'s")) (else false))
 ...
 (cond
  ((begin (set! *it* (top-texframe)) *it*) (set! *catcodes* (texframe*-catcodes *it*)) *catcodes*)
  ...
  (add-postlude-to-top-frame
   (lambda ()
    (let ((res (get-output-string *html*)))
      ...
      ((or *math-delim-left* *math-delim-right*)
       ...
       (set! res
        (let
         ((%type 'string)
          (%ee
           (list "<table><tr><td>" (tex-math-delim-string *math-delim-left*) "</td><td>" res
            "</td><td>" (tex-math-delim-string *math-delim-right*) "</td></tr></table>")))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
     ...
     (set! *math-height* (+ old-math-height *math-height*)) (pop-tabular-stack ':mathbox)
     (emit "</td><td>") (emit res) (emit "</td><td>"))))))

(define (do-math-left) (ignorespaces 1.5)
 (cond
...
(define (do-math-right) (ignorespaces 1.5)
 (cond
  ((and *in-display-math-p* (not *math-script-mode-p*))
   (let ((s (get-token)))
...
(define (cleanse-tdef d) (set!tdef*-argpat d null) (tdef*-argpat d) (set!tdef*-expansion d "")
 ...
 (tdef*-thunk d) (set!tdef*-prim d false) (tdef*-prim d) (set!tdef*-defer d false) (tdef*-defer d)
...
(define (tex-def name argpat expansion optarg thunk prim defer frame)
 ...
 (let ((d (ensure-def name frame))) (set!tdef*-argpat d argpat) (set!tdef*-expansion d expansion)
  (set!tdef*-optarg d optarg) (set!tdef*-thunk d thunk) (set!tdef*-prim d prim)
  (set!tdef*-defer d defer) (set!tdef*-catcodes d *catcodes*) (tdef*-catcodes d))
...
(define (do-undefcsactive) (ignorespaces 1.5) (deactivate-cdef (string-ref (get-ctl-seq) 1)))

(define (do-catcode)
 (let ((c (get-tex-char-spec))) (let ((val (begin (get-equal-sign) (get-number)))) (catcode c val))))

(define (do-opmac-activettchar) (ignorespaces 1.5)
...
(define (do-global) (ignorespaces 1.5)
...
(define (output-external-title)
 (let ((%fluid-var-*outputting-external-title-p* true))
  (fluid-let ((*outputting-external-title-p* %fluid-var-*outputting-external-title-p*))
   (emit "<title>") (emit-newline) (emit (or *title* *jobname*)) (emit-newline) (emit "</title>")
   (emit-newline))))
...
(define (do-subject) (tex-gdef-0arg "\\TIIPtitleused" "1") (do-end-para) (ignorespaces 1.5)
 (let
  ((title
    (let ((c (snoop-actual-char)))
...
(define (do-today)
 (let ((m (get-gcount "\\month")))
  (if (= m 0) (emit "[today]")
   (begin (emit (vector-ref *month-names* (sub1 m))) (emit " ") (emit (get-gcount "\\day"))
    (emit ", ") (emit (get-gcount "\\year"))))))
...
(define (do-end-para)
 (cond
  (*in-para-p* (cond (*use-closing-p-tag-p* (emit "</p>")) (else false))
   (cond
    ((not (null? *afterpar*)) (for-each (lambda (ap) (ap)) (reverse *afterpar*))
     (set! *afterpar* null) *afterpar*)
    (else false))
   (emit-newline) (set! *in-para-p* false) *in-para-p*)
  (else false)))

(define (do-para) (do-end-para)
 (let ((in-table-p (and (not (null? *tabular-stack*)) (member (car *tabular-stack*) '(:block)))))
...
(define (do-indent)
 (let ((parindent (sp-to-pixels (find-dimen "\\parindent")))) (emit "<span style=\"margin-left: ")
  (emit parindent) (emit "pt\"></span>")))

(define (do-para-nopadding) (do-end-para) (emit-newline) (emit "<p class=nopadding>")
 (set! *in-para-p* true) *in-para-p*)

(define (do-maketitle) (do-end-para) (bgroup)
 (tex2page-string
  (let
   ((%type 'string) (%ee (list "\\let\\\\\\break" "\\let\\and\\break" "\\let\\thanks\\symfootnote")))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
 ...
 (tex2page-string "\\TIIPauthor") (do-para) (tex2page-string "\\TIIPdate") (do-end-para)
 (emit "</div>") (emit-newline) (egroup) (do-para))

(define (do-inputcss) (ignorespaces 1.5)
 (let ((f (get-filename-possibly-braced)))
  (cond ((null? *stylesheets*) (flag-missing-piece ':stylesheets)) (else false))
  (write-aux `(!stylesheet ,f))))

(define (do-csname) (ignorespaces 1.5)
 (let ((r null))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return
    ...
    (let ((c (snoop-actual-char)))
     (cond
      ((esc-char-p c)
       (let ((x (get-ctl-seq)))
        (cond
         ((string=? x "\\endcsname") (toss-back-char #\}) (for-each toss-back-string r)
          (toss-back-char c) (toss-back-char #\{) (toss-back-string "TIIPcsname") (toss-back-char c)
          (return))
...
(define (link-stylesheets)
 (let
  ((link-it
    (lambda (css) (emit "<link rel=\"stylesheet\" href=\"") (emit css) (emit "\" />")
     (emit-newline))))
...
(define (link-scripts)
 (let
  ((link-it (lambda (jsf) (emit "<script src=\"") (emit jsf) (emit "\"></script>") (emit-newline))))
...
(define (increment-section-counter seclvl nonum-p)
 ...
 (table-for-each
  (lambda (k v)
   (cond
    ((and (> k seclvl) (> k 0)) (table-put! k *section-counters* 0) (table-get k *section-counters*))
    (else false)))
  *section-counters*)
     ...
     (let*
      ...
      (let %loop () (cond ((> i seclvl) (return r)) (else false))
       (unless %loop-returned
        (set! r
         (let
          ((%type 'string) (%ee (list r "." (write-to-string (table-get i *section-counters* 0)))))
          (let ((%res (if (eq? %type 'string) "" null)))
           (let %concatenate-loop ((%ee %ee))
            (if (null? %ee) %res
             (let ((%a (car %ee)))
       ...
       (let*
        ...
        (let %loop ()
         (cond ((> i+3 n-9) (return k))
          ((string=? (substring s i i+3) "sub") (set! i i+3) (set! i+3 (+ i+3 3)) (set! k (add1 k))
           k)
          (else (return false)))
         (if %loop-returned %loop-result (%loop))))))
...
(define (do-heading-help seclvl starred-p nonum-p notoc-p lbl-val header)
 ...
 (cond
  ...
  (else false))
 (cond ((and (= seclvl 1) (tex2page-flag-boolean "\\TZPslides")) (do-eject)) (else false))
   ...
   ((lbl
     (let
      ...
      (let ((%res (if (eq? %type 'string) "" null)))
       ...
       %res))))
   (cond
    ((not false) (tex-def-0arg "\\TIIPcurrentnodename" lbl) (tex-def-0arg "\\@currentlabel" lbl-val))
    (else false))
   (do-end-para) (emit-anchor lbl) (emit-newline) (ignorespaces 2) (emit "<h") (emit htmlnum)
   ...
   (let
    ((write-to-toc-p
      (and (not notoc-p) *toc-page*
       (not (and (eqv? *tex-format* ':latex) (string=? header "Contents"))))))
    (cond
    ...
    (cond
     ...
     (else false))
    (cond
     ((not (or (and (eqv? *tex-format* ':latex) (<= seclvl 0)) nonum-p)) (emit lbl-val)
      (emit-nbsp 2))
     (else false))
...
(define (do-addcontentsline)
 (let ((toc (get-peeled-group)))
  (cond ((not (string=? toc "toc")) (terror 'do-addcontentsline "only #1=toc supported"))
   (else false))
   ...
   (let %loop ()
    (let ((c (get-actual-char)))
     (cond
      ...
      (newline-p
       (cond
        ((not (char-whitespace? c)) (set! r (cons #\space r)) (set! r (cons c r))
         (set! newline-p false) newline-p)
        (else false)))
      ((char=? c #\newline) (set! newline-p true) newline-p)
      (else (set! r (cons c r)) (set! newline-p false) newline-p)))
    (if %loop-returned %loop-result (%loop))))))

(define (do-beginsection) (ignorespaces 1.5)
 (let
  ((header
    (let ((%fluid-var-*tabular-stack* (list ':header)))
     (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*))
...
(define (do-beginchapter) (ignorespaces 1.5)
 (let ((chapno (tex-string-to-html-string (get-till-char #\space))))
  (let
   ((header
     (begin (ignorespaces 1.5)
      (let ((%fluid-var-*tabular-stack* (list ':header)))
       (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*))
        (tex-string-to-html-string (get-till-par)))))))
   (tex-gdef-0arg "\\chapno" chapno) (tex-gdef-count "\\subsecno" 0)
...
(define (do-end-table/figure type)
 ...
 (do-end-para) (emit "</td></tr>") (emit "</table>") (emit "</div>") (pop-tabular-stack type)
 (egroup) (do-para))
...
(define (do-caption) (do-end-para)
 (let
  ...
  (let
   ...
   (let
    ((type
      (cond ((and (not i-fig) (not i-tbl)) (terror 'do-caption "Mislaid \\caption"))
       ((not i-fig) ':table) ((not i-tbl) ':figure) ((< i-fig i-tbl) ':figure)
       ((< i-tbl i-fig) ':table) (else (terror 'do-caption "cant happen")))))
...
(define (do-marginnote) (emit "<span class=marginnote>") (tex2page-string (get-group))
 (emit "</span>"))
...
(define (do-end-equation) (do-end-para) (emit "</td>")
 (cond
    ...
    (let*
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true)
        (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop () (if (>= i %dotimes-n) (return) false) (unless %loop-returned (ignorespaces 1.5))
      (unless %loop-returned
       (let ((c (snoop-actual-char)))
        (cond
 ...
 (let
  ...
  (fluid-let
   ((*imgdef-file-count* %fluid-var-*imgdef-file-count*)
    (*img-file-count* %fluid-var-*img-file-count*) (*subjobname* %fluid-var-*subjobname*))
   (cond
    ((eqv? *tex-format* ':latex)
     (tex2page-string
      (if *using-chapters-p* "\\chapter*{\\contentsname}" "\\section*{\\contentsname}")))
    (else false))
   (emit-anchor
         ...
         (let ((secnum (tocentry*-number x)))
          (let ((seclabel (tocentry*-label x)))
           (let
            ...
            (cond
             (subentries-p (if *tex-like-layout-p* (do-bigskip ':medskip) (do-para)) (do-noindent)
              (emit "<b>") (emit-newline))
             (else false))
            (indent-n-levels lvl)
            (emit-anchor
            ...
            (cond
             ((not (or (string=? secnum "") (string=? secnum "IGNORE"))) (emit secnum) (emit-nbsp 2))
             (else false))
            (let ((%fluid-var-*tabular-stack* (list 'header)))
  ...
  ((hue
    (* 1/6
     (cond ((<= w 362.857) 5) ((< w 440) (+ 4 (/ (- w 440) -60))) ((< w 490) (- 4 (/ (- w 440) 50)))
      ((< w 510) (+ 2 (/ (- w 510) -20))) ((< w 580) (- 2 (/ (- w 510) 70)))
      ((< w 645) (/ (- w 645) -65)) (else 0))))
   (brightness
      ...
      (let
       ...
       (let
        ((y
          (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
           %read-res)))
        (ignorespaces 1.5) (rgb-frac-to-rrggbb (- 1 c) (- 1 m) (- 1 y))))))))
  ((:cmyk) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
       ...
       (let
        ...
        (let
         ((k
           (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
            %read-res)))
         (ignorespaces 1.5) (cmyk-to-rrggbb c m y k))))))))
  ((:rgb) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
      ...
      (let
       ...
       (let
        ((b
          (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
           %read-res)))
        (ignorespaces 1.5) (rgb-frac-to-rrggbb r g b)))))))
  ((:rgb255) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
      ...
      (let
       ...
       (let
        ((b
          (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
           %read-res)))
        (ignorespaces 1.5) (rgb-dec-to-rrggbb r g b)))))))
  ((:gray)
    ...
    (lambda (i)
     (let
      ((g
        (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
         %read-res)))
      (ignorespaces 1.5) (cmyk-to-rrggbb 0 0 0 (- 1 g))))))
  ((:gray15)
   (call-with-input-string (tex-string-to-html-string (get-token))
    (lambda (i)
     (let
      ((g
        (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
         %read-res)))
      (ignorespaces 1.5) (cmyk-to-rrggbb 0 0 0 (- 1 (/ g 15)))))))
  ((:html)
   (call-with-input-string (tex-string-to-html-string (get-token))
    (lambda (i) (let ((rrggbb (read-6hex i))) (ignorespaces 1.5) rrggbb))))
  ((:hsb) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
      ...
      (let
       ...
       (let
        ((b
          (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
           %read-res)))
        (ignorespaces 1.5) (hsb-to-rrggbb h s b)))))))
  ((:hsb360) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
      ...
      (let
       ...
       (let
        ((b
          (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
           %read-res)))
        (ignorespaces 1.5) (hsb-to-rrggbb (/ h 360) s b)))))))
  ((:hsb240) (bgroup)
   (call-with-input-string
    (tex-string-to-html-string
     (let ((%type 'string) (%ee (list "\\defcsactive\\,{ }" (get-token))))
      ...
      (let
       ...
       (let
        ((b
          (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
           %read-res)))
        (ignorespaces 1.5) (hsb-to-rrggbb (/ h 240) (/ s 240) (/ b 240))))))))
  ((:wave)
   (call-with-input-string (tex-string-to-html-string (get-token))
    (lambda (i)
     (let
      ((w
        (let ((%read-res (read i))) (when (eof-object? %read-res) (set! %read-res false))
         %read-res)))
      (ignorespaces 1.5) (wavelength-to-rrggbb w)))))
  (else
   (let ((name (get-peeled-group)))
    (let ((c (assoc name *color-names*))) (ignorespaces 1.5) (if c (cdr c) name))))))
...
(define (color-model-to-keyword model)
 (cond ((not model) ':colornamed) ((string=? model "rgb") ':rgb) ((string=? model "RGB") ':rgb255)
  ...
  ((string=? model "Hsb") ':hsb360) ((string=? model "HSB") ':hsb240)
  ((string=? model "wave") ':wave) (else ':colornamed)))
   ...
   (add-postlude-to-top-frame
    (case sw
     ...
     ((:cmy :cmyk :rgb :rgb255 :gray :gray15 :html :hsb :hsb360 :hsb240 :wave :colornamed)
      (emit "<span style=\"color: ") (emit (read-color-as-rrggbb sw)) (emit "\">")
      (lambda () (emit "</span>")))
...
(define (do-obeylines) (ignorespaces 1) (activate-cdef #\newline)
...
(define (do-block z) (do-end-para) (emit "<div ")
 (emit
  (case z ((:flushleft) "class=leftline") ((:flushright) "class=rightline")
...
(define (do-aftergroup) (ignorespaces 1.5)
...
(define (get-scaled-points)
 (let ((n (or (get-real) 1))) (ignorespaces 1.5)
  (* n
   (if (esc-char-p (snoop-actual-char)) (let ((x (get-ctl-seq))) (get-dimen x))
    (begin
     (let*
      ((%loop-returned false) (%loop-result 0)
       (return
...
(define (do-enquote) (ignorespaces 1.5)
 (cond
  ((and (char=? (snoop-actual-char) #\*)) (get-actual-char) (ignorespaces 1.5)
...
(define (do-refexternal)
 (let ((ext-file (get-peeled-group))) (do-ref-aux (get-label) ext-file false)))
  ...
  ((not (null? *missing-pieces*)) (write-log ':separation-newline)
   ...
   (cond
    ((member ':last-modification-time *missing-pieces*)
     (write-log "Last modification time not determined") (write-log ':separation-newline))
    (else false))
   (cond
   ...
   ((and (> n 0) (char=? (string-ref url 0) #\#))
    (let ((label (substring url 1)))
     (let ((label-ref (label-bound-p label)))
      (if label-ref
       (if (label*-src label-ref) false (list (label*-page label-ref) (label*-name label-ref)))
       false))))
   (else false))))
...
(define (do-htmladdimg)
 (let ((align-info (get-bracketed-text-if-any)))
  (let ((url (fully-qualify-url (get-url)))) (emit "<img src=\"") (emit url) (emit "\"")
...
(define (do-pdfximage)
 (let ((height false) (width false))
  ...
  (emit (fully-qualify-url (get-filename-possibly-braced))) (emit "\">") (ignorespaces 1.5)
  (get-ctl-seq) (ignorespaces 1.5) (get-ctl-seq)))
   ...
   (let ((pageno (table-get n *index-table*)))
    ...
    (cond
     ((begin (set! *it* (table-get pageno *index-page-mention-alist*)) *it*)
      (let ((n (add1 *it*))) (emit (number-to-roman n))
...
(define (html-length s)
 (let
  ((n (let ((%length-arg s)) ((if (string? %length-arg) string-length length) %length-arg))) (res 0)
   (i 0) (skip-tag false) (skip-entity false))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return
     (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
 ...
 (let ((txt (tex-string-to-html-string (get-group))))
  (let ((html-len (html-length txt)))
   (let ((txt-len (sp-to-pixels (tex-length html-len ':ex))))
    (emit "<span style=\"position: relative\">") (emit "<span style=\"position: absolute; left: -")
    (emit txt-len) (emit "pt\">") (emit txt) (emit "</span></span>")))))
 ...
 (let ((thing (get-bracketed-text-if-any)))
  (cond
   (thing (set! thing (string-trim thing))
    (cond
     ((not (string=? thing "")) (bgroup) (emit "<b>") (tex2page-string thing) (emit "</b>") (egroup))
     (else false)))
   (else false)))
 (emit "</dt><dd>"))
...
(define (do-plain-item n) (do-end-para) (emit-newline)
 (let ((parindent (sp-to-pixels (find-dimen "\\parindent")))) (emit "<p style=\"margin-left: ")
  ...
  (tex2page-string (get-group)) (ignorespaces 1.5) (emit-nbsp 2) (emit "</span></span>")))

(define (do-textindent)
 (let ((parindent (sp-to-pixels (find-dimen "\\parindent")))) (do-noindent)
...
(define (do-proclaim)
 (let ((head (tex-string-to-html-string (get-till-char #\.))))
  (let
   ((body (begin (get-actual-char) (ignorespaces 1.5) (tex-string-to-html-string (get-till-par)))))
   (do-end-para) (emit "<div class=\"proclaim medskip\"><b>") (do-noindent) (emit head)
   (emit ".</b>") (emit-nbsp 2) (emit "<i>") (emit body) (emit "</i>") (do-end-para) (emit "</div>")
   (do-para))))
...
(define (do-itemize) (do-end-para) (set! *tabular-stack* (cons ':itemize *tabular-stack*))
 (emit "<ul") (cond ((tex2page-flag-boolean "\\TZPslides") (emit " class=incremental")) (else false))
 (emit ">") (emit-newline))

(define (do-enditemize) (do-end-para) (pop-tabular-stack ':itemize) (emit "</ul>") (do-noindent))

(define (do-enumerate) (do-end-para) (set! *tabular-stack* (cons ':enumerate *tabular-stack*))
 (emit "<ol") (cond ((tex2page-flag-boolean "\\TZPslides") (emit " class=incremental")) (else false))
 (emit ">") (emit-newline))

(define (do-endenumerate) (pop-tabular-stack ':enumerate) (do-end-para) (emit "</ol>") (do-noindent))

(define (do-opmac-list-style) (ignorespaces 1.5) (set! *opmac-list-style* (get-actual-char))
 *opmac-list-style*)
...
(define (do-hspace) (ignorespaces 1.5)
 (cond ((eqv? (snoop-actual-char) #\*) (get-actual-char)) (else false)) (get-group) (emit-nbsp 3))

(define (do-vspace) (ignorespaces 1.5)
 (cond ((eqv? (snoop-actual-char) #\*) (get-actual-char)) (else false)) (get-group)
 (do-bigskip ':vspace))
  ...
  (let ((next-page (if (= *html-page-count* *last-page-number*) false (add1 *html-page-count*))))
   (cond
    ((not (= *last-page-number* 0)) (cond (prev-page (emit-page-link-start prev-page)) (else false))
     (emit "&#x3c;&#xb7;&#xb7;&#xb7;Prev ") (cond (prev-page (emit-link-stop)) (else false))
     (emit "||") (cond (next-page (emit-page-link-start next-page)) (else false))
     (emit " Next&#xb7;&#xb7;&#xb7;&#x3e;") (cond (next-page (emit-link-stop)) (else false)))
    (else false)))))

(define (output-head-or-foot-line head-or-foot)
 (cond
  ((not (tex2page-flag-boolean "\\TZPsinglepage")) (do-end-para) (emit "<div class=navigation>")
   (cond
      ...
      (let ((prev-page (if first-page-p false (sub1 *html-page-count*))))
       (let ((next-page (if last-page-p false (add1 *html-page-count*))))
        (cond
         ((not
           (and first-page-p
            (or last-page-p (and (eq? head-or-foot ':head) last-page-not-determined-p))))
          (emit "[") (emit *navigation-sentence-begin*) (emit "<span")
          (cond (first-page-p (emit " class=disable")) (else false)) (emit ">")
          (cond ((not first-page-p) (emit-page-link-start 0)) (else false))
          (emit *navigation-first-name*) (cond ((not first-page-p) (emit-link-stop)) (else false))
          (emit ", ") (cond ((not first-page-p) (emit-page-link-start prev-page)) (else false))
          ...
          (cond
           ((or *toc-page* *index-page*) (emit "<span")
            ...
            (cond
             (*toc-page* (emit "<span") (cond (toc-page-p (emit " class=disable")) (else false))
              (emit ">")
              (cond
               ((not toc-page-p)
                (emit-page-node-link-start *toc-page*
              ...
              (cond ((not toc-page-p) (emit-link-stop)) (else false)) (emit "</span>"))
             (else false))
            (cond
             (*index-page* (emit "<span") (cond (index-page-p (emit " class=disable")) (else false))
              ...
              (cond ((not index-page-p) (emit-link-stop)) (else false)) (emit "</span>"))
             (else false)))
           (else false))
...
(define (do-eject)
 (cond
  ((tex2page-flag-boolean "\\TZPslides") (do-end-para) (emit "</div>") (emit-newline)
   (emit "<div class=slide>") (do-para))
...
(define (output-html-preamble)
 (cond ((string? *doctype*) (emit "<!DOCTYPE ") (emit *doctype*) (emit ">") (emit-newline))
  (else false))
 ...
 (emit "<meta charset=\"utf-8\">") (emit-newline) (output-external-title) (link-stylesheets)
 (link-scripts) (emit "<meta name=robots content=\"index,follow\">") (emit-newline)
 (for-each emit *html-head*) (emit "</head>") (emit-newline) (emit "<body>") (emit-newline)
 (emit "<div") (cond ((tex2page-flag-boolean "\\TZPslides") (emit " class=slide")) (else false))
 (emit ">") (emit-newline))
...
(define (do-bye) (check-tex2page-lisp) (note-down-tex2page-flags)
 ...
 (cond
  ((not (null? *tex-env*))
   (trace-if true "\\end occurred inside a group at level " (length *tex-env*)))
  (else false))
 (perform-postludes)
 (cond
  ((not (or (>= *last-page-number* 0) (= *html-page-count* 0))) (flag-missing-piece ':last-page))
  (else false))
 (!last-page-number *html-page-count*) (write-aux `(!last-page-number ,*last-page-number*))
 (do-end-page)
 (cond
...
(define (note-down-tex2page-flags) (write-aux `(!lang ,(resolve-defs "\\TZPlang")))
 ...
 (cond
  ...
  (else false))
 (cond ((tex2page-flag-boolean "\\TZPcolophonlastpage") (write-aux `(!colophon ':last-page)))
  (else false))
 (cond
...
(define do-diacritic
 (let
  ...
  (lambda (diac)
   ...
   (emit
    (case diac ((:grave) "&#x300;") ((:acute) "&#x301;") ((:circumflex) "&#x302;")
     ((:umlaut) "&#x308;") ((:tilde) "&#x303;") ((:macron) "&#x304;") ((:dot) "&#x307;")
     ((:breve) "&#x306;") ((:hacek) "&#x30c;") ((:hungarianumlaut) "&#x30b;") ((:cedilla) "&#x327;")
     ((:dotunder) "&#x323;") ((:barunder) "&#x331;") ((:tieafter) "&#x361;") ((:ring) "&#x30a;")
     ((:ogonek) "&#x328;") (else (error 'ecase "0xdeadc0de")))))))
...
(define (do-mathdg)
 (let
  ((%fluid-var-*math-mode-p* true) (%fluid-var-*in-display-math-p* true)
   (%fluid-var-*tabular-stack* null) (%fluid-var-*ligatures-p* false))
  (fluid-let
...
(define (do-mathg)
 (let
  ((%fluid-var-*math-mode-p* true) (%fluid-var-*in-display-math-p* false)
   (%fluid-var-*tabular-stack* null) (%fluid-var-*ligatures-p* false))
  (fluid-let
   ((*ligatures-p* %fluid-var-*ligatures-p*) (*tabular-stack* %fluid-var-*tabular-stack*)
    (*in-display-math-p* %fluid-var-*in-display-math-p*) (*math-mode-p* %fluid-var-*math-mode-p*))
  ...
  ((:latex) (display "\\documentclass{" o) (display (if *using-chapters-p* "report" "article") o)
   ...
   (cond
    ((member ':includegraphics *imgpreamble-inferred*)
     (display "\\ifx\\includegraphics\\UNDEFINED" o) (display "\\usepackage{graphicx}\\fi" o)
     (newline o))
    (else false))
   (cond
...
(define (next-html-image-file-stem) (set! *img-file-count* (+ *img-file-count* 1))
 (let
  ((%type 'string) (%ee (list *subjobname* *img-file-suffix* (write-to-string *img-file-count*))))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
    (if (null? %ee) %res
     (let ((%a (car %ee)))
...
(define (do-math-fragment s . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (display-p false))
  ...
  (let
   ...
   (cond (display-p (emit "<table style=\"margin-left:auto; margin-right:auto\"><tr><td>"))
    (else false))
...
(define (tex-math-delim-string type)
 (let ((top null) (mid null) (bot null) (ext null))
  (case type
   ...
   ((:lbrace) (set! top "&#x23a7;") (set! mid "&#x23a8;") (set! bot "&#x23a9;") (set! ext "&#x23aa;")
    ext)
   ...
   ((:rbrace) (set! top "&#x23ab;") (set! mid "&#x23ac;") (set! bot "&#x23ad;") (set! ext "&#x23ae;")
    ext)
...
(define (do-math)
 (let ((display-p false))
  ...
  (let ((o (open-output-string))) (dump-till-char #\$ o)
   (cond
    (display-p
     (let ((c (get-actual-char)))
      (cond ((or (not c) (not (char=? c #\$))) (terror 'do-math "Display math should end with $$."))
       (else false))))
    (else false))
    ...
    (let*
     ...
     (let %loop ()
      (let ((c (snoop-actual-char))) (cond ((not c) (terror 'dump-till-ctl-seq)) (else false))
       (cond ((esc-char-p c) (let ((x (get-ctl-seq))) (if (string=? x cs) (return) (display x o))))
  ...
  (lambda (f)
   (cond
    ((not tex-prog-name)
     ...
     (cond ((not tex-prog-name) (set! tex-prog-name "xetex") tex-prog-name) (else false))
...
(define (ps-to-img/gif/netpbm ps-file img-file)
 (system
  (let
   ((%type 'string)
    (%ee
     (list *ghostscript* *ghostscript-options* " -sOutputFile=" img-file ".ppm.1 " ps-file
      " quit.ps")))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
 ...
 (system
  (let
   ((%type 'string)
    (%ee (list "ppmtogif -transparent rgb:ff/ff/ff < " img-file ".ppm > " *aux-dir/* img-file)))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
...
(define (ps-to-img/png/netpbm ps-file img-file)
 (system
  (let
   ((%type 'string)
    (%ee
 ...
 (system
  (let
   ((%type 'string)
    (%ee
     (list "pnmtopng -interlace -transparent \"#FFFFFF\" " " < " img-file ".ppm.tmp > " *aux-dir/*
      img-file)))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
...
(define (ps-to-img/jpeg/netpbm ps-file img-file)
 (system
  (let
   ((%type 'string)
    (%ee
 ...
 (system
  (let
   ((%type 'string) (%ee (list "ppmtojpeg --grayscale < " img-file ".ppm > " *aux-dir/* img-file)))
   (let ((%res (if (eq? %type 'string) "" null)))
    (let %concatenate-loop ((%ee %ee))
     (if (null? %ee) %res
      (let ((%a (car %ee)))
   ...
   (system
    (let ((%type 'string) (%ee (list "convert -transparent white -trim " ps-file " " img-file)))
     (let ((%res (if (eq? %type 'string) "" null)))
      ...
      %res))))
  (else
   (case (tex2page-flag-value "\\TZPimageformat") ((#\p #\P) (ps-to-img/png/netpbm ps-file img-file))
  ...
  ((img-file
    (let ((%type 'string) (%ee (list *aux-dir/* f (find-img-file-extn))))
     (let ((%res (if (eq? %type 'string) "" null)))
      ...
      %res))))
  (cond
   ((not (file-exists? img-file)) (write-log ':separation-space) (write-log #\{)
    (write-log
     (let ((%type 'string) (%ee (list f ".tex")))
      (let ((%res (if (eq? %type 'string) "" null)))
       (let %concatenate-loop ((%ee %ee))
    ...
    (cond
     ((begin (set! *it* (call-tex f)) *it*) (ps-to-img *it* img-file) (write-log img-file)
      '(for-each
        (lambda (e)
         (ensure-file-deleted
    ...
    (let*
     ...
     (let %loop ()
      (cond ((eat-word "height") (get-equal-sign) (set! height (get-pixels)) height)
       ...
       ((eat-word "enoughalready") (ignorespaces 1.5) (return)) (else (get-actual-char)))
      (if %loop-returned %loop-result (%loop))))
    (cond (height (emit " height=") (emit height)) (else false))
    (cond (width (emit " width=") (emit width)) (else false))))
  (else false))
 (emit " src=\"") (emit (fully-qualify-url image-file)) (emit "\" ") (emit " alt=\"")
...
(define (do-box) (ignorespaces 1.5) (get-to) (eat-dimen) (ignorespaces 1.5)
...
(define (do-tex-frac) (ignorespaces 1.5)
 (let
  ...
  (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*))
   (cond
    (inner-level-p (emit "<sup>") (tex2page-string (get-till-char #\/)) (emit "</sup>/<sub>")
     (get-actual-char) (ignorespaces 1.5) (tex2page-string (get-token)) (emit "</sub>"))
    (else (emit "</td><td><table class=leftline><tr><td class=centerline>")
     (tex2page-string (get-till-char #\/)) (get-actual-char) (ignorespaces 1.5) (emit "<hr noshade>")
     (tex2page-string (get-token)) (emit "</td></tr></table></td><td>"))))))

(define (do-eqno)
 (cond ((not *in-display-math-p*) (terror 'do-eqno "You can't use \\eqno in math mode"))
  (else false))
 (emit "</td><td width=10% class=rightline>"))

(define (do-eqalign type) (ignorespaces 1.5)
 (let ((c (get-actual-char))) (cond ((not c) (terror 'do-eqalign "Missing {")) (else false))
  ...
  (cond (*in-display-math-p* (emit "</td><td>")) (else false)) (emit-newline)
  (emit "<table><tr><td>")))
...
(define (do-pmatrix) (ignorespaces 1.5)
 (let ((c (get-actual-char)))
 ...
 (let*
  ((%loop-returned false) (%loop-result 0)
   (return
    (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
  (let %loop ()
   (let ((c (get-actual-char))) (cond ((or (not c) (char=? c d)) (return)) (else false)))
   (if %loop-returned %loop-result (%loop)))))
 ...
 (let
  ...
  (let
   ...
   (if (not f) false
    (let ((f (string-trim f)))
     (cond
      ((= (let ((%length-arg f)) ((if (string? %length-arg) string-length length) %length-arg)) 0)
       false)
...
(define (initialize-scm-words) (set! *scm-keywords* (make-table ':test equal?))
 ...
 (for-each (lambda (s) (table-put! s *scm-keywords* true) (table-get s *scm-keywords*))
  '("=>" "and" "begin" "begin0" "case" "cond" "define" "define-macro" "define-syntax" "defmacro"
    "defstruct" "delay" "do" "else" "flet" "fluid-let" "if" "labels" "lambda" "let" "let*"
    "let-syntax" "let-values" "letrec" "letrec-syntax" "macrolet" "or" "quasiquote" "quote" "set!"
    "syntax-case" "syntax-rules" "unless" "unquote" "unquote-splicing" "when" "with" "with-handlers"
    "assert" "block" "decf" "defpackage" "defparameter" "defun" "defvar" "destructuring-bind"
    "do-all-symbols" "do-external-symbols" "do-symbols" "dolist" "dotimes" "ecase" "etypecase"
    "eval-when" "handler-bind" "handler-case" "incf" "loop" "multiple-value-bind"
    "multiple-value-setq" "pop" "prog1" "progn" "push" "setf" "setq" "typecase" "unwind-protect"
    "with-input-from-string" "with-open-file" "with-open-socket" "with-open-stream"
...
(define (initialize-globals) (set-start-time) (tex-def-count "\\language" 256 true)
 ...
 (tex-gdef-0arg "\\TZPcolophoncredit" "1") (tex-gdef-0arg "\\TZPcolophonweblink" "1")
 (tex-gdef-0arg "\\TZPimageformat" "PNG") (tex-gdef-0arg "\\TZPimageconverter" "NetPBM")
...
(define (do-newcommand renewp) (ignorespaces 1.5)
...
(define (do-newenvironment renewp) (ignorespaces 1.5)
     ...
     (let
      ...
      (let
       ...
       (let ((ok-to-def-p (or renewp (not (find-def bs-envname)))))
        (tex-def bs-envname (latex-argnum-to-plain-argpat argc) beginning optarg false false false
         false)
        (tex-def
        ...
        (cond ((not ok-to-def-p) (trace-if true "{" envname "} already defined"))
         (else false))))))))))
     ...
     (let ((within (if numbered-like false (get-bracketed-text-if-any))))
      (let
       ...
       (tex-def
        ...
        (let
         ((%type 'string)
          (%ee (list "\\par\\begingroup\\TIIPtheorem{" counter-name "}{" caption "}")))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
 ...
 (let ((n n) (s null))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return
     (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
   (let %loop () (cond ((<= n 0) (return s)) (else false))
        ...
        (let %concatenate-loop ((%ee %ee))
         (if (null? %ee) %res
          (let ((%a (car %ee)))
           ...
           (%concatenate-loop (cdr %ee)))))
        %res))))
    (write-log #\() (write-log f) (write-log ':separation-space) (valid-img-file-p f)
    (emit "<img src=\"") (emit img-file) (emit "\"") (emit " style=\"border: 0\"") (emit " alt=\"")
        ...
        (let %loop ()
         (let ((c (snoop-actual-char)))
          ...
          (cond (escape-p (set! s (cons (get-actual-char) s)) (set! escape-p false) escape-p)
           ((char=? c c0) (return s))
           ((esc-char-p c) (set! s (cons (get-actual-char) s)) (set! escape-p true) escape-p)
...
(define (do-halign) (do-end-para) (ignorespaces 1.5)
 (let ((c (get-actual-char)))
    ...
    (let*
     ...
     (let %loop () (ignorespaces 1.5)
      (let ((c (snoop-actual-char)))
       ...
       (cond
        ((char=? c #\}) (get-actual-char) (emit "</table>") (emit-newline) (egroup) (do-para)
         (return))
          ...
          ((%loop-returned false) (%loop-result 0)
           (return
            (lambda %args (set! %loop-returned true)
             (set! %loop-result (and (pair? %args) (car %args))))))
          (let %loop ()
           (cond ((null? tmplt) (terror 'expand-halign-line "Eof in \\halign")) (else false))
           (let ((y (car tmplt)))
            (case y
             ((()) (emit "<td>")
              (tex2page-string
              ...
              (if (string=? x "\\cr")
               (begin (emit "</tr>") (emit-newline)
                (begin (set! outer-loop-done true) outer-loop-done) (return))
               (begin
...
(define (read-till-next-sharp k argpat) (ignorespaces 1.5)
      ...
      (let %loop () (set! c (if (< i n) (list-ref argpat i) #\#))
       ...
       (unless %loop-returned
        (let ((d (snoop-actual-char)))
         (cond
          ((and (char=? c #\space) (char-whitespace? d)) (ignorespaces 1.5) (set! i (+ i 1))
           (set! s (cons c s)) s)
     ...
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true)
        (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop ()
      (cond ((>= k n) (cond ((= k 0) (ignorespaces 1)) (else false)) (return r)) (else false))
      (unless %loop-returned
       (let ((c (list-ref argpat k)))
        (cond
         ((char=? c #\#)
          (cond ((= k (sub1 n)) (ignorespaces 1.5) (set! r (cons (get-till-char #\{) r)) (return r))
            ...
            (let ((x (get-ctl-seq))) (toss-back-char *invisible-space*)
             (cond
              ((or (string=? x "\\the") (string=? x "\\number"))
               (let ((x2 (get-raw-token/is))) (toss-back-char *invisible-space*)
                (toss-back-string x2)
                (cond
                 ((ctl-seq-p x2)
 ...
 (let ((k 0))
  (let
   ...
   (let ((args (read-macro-args argpat k r)))
    (let
     ((rhs-n
       (let ((%length-arg rhs)) ((if (string? %length-arg) string-length length) %length-arg))))
...
(define (do-verbatimescapechar) (ignorespaces 1.5)
 (let ((c1 (get-actual-char)))
  (let ((c2 (get-actual-char)))
   (cond ((not (esc-char-p c1)) (terror 'do-verbatimescapechar "Arg must be \\<char>")) (else false))
    ...
    (let %loop ()
     (let ((c (get-actual-char)))
      ...
      (cond
       ...
       ((char=? c #\}) (cond ((= nesting 0) (return)) (else false)) (emit #\})
...
(define (do-verb) (ignorespaces 1.5) (bgroup)
...
(define (get-char-definitely c0) (ignorespaces 1.5)
 (let ((c (get-actual-char)))
  (cond ((not c) (terror 'get-char-defnitely "Runaway argument")) (else false))
  (cond ((not (char=? c c0)) (terror 'get-char-defnitely "Missing" c0)) (else false))))

(define (get-char-optionally cc) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (cond ((not c) false) ((member c cc) (get-actual-char) c) (else false))))

(define (get-unsigned-number-optionally) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
        ...
        ((n
          (and f
           (let ((n (table-get f *opmac-verbinput-table*)))
            (or n
             (begin
              (begin (table-put! f *opmac-verbinput-table* 0) (table-get f *opmac-verbinput-table*))
              0))))))
        (cond
         ((and f (file-exists? f)) (do-end-para) (bgroup) (emit "<pre class=verbatim>")
          (let ((i (open-input-file f)))
           (let
            ((%with-open-file-res
              (cond
               ((and s1 n1 s2 n2 (char=? s1 #\-) (char=? s2 #\+))
                (opmac-verbinput-skip-lines i (+ n n1)) (opmac-verbinput-print-lines i n2)
                (set! n (+ n (+ n1 n2))) n)
               ...
               ((and (not s1) n1 s2 (not n2) (char=? s2 #\-))
                (opmac-verbinput-skip-lines i (sub1 n1)) (opmac-verbinput-print-lines i true)
                (set! n 0) n)
            ...
            (cond (i ((if (input-port? i) close-input-port close-output-port) i)) (else false))
            %with-open-file-res))
          (table-put! f *opmac-verbinput-table* n) (table-get f *opmac-verbinput-table*)
          (emit "</pre>") (egroup) (do-para))
         (else (non-fatal-error "File " f0 " not found")))))))))))
...
(define (dump-groupoid p) (ignorespaces 1.5)
    ...
    (let ((nesting 0) (c false))
     (let*
      ...
      (let %loop () (set! c (get-actual-char))
       ...
       (cond ((esc-char-p c) (write-char c p) (write-char (get-actual-char) p))
        ((char=? c #\{) (write-char c p) (set! nesting (+ nesting 1)) nesting)
        ((char=? c #\})
         (cond ((= nesting 0) (return))
          (else (write-char c p) (set! nesting (- nesting 1)) nesting)))
        (else (write-char c p)))
       (if %loop-returned %loop-result (%loop))))))
   (else
    (let*
...
(define (do-makehtmlimage) (ignorespaces 1.5)
 (cond
...
(define (do-string)
 (let ((c (snoop-actual-char)))
  (cond ((not c) false)
   ...
   ((comment-char-p c) (eat-till-eol) (do-string)) (else (toss-back-char (get-actual-char))))))
  ...
  (lambda ()
   (let*
    ((%loop-returned false) (%loop-result 0)
     (return
      (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
    (let %loop () (ignorespaces 2) (cond ((not (snoop-actual-char)) (return)) (else false))
 ...
 (let ((esc-p false))
  (let*
   ...
   (let %loop ()
    (let ((c (get-actual-char)))
     (case c
      ((#\") (cond ((not esc-p) (return)) (else false)) (scm-emit-html-char c) (set! esc-p false)
       esc-p)
...
(define (scm-display-token s)
 (let
  ((n (let ((%length-arg s)) ((if (string? %length-arg) string-length length) %length-arg))) (k 0))
  (let*
   ((%loop-returned false) (%loop-result 0)
    (return
     (lambda %args (set! %loop-returned true) (set! %loop-result (and (pair? %args) (car %args))))))
...
(define (do-scm-braced result-p) (get-actual-char)
 (let ((display-p (munched-a-newline-p)))
  ...
  (bgroup)
  (let ((%fluid-var-*catcodes* *catcodes*) (%fluid-var-*verb-display-p* display-p) (nesting 0))
          ...
          (cond ((member x '("\\ " "\\{" "\\}")) (scm-emit-html-char (string-ref x 1)))
           (else
            (let ((%fluid-var-*catcodes* *catcodes*))
             (fluid-let ((*catcodes* %fluid-var-*catcodes*)) (catcode #\\ 0)
              (do-tex-ctl-seq-completely x)))))))
        ((char=? c #\{) (get-actual-char) (scm-emit-html-char c) (set! nesting (+ nesting 1))
         nesting)
        ((char=? c #\}) (get-actual-char) (cond ((= nesting 0) (return)) (else false))
         (scm-emit-html-char c) (set! nesting (- nesting 1)) nesting)
        (else (scm-output-next-chunk))))
      (if %loop-returned %loop-result (%loop))))))
  (egroup) (if (not display-p) (emit "</code>") (begin (emit "</pre>") (do-noindent)))))
     ...
     (let*
      ...
      (let %loop ()
       (let ((c (snoop-actual-char)))
        ...
        (cond ((char=? c d) (get-actual-char) (return)) (else false)) (scm-output-next-chunk))
       (if %loop-returned %loop-result (%loop))))))
...
(define (do-scm . %lambda-rest-arg)
 (let ((%lambda-rest-arg-len (length %lambda-rest-arg)) (result-p false))
  ...
  (cond (*outputting-external-title-p* (do-verb))
   (else (ignorespaces 1.5) (bgroup)
    (let ((%fluid-var-*ligatures-p* false))
     (fluid-let ((*ligatures-p* %fluid-var-*ligatures-p*))
      ((if (char=? (snoop-actual-char) #\{) do-scm-braced do-scm-delimed) result-p)))
    (egroup)))))

(define (do-scminput) (ignorespaces 1.5) (do-end-para) (bgroup) (emit "<pre class=scheme>")
 (let ((f (add-dot-tex-if-no-extension-provided (get-filename-possibly-braced))))
     ...
     ((%loop-returned false) (%loop-result 0)
      (return
       (lambda %args (set! %loop-returned true)
        (set! %loop-result (and (pair? %args) (car %args))))))
     (let %loop ()
      (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false)) (scm-output-next-chunk))
      (if %loop-returned %loop-result (%loop)))))))
...
(define (scm-get-type s)
 (cond ((table-get s *scm-special-symbols*) ':special-symbol)
  ((table-get s *scm-keywords*) ':keyword) ((table-get s *scm-builtins*) ':builtin)
  ((table-get s *scm-variables*) ':variable) ((string-is-flanked-by-stars-p s) ':global)
  ((begin
    (set! *it*
...
(define (do-cr z) (ignorespaces 1.5)
 (case (car *tabular-stack*)
  ...
  ((:eqalign :eqalignno :displaylines :pmatrix :mathbox)
   (cond
    ((not (char=? (snoop-actual-char) #\})) (set! *math-height* (+ *math-height* 1))
     (emit "</td></tr>") (emit-newline) (emit "<tr><td class=centerline>")
   ...
   ((not mathp)
    (add-postlude-to-top-frame
     (let ((old-math-mode-p *math-mode-p*) (old-in-display-math-p *in-display-math-p*))
      ...
      (lambda () (set! *math-mode-p* old-math-mode-p)
       (set! *in-display-math-p* old-in-display-math-p) *in-display-math-p*))))
   (else false))
  (let
   ((border-width
     (if
      (let ((%position-v #\|) (%position-s (get-group)))
       (cond ((string? %position-s) (string-index %position-s %position-v))
        (else (list-position %position-v %position-s))))
      1 0)))
   (set! *tabular-stack* (cons ':tabular *tabular-stack*)) (emit "<table border=")
   (emit border-width) (emit "><tr><td valign=top ") (do-tabular-multicolumn))))
...
(define (do-ruledtable-colsep) (emit-newline) (emit "</td><td") (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (cond
   ((char=? c #\\)
    (let ((x (get-ctl-seq)))
...
(define (do-flipcase kase)
 (emit
  (map 'string (case kase ((:uccode) tex-char-upcase) ((:lccode) tex-char-downcase)) (get-token))))
...
(define (set-latex-counter-aux counter-name addp new-value)
 (cond
  ((begin (set! *it* (table-get counter-name *dotted-counters*)) *it*)
   (let ((counter *it*))
    (if addp
     (begin (set!counter*-value counter (+ (counter*-value counter) new-value))
      (counter*-value counter))
...
(define (tex-math-cal c)
 (let
  ((%type 'string)
   (%ee
    (list "&#x" (write-to-string (+ 120016 (- (char->integer c) (char->integer #\A))) ':base 16)
     ";")))
  (let ((%res (if (eq? %type 'string) "" null)))
   (let %concatenate-loop ((%ee %ee))
    (if (null? %ee) %res
...
(define (do-tex-char c)
 (cond ((comment-char-p c) (do-comment)) ((inside-false-world-p) true) ((char=? c #\{) (bgroup))
  ((char=? c #\}) (egroup)) ((char=? c #\$) (do-math)) ((char=? c #\-) (do-hyphen))
  ((char=? c #\`) (do-lsquo)) ((char=? c #\') (do-rsquo)) ((char=? c #\~) (emit-nbsp 1))
    ...
    (*tabular-stack*
     (case (car *tabular-stack*)
      ...
      ((:eqalignno) (set! *equation-position* (add1 *equation-position*)) (emit "</td><td")
       (cond ((= *equation-position* 2) (emit " width=30% class=rightline")) (else false))
       (emit ">"))
      ((:eqnarray :eqnarray*) (set! *equation-position* (add1 *equation-position*)) (emit "</td><td")
       (cond ((= *equation-position* 1) (emit " class=centerline width=2%")) (else false))
       (emit ">"))
...
(define (do-input) (ignorespaces 1.5)
 (let ((f (get-filename-possibly-braced)))
  (let ((boilerplate-index *inputting-boilerplate-p*))
   (cond
    ((eqv? *inputting-boilerplate-p* 0) (set! *inputting-boilerplate-p* false)
...
(define (do-includeonly) (ignorespaces 1.5)
 (cond ((eq? *includeonly-list* true) (set! *includeonly-list* null) *includeonly-list*)
  (else false))
 (let ((c (get-actual-char)))
   ...
   (let*
    ...
    (let %loop () (ignorespaces 1.5)
     (let ((c (snoop-actual-char))) (cond ((not c) (terror 'do-includeonly)) (else false))
      (cond ((comment-char-p c) (eat-till-eol)) ((char=? c #\,) (get-actual-char))
 ...
 (let
  ((run-bibtex-p
    (cond ((not *using-bibliography-p*) false)
     ...
     (*source-changed-since-last-run-p* (flag-missing-piece ':fresh-bibliography) true)
     (else false)))
   (run-makeindex-p
  ...
  (cond
   (run-bibtex-p (write-log ':separation-newline) (write-log "Running: bibtex ")
    (write-log *aux-dir/*) (write-log *jobname*) (write-log *bib-aux-file-suffix*)
    (write-log #\space)
    (system
    ...
    (cond
     ((not
       (file-exists?
        (let ((%type 'string) (%ee (list *jobname* *bib-aux-file-suffix* ".bbl")))
         (let ((%res (if (eq? %type 'string) "" null)))
          (let %concatenate-loop ((%ee %ee))
           (if (null? %ee) %res
            (let ((%a (car %ee)))
  ...
  (for-each
   (lambda (f)
    (cond
     ((file-exists? f) (write-log ':separation-newline) (write-log "Running: metapost ")
      (write-log f) (write-log ':separation-newline) (call-mp f))
     (else false)))
   *mp-files*)
  (for-each
 ...
 (let ((home (retrieve-env "HOME")))
  (and home
   (let
    ((slash-already-p
      (let
       ((n (let ((%length-arg home)) ((if (string? %length-arg) string-length length) %length-arg))))
...
(define (make-target-dir)
 (let
  ...
  (cond
   (hdir-file
    (let
     ((hdir (call-with-input-file/buffered hdir-file (lambda () (get-filename-possibly-braced)))))
     (cond
      ((not
        (= (let ((%length-arg hdir)) ((if (string? %length-arg) string-length length) %length-arg))
         0))
       (system
...
(define (load-aux-file)
      ...
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
         ...
         (%concatenate-loop (cdr %ee)))))
      %res))))
  (cond ((file-exists? aux-file) (load-tex2page-data-file aux-file) (delete-file aux-file))
   (else false)))
 (start-css-file)
 ...
 (cond ((not (null? *stylesheets*)) (set! *stylesheets* (reverse *stylesheets*)) *stylesheets*)
  (else false))
 (cond ((not (null? *html-head*)) (set! *html-head* (reverse *html-head*)) *html-head*)
  (else false)))
...
(define (!index index-number html-page-number)
 (table-put! index-number *index-table* html-page-number) (table-get index-number *index-table*))
...
(define (load-tex2page-data-file f)
 (let
  ((i
    (let*
     ((%f f) (%ee (list ':direction ':input ':if-does-not-exist false))
      (%direction (memv ':direction %ee)) (%if-exists (memv ':if-exists %ee))
      (%if-does-not-exist ':error) (%if-does-not-exist-from-user (memv ':if-does-not-exist %ee)))
     (when %direction (set! %direction (cadr %direction)))
     (when %if-exists (set! %if-exists (cadr %if-exists)))
  ...
  (let
   ((%with-open-file-res
     (cond
      (i
       (let
        ((%fluid-var-*current-source-file* f) (%fluid-var-*input-line-no* 0) (e false)
         (directive false))
        (fluid-let
         ((*input-line-no* %fluid-var-*input-line-no*)
          (*current-source-file* %fluid-var-*current-source-file*))
           ...
           (unless %loop-returned
            (apply
             (case directive ((!colophon) !colophon) ((!default-title) !default-title)
              ...
              ((!external-labels) !external-labels) ((!foot-line) !foot-line)
              ((!head-line) !head-line) ((!html-head) !html-head) ((!html-redirect) !html-redirect)
              ((!index) !index) ((!index-page) !index-page)
              ((!infructuous-calls-to-tex2page) !infructuous-calls-to-tex2page) ((!label) !label)
              ((!lang) !lang) ((!last-modification-time) !last-modification-time)
              ...
              (else
               (terror 'load-tex2page-data-file "Fatal aux file error " directive "; I'm stymied.")))
             (cdr e)))
           (if %loop-returned %loop-result (%loop)))))))
      (else false))))
   (cond (i ((if (input-port? i) close-input-port close-output-port) i)) (else false))
...
(define (do-math-ctl-seq s)
 (cond ((begin (set! *it* (find-math-def s)) *it*) ((tdef*-thunk *it*)))
  (else
   (cond ((not *math-needs-image-p*) (set! *math-needs-image-p* true) *math-needs-image-p*)
    (else false))
   (emit (substring s 1)))))
...
(define (do-not) (ignorespaces 1.5)
 (let ((c (snoop-actual-char)))
  (if (esc-char-p c)
   (let ((x (get-ctl-seq)))
    (emit
...
(for-each texbook-18-2
 '("arccos" "arcsin" "arctan" "arg" "cos" "cosh" "cot" "coth" "csc" "deg" "det" "dim" "exp" "gcd"
   "hom" "inf" "ker" "lg" "lim" ("liminf" "lim inf") ("limsup" "lim sup") "ln" "log" "max" "min" "Pr"
...
(tex-def-prim "\\MF"
 (let
  ((mf
    (let
     ((%type 'string) (%ee (list "<span style=\"" "font-family: sans-serif" "\">METAFONT</span>")))
     (let ((%res (if (eq? %type 'string) "" null)))
      (let %concatenate-loop ((%ee %ee))
       (if (null? %ee) %res
        (let ((%a (car %ee)))
   ...
   (let ((rotated false))
    (let ((width false))
     (let ((img-file-stem (next-html-image-file-stem)))
      (let
       ((img-file
         (let ((%type 'string) (%ee (list *aux-dir/* img-file-stem (find-img-file-extn))))
          (let ((%res (if (eq? %type 'string) "" null)))
           (let %concatenate-loop ((%ee %ee))
            (if (null? %ee) %res
             (let ((%a (car %ee)))
       ...
       (emit img-file) (emit "\"") (cond (height (emit " height=") (emit height)) (else false))
       (cond
 ...
 (call-with-input-string/buffered (ungroup (get-group))
  (lambda ()
   (let*
    ...
    (let %loop () (ignorespaces 2)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (let ((s (scm-get-token))) (table-put! s *scm-builtins* false)
       (table-put! s *scm-variables* false) (table-put! s *scm-keywords* true)
 ...
 (call-with-input-string/buffered (ungroup (get-group))
  (lambda ()
   (let*
    ...
    (let %loop () (ignorespaces 2)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (let ((s (scm-get-token))) (table-put! s *scm-keywords* false)
       (table-put! s *scm-variables* false) (table-put! s *scm-builtins* true)
  ...
  (lambda ()
   (let*
    ...
    (let %loop () (ignorespaces 2)
     (let ((c (snoop-actual-char))) (cond ((not c) (return)) (else false))
      (let ((s (scm-get-token))) (table-put! s *scm-builtins* false)
       (table-put! s *scm-keywords* false) (table-put! s *scm-variables* true)
...
(define (do-index)
 (let ((idx-entry (ungroup (get-group)))) (ignorespaces 1.5)
...
(define (do-indexitem indent) (set! *index-page-mention-alist* (make-table)) (emit "<br>")
 (emit-newline) (emit-nbsp (* indent 4)))
...
(define (do-xrdef)
 (let ((tag (get-peeled-group))) (do-tag-aux tag (write-to-string *html-page-count*))))
 ...
 (let ((label-ref (label-bound-p (get-peeled-group))))
  (if label-ref
   (let ((pageno (label*-page label-ref)))
    (emit-ext-page-node-link-start (label*-src label-ref) pageno false) (emit pageno)
    (emit-link-stop))
   (non-fatal-error "***"))))
         ...
         (call-with-html-image-port
          (lambda (o)
           (cond
            ...
            (else false))
           (cond
            ((not (= epsf-y-size 0)) (display "\\epsfysize=" o) (display epsf-y-size o)
             (display "sp" o) (newline o))
            (else false))
    ...
    (if *outer-p*
     (ungroup
      (let ((%fluid-var-*catcodes* *catcodes*) (%fluid-var-*expand-escape-p* true))
       (fluid-let
        ((*expand-escape-p* %fluid-var-*expand-escape-p*) (*catcodes* %fluid-var-*catcodes*))
...
(define (do-thebibliography) (do-end-para) (get-group)
 ...
 (tex2page-string "\\def\\newblock{ }") (emit "<table>") (munch-newlines) (emit-newline))
  ...
  (let ((bibitem-num-s (write-to-string *bibitem-num*)))
   (let
    ...
    (let
     ...
     (tex-def-0arg "\\@currentlabel" bibmark) (emit-anchor node-name) (emit "[")
     (tex2page-string bibmark) (emit "]") (emit-nbsp 2) (do-label-aux key) (emit "</td><td>"))))))
...
(define (do-bibliographystyle-help s) (write-bib-aux "\\bibstyle{") (write-bib-aux s)
   ...
   (let ((first-key-p true) (key false))
    (let*
     ...
     (let %loop () (set! key (get-csv closing-delim)) (cond ((not key) (return)) (else false))
      ...
      (unless %loop-returned (write-bib-aux key)) (unless %loop-returned (write-bib-aux "}"))
...
(define (do-nocite) (ignorespaces 1.5)
...
(define (do-color)
 (let ((model (color-model-to-keyword (get-bracketed-text-if-any)))) (do-switch model)))
...
(define (do-definecolor)
 (let ((name (get-peeled-group)))
  (let ((model (color-model-to-keyword (get-peeled-group))))
   (set! *color-names* (cons (cons name (read-color-as-rrggbb model)) *color-names*))
   *color-names*)))
...
(define (do-opmac-heading seclvl) (ignorespaces 1.5)
 (let
  ((header
    (let ((%fluid-var-*tabular-stack* (list ':header)))
     (fluid-let ((*tabular-stack* %fluid-var-*tabular-stack*))
...
(define (do-opmac-ii retainp)
 (let ((lhs (get-word)))
  (let ((sub (and *opmac-index-sub-table* (table-get lhs *opmac-index-sub-table*))))
   (if retainp (toss-back-string lhs) (ignorespaces 1.5))
   (do-index-help
    (cond (sub sub)
...
(tex-def-prim "\\ignorenextinputtimestamp"
 (lambda ()
  (cond
   ((not *inputting-boilerplate-p*) (set! *inputting-boilerplate-p* 0) *inputting-boilerplate-p*)
   (else false))))
...
(define (do-verbatim)
 (if (eqv? *tex-format* ':latex) (do-verbatim-latex "verbatim") (do-verbatim-eplain)))
...
(define (do-latex-display-math)
 (do-display-math
  (let ((o (open-output-string))) (dump-till-ctl-seq "\\]" o) (get-output-string o))))
    ...
    (let
     ((xe
       (let
        ((%type 'string)
         (%ee
          (list "X" "<span style=\"" "text-transform: uppercase; " "position: relative; "
           "top: 0.5ex; " "margin-left: -0.125em; " "margin-right: -0.1667em" "\">&#x1dd;</span>")))
        (let ((%res (if (eq? %type 'string) "" null)))
         (let %concatenate-loop ((%ee %ee))
          (if (null? %ee) %res
           (let ((%a (car %ee)))
...
(define (tex2page tex-file) (cond ((not (= *write-log-index* 0)) (newline)) (else false))
 (cond ((or (not tex-file) (string=? tex-file "")) (set! tex-file "--missing-arg") tex-file)
  (else false))
 (let
  ((%fluid-var-*afterassignment* false) (%fluid-var-*afterbye* null) (%fluid-var-*afterpar* null)
   ...
   (%fluid-var-*external-label-tables* (make-table ':test equal?)) (%fluid-var-*footnote-list* null)
   ...
   (%fluid-var-*temp-string-count* 0) (%fluid-var-*temporarily-use-utf8-for-math-p* false)
   (%fluid-var-*tex-env* null) (%fluid-var-*tex-format* ':plain) (%fluid-var-*tex-if-stack* null)
   (%fluid-var-*tex-like-layout-p* *tex-like-layout-p*) (%fluid-var-*tex-output-format* false)
   (%fluid-var-*tex2page-inputs* (string=split (retrieve-env "TEX2PAGEINPUTS") *path-separator*))
  ...
  (fluid-let
   ((*write-log-possible-break-p* %fluid-var-*write-log-possible-break-p*)
    ...
    (*tex-like-layout-p* %fluid-var-*tex-like-layout-p*) (*tex-if-stack* %fluid-var-*tex-if-stack*)
    ...
    (*in-display-math-p* %fluid-var-*in-display-math-p*) (*in-alltt-p* %fluid-var-*in-alltt-p*)
    ...
    (*html-only* %fluid-var-*html-only*) (*html-head* %fluid-var-*html-head*)
    (*html* %fluid-var-*html*) (*graphics-file-extensions* %fluid-var-*graphics-file-extensions*)
    ...
    (*esc-char-verb* %fluid-var-*esc-char-verb*) (*equation-position* %fluid-var-*equation-position*)
