...
\q{tex2page} whose one exported symbol is \q{tex2page}.  Thus, the
...
`\definecolor`~\cite{grfguide}.  These are provided by the \LaTeX\
package `color.sty`, which can also be loaded into
plain \TeX\ (after loading `miniltx`).  E.g.,
...
In addition to the {\em color models} `cmyk`, `gray`, `hsb`, `rgb`,
and `RGB`, \TeX2page also supports `cmy`, `Gray`, `Hsb`,
`HSB`, `HTML`, and `wave`, which are defined in `xcolor.sty` (a
\LaTeX\ package that supersets `color.sty`). Unfortunately,
`xcolor.sty` is currently not loadable in plain \TeX.

`\definecolor` defines new color names, e.g.,

\begintt
\definecolor{dodgerblue}{rgb}{.116, .565, 1}
\definecolor{orangered}{rgb}{1, .27, 0}
\definecolor{thistle}{cmyk}{.12, .59, 0, 0}
Poor Mr Norrell could only watch on helplessly as the gentleman
with the {\color{thistle}thistle-down hair} seized his only
...
presentation doesn’t need WiFi.
...
Consult the Slidy documentation for the keys and clicks you can use
...
`\activettchar`\f{\verb{\activettchar`} lets you write
